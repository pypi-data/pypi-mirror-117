...
class TelegramWebhookProxy(BaseProxy):
    ...
    async def configure_webhook(self, **kwargs: typing.Any) -> None:
        ...
        API method `set_webhook` like this: self.advanced.bot.set_webhook()

        """
        url = self._webhook_domain + self.prefix
        if isinstance(self.execution_path, str):
            url += self.execution_path
        await self.dispatcher.bot.set_webhook(url, certificate=self._ssl_context, **kwargs)
