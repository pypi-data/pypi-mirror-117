...
class Colony:
    ...
    def submit_new_user_survey(self, account_id: str, survey: NewUserSurveyRequest):
    ...
    def is_new_user_survey_taken(self, account_id: str) -> bool:
        method_url = self._api_root.account.is_new_user_survey_taken(account_id)
        resp = self.session.get(url=method_url)
        GatewayUtils.handle_response(response=resp, return_codes=[200])
        return Utils.str_to_bool(resp.text)
