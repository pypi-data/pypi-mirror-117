...
def convert_result_df_to_data_as_rows(result_df: pd.DataFrame):
    data_rows = result_df.toPandas()[
        ["label", "base_perc", "target1_abs", "target1_perc", "target1_dev", "target2_abs", "target2_perc", "target2_dev"]].to_numpy().copy().tolist()
    for i, row in enumerate(data_rows):
        ...
        for j, value in enumerate(row):
            ...
            if j in [4, 7]:
                ...
                data_rows[i][j] = str(data_rows[i][j]).replace('[', '').replace(']', '')
