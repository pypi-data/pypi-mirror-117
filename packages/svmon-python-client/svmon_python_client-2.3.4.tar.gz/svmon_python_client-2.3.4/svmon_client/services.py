...
def get_svmon_version(svmon_app_path):
    ...
    print('test')
    print(svmonVersionExecResult[0])

    if not isinstance(svmonVersionExecResult[0], string):
        print("Svmon path executable is incorrect or no versions were found")
        exit(1)

    svmonVersionsArray = svmonVersionExecResult[0].split(',')
