...
def _get_normalized_data(column, normalizer):
    ...
    if normalizer == "min_max":
        y_std = _Normalizer.min_max(column.dropna())
    elif normalizer == "mean":
        y_std = _Normalizer.mean(column.dropna())
    elif normalizer == "z_score":
        y_std = _Normalizer.z_score(column.dropna())
    return y_std
...
def _chi_squared_goodness_of_fit(data, distribution, number_of_bins):
    ...
    dist = _Distributions().get_dist(distribution)
    size = len(data)
    param = dist.fit(data)
...
def _kolmogorov_goodness_of_fit(data, distribution, outlier_removal=False):
...
def distribution_discretizer(dataset, number_of_bins, statistical_test="chi2", cutoff_margin=0.2, kolmogorov_opt=False,
    ...
    if not single_column_discretization:
        ...
        best_dist, best_statistic, data_used = _best_distribution(distributions, kolmogorov_opt,
                                                                             number_of_bins, statistical_test,
                                                                             y_std)

        for column in dataset.columns:
            dataset[column] = _discritize(best_dist, cutoff_margin, data_used, dataset[column],
        ...
        for column in dataset.columns:
            ...
            dataset[column] = _discritize(best_dist, cutoff_margin, data_used, dataset[column], number_of_bins, y_std)

    return dataset


def _discritize(best_dist, cutoff_margin, data_used, dataset, number_of_bins, y_std):
    # Set up bins for discretization
    percentile_bins = np.linspace(0, 100, (number_of_bins + 1))
    percentile_bins.sort()
    # Fit distribution according to column
    dist = _Distributions().get_dist(best_dist)
...
def _best_distribution(distributions, kolmogorov_opt, number_of_bins, statistical_test, y_std):
    dist_list = []
    for distribution in distributions:
        results = []
        if statistical_test == "chi2":
            if kolmogorov_opt:
                results = _kolmogorov_goodness_of_fit(y_std.copy(), distribution, kolmogorov_opt)
                results = _chi_squared_goodness_of_fit(results[1], distribution, number_of_bins)
            else:
                results = _chi_squared_goodness_of_fit(y_std.copy(), distribution, number_of_bins)
    ...
    for value in dist_list:
        ...
        if value["statistic"] == "chi2":
            significant = _chi_squared_significant(value["statistic_value"], number_of_bins-1-value["num_estimated_parameters"])
        else:
            significant = _kolmogorov_significant(value["statistic_value"], len(value["data"]))
...
def _chi_squared_significant(chi_squared_statistic, df):
    if df < 1:
        return False
    else:
...
def _kolmogorov_significant(kolmogorov_statistic, size):
    if size > 40:
