    ...
    def __validate(path: typing.Optional[PurePath], bundle: typing.Optional["momotor.bundles.Bundle"]):
        if path is None:
            ...
            if not typing.cast(Path, path).exists():
                raise FileNotFoundError(f"attachment file {path!r} does not exist")

        elif bundle is None:
            raise FileNotFoundError(f"attachment file {path!r} is relative but no bundle is provided")

        elif bundle._base:
            if not (bundle._base / path).exists():
                raise FileNotFoundError(f"attachment file {path!r} does not exist in related bundle {bundle}")

        elif bundle._zip_wrapper:
            with bundle._zip_wrapper as zip_file:
                # Need to test as both a regular file, or as a directory
                zip_path = str(path)
                try:
                    ...
                    try:
                        ...
                        raise FileNotFoundError(
                            f"attachment file {path!r} does not exist in related zip bundle {bundle}"
                        )
    ...
    def is_dir(self: typing.Union[ElementMixinProtocol, "SrcAttachmentMixin"],
        ...
        if self.has_zip_bundle():
            with self.__zip_wrapper() as zip_file:
                try:
                    zip_file.getinfo(self.__get_zip_path(path)+'/')
                except KeyError:
                    return False
                else:
                    return True
