...
def get_text_by_audio(dirpath, language):
    ...
    phrase_regognize = r.recognize_google(audio, language=language)
    try:
        phrase_regognize = r.recognize_google(audio, language=language)
...
def recaptcha_solver(path_download: str,  img_activate_recaptcha: str,
                     ...
                     imgs_verify: list, img_check: str, img_back_page: str = None, number_try: int = 5, language: str = 'en-US') -> bool:
    ...
    while (pass_test == False) and (control <= number_try):
        ...
        if pass_test == False:
            try:
                ...
                phrase_regognize = get_text_by_audio(dirpath, language)
