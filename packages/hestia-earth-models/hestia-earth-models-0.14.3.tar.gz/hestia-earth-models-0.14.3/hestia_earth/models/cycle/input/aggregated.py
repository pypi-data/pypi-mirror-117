...
def _find_closest_impact(cycle: dict, end_date: str, input: dict):
    query = {
        'bool': {
            'must': [
                ...
                # match global impact as we cannot know for sure where the Input comes from
                {'match': {'country.name.keyword': 'World'}}
            ],
            'should': [
...
def _run_seed(cycle: dict, product: dict, input: dict, end_date: str):
    impact = _find_closest_impact(cycle, end_date, product)
...
def _run(cycle: dict, inputs: list, end_date: str):
    inputs = [
        {**i, MODEL_KEY: linked_node(_find_closest_impact(cycle, end_date, i))} for i in inputs
    ]
