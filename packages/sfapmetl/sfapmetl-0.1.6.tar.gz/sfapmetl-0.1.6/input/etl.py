...
def get_data(urlDict):
    jobsUrl = urlDict.get("jobUrl","")
    stagesUrl = urlDict.get("stageUrl","")
    tasksUrl = urlDict.get("taskUrl","")
    all_data = []
...
def work(metricConfig,filepath):
    # data = []

    #GET LOGS FROM ES
    # with open('input/crondata.json' ) as f:
    #     datalogs = json.load(f) 
    datalogs = get_data(metricConfig)
    if "/" in filepath:
        bufferPath = "/".join(filepath.split("/")[:-1]) + "/bufferdata.json"
    else:
    ...
    if data:
        ...
        start_jobs = {record["jobId"]:record["time"] for record in data if record["type"] == "job" and record.get("status","").lower() == "started"}
        success_jobs = {record["jobId"]:record["time"] for record in data if record["type"] == "job" and record.get("status","").lower() == "success"}
        truncate_jobs = {record["jobId"]:record["time"] for record in data if record["type"] == "job" and record.get("status","").lower() in ["aborted","failed"]}
        ...
        for record in data:
            ...
            if record["type"] == "task" and record.get("status","").lower() == "success":
                ...
                if  record["taskId"] in list(truncate_tasks.get(record["jobId"],{}).get(record["stageId"],{}).keys()):
                    ...
                    localdict["status"] = record.get("status","").lower()
                    localdict["time"] = localdict["endTime"] 
                    localdict["type"] = "task"    
                    localdict["duration"] = localdict["endTime"] - localdict["startTime"]
            ...
            if record["type"] == "stage" and record.get("status","").lower() == "success":
            ...
            if record["type"] == "stage" and record.get("status","").lower() in ["failed","aborted"]:
            ...
            if record["type"] == "job":
                ...
                if record["jobId"] in list(success_jobs.keys()) and record.get("status","").lower() == "success":
                    localdict = {}
                    localdict["jobName"] = record["jobName"]
                    localdict["jobId"] = record["jobId"]
