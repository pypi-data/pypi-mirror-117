...
to_remove = pkgutil.get_data(__package__, 'blacklist.txt')
STOPWORDS.extend(to_remove)

# with open("to_normalize.txt", "r", encoding="utf-8") as f:
#     roles_to_normalize = f.read().splitlines()
#     roles_to_normalize = [eval(x) for x in roles_to_normalize]

KEYWORDS_TO_NORMALIZE = pkgutil.get_data(__package__, 'to_normalize.txt')
