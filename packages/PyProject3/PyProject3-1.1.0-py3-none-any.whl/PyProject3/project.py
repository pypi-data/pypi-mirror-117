...
class BaseProject(object):
    def __init__(self, name, root_dir=None):
        ...
        self.logging_utils_file = os.path.join(self.package_dir, 'logging_utils.py')
    ...
    def create(self, override=False):
        ...
        create_file(self.setup_file, contents.SETUP_CONTENT.replace('{project_name}', self.name))
        ...
        self.copy_dir('tests', root=self.root_dir)
        self.copy_dir('examples', dst_dir_name='examples', root=self.root_dir)
        self.copy_dir('TemplateProject/settings', dst_dir_name='settings', root=self.package_dir)
        self.copy_dir('TemplateProject/stores', dst_dir_name='stores', root=self.package_dir)
...
class PyQtProject(BaseProject):
    ...
    def create(self, override=False):
        ...
        create_file(app_file, contents.APP_CONTENT.replace("{project_name}", self.name))
        self.copy_dir('TemplateProject/widgets', dst_dir_name='widgets', root=self.package_dir)
...
class CythonProject(BaseProject):
    ...
    def create(self, override=False):
        ...
        create_file(test_file, contents.CYTHON_TEST_FILE.replace("{project_name}", self.name))
        create_file(self.setup_file, contents.CYTHON_SETUP_CONTENT.replace('{project_name}', self.name), override=True)
        create_file(hello_pxd, contents.CYTHON_HELLO_PXD)
        create_file(hello_pyx, contents.CYTHON_HELLO_PYX)
        create_file(world_pyx, contents.CYTHON_WORLD_PYX.replace("{project_name}", self.name))
