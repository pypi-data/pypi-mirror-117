...
SENT_EMAILS=[]
EMAIL_BLACKLIST=XnwJH()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",XnwJD(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=XnwJU,from_name=XnwJU,smtp_host=XnwJU,smtp_user=XnwJU,smtp_pass=XnwJU,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if XnwJc(recipients,XnwJr)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=XnwJU):
 ...
 result["From"]=formataddr((XnwJD(Header(from_name,"utf-8")),from_email))
 return result
