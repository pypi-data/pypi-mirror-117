...
class PodInfo:
 def __init__(self,name=xaSuf,pod_size=0):
  self.name=name
  self.pod_size=pod_size
  self.pod_size_compressed=0
  self.persisted_resource_names=[]
class CloudPodManager(xaSuh):
 BACKEND="_none_"
 def __init__(self,pod_name=xaSuf,config=xaSuf):
  self.pod_name=pod_name
  self._pod_config=config
 def push(self)->PodInfo:
  raise xaSuv("Not implemented")
 def pull(self):
  raise xaSuv("Not implemented")
 ...
 def get(cls,pod_name,pre_config=xaSuf):
  ...
  raise xaSuv('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 def deploy_pod_into_instance(self,pod_path):
  delete_pod_zip=xaSuw
  if os.path.isdir(pod_path):
   ...
   for folder in PERSISTED_FOLDERS:
    ...
    cp_r(src_folder,tgt_folder,rm_dest_on_conflict=xaSut)
   pod_path=create_zip_file(tmpdir)
   rm_rf(tmpdir)
   delete_pod_zip=xaSut
  zip_content=load_file(pod_path,mode="rb")
  url=self.get_pods_endpoint()
  result=requests.post(url,data=zip_content)
  if result.status_code>=400:
   raise xaSuv("Unable to restore pod state via local pods management API %s (code %s): %s"%(url,result.status_code,result.content))
  if delete_pod_zip:
   rm_rf(pod_path)
  else:
   return pod_path
 def get_state_zip_from_instance(self,get_content=xaSuw):
  url=f"{self.get_pods_endpoint()}/state"
  result=requests.get(url)
  if result.status_code>=400:
   raise xaSuv("Unable to get local pod state via management API %s (code %s): %s"%(url,result.status_code,result.content))
  if get_content:
 ...
 def get_pod_info(self,pod_data_dir:xaSuT=xaSuf):
...
class CloudPodManagerManaged(CloudPodManager):
 ...
 def push(self)->PodInfo:
  ...
  zip_data_content=self.get_state_zip_from_instance(get_content=xaSut)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(xaSuW(zip_data_content)/1000.0)))
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise xaSuv("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  print("Done.")
  result=self.get_pod_info()
  result.pod_size_compressed=xaSuW(zip_data_content)
  return result
 def pull(self):
 ...
 def presigned_url(self,mode):
  ...
  if response.status_code>=400:
   raise xaSuv("Unable to push cloud pod to API (code %s): %s"%(response.status_code,content))
...
class CloudPodManagerGit(CloudPodManager):
 ...
 def push(self):
  ...
  is_empty_repo=b"HEAD" not in repo or repo.refs.allkeys()==xaSuO([b"HEAD"])
  if is_empty_repo:
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=xaSut)
 ...
 def pull(self):
  ...
  if remote_ref not in remote_refs:
   raise xaSuv('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=xaSur(self,"clone_dir",xaSuf)
...
class PodConfigManagerMeta(xaSum):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=xaSuf
   for manager in cls.CHAIN:
    try:
     tmp=xaSur(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif xaSuF(tmp,xaSuH)and xaSuF(result,xaSuH):
       result.extend(tmp)
    except xaSuv:
     pass
   if result is not xaSuf:
    return result
   raise xaSuv('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(xaSuh,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @xaSui
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise xaSuv('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(xaSuh):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=xaSut).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=xaSuO([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=xaSuw):
  try:
   return yaml.load(to_str(load_file(self.CONFIG_FILE)))
  except xaSuv:
   if safe:
    return{}
   raise xaSuv('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(xaSuh):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise xaSuv("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise xaSuv("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def push_state(pod_name,args,**kwargs):
 pre_config=kwargs.get("pre_config",xaSuf)
...
def pull_state(pod_name,args,**kwargs):
 pre_config=kwargs.get("pre_config",xaSuf)
 if not pod_name:
  raise xaSuv("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.pull()
 print("Done.")
def list_pods(args):
 return PodConfigManager.list_pods()
def get_data_dir_from_container()->xaSuT:
 try:
...
def get_persisted_resource_names(data_dir)->List[xaSuT]:
 names=[]
 ...
 with os.scandir(os.path.join(data_dir,"api_states"))as entries:
  for entry in entries:
   if entry.is_dir()and xaSuW(os.listdir(entry.path))>0:
    names.append(entry.name)
