...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def is_active(self):
  ...
  return YBDRJ if e else YBDRS
 def attach(self,cli:LocalstackCli)->YBDRu:
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(YBDRu)
  status.stop()
...
@click.argument("name")
def cmd_pod_push(name:YBDRN):
 from localstack_ext.bootstrap import pods_client
 pods_client.push_state(name,YBDRu)
@pod.command(name="pull",help="Pull the state of a cloud pod into the running LocalStack instance")
@click.argument("name")
def cmd_pod_pull(name:YBDRN):
 from localstack_ext.bootstrap import pods_client
 pods_client.pull_state(name,YBDRu)
def print_table(columns:Dict[YBDRN,YBDRN],rows:List[Dict[YBDRN,Any]],formatters:Dict[YBDRN,Callable[[Any],YBDRN]]=YBDRu):
 from rich.table import Table
 if formatters is YBDRu:
  formatters=YBDRl()
 t=Table()
