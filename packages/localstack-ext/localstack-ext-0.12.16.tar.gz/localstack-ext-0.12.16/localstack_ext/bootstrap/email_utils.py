...
SENT_EMAILS=[]
EMAIL_BLACKLIST=LFIzv()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",LFIzB(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=LFIzi,from_name=LFIzi,smtp_host=LFIzi,smtp_user=LFIzi,smtp_pass=LFIzi,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if LFIzs(recipients,LFIzd)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=LFIzi):
 ...
 result["From"]=formataddr((LFIzB(Header(from_name,"utf-8")),from_email))
 return result
