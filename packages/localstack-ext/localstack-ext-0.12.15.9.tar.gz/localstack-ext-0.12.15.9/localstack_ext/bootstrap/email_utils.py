...
SENT_EMAILS=[]
EMAIL_BLACKLIST=RhOjI()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",RhOjm(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=RhOjV,from_name=RhOjV,smtp_host=RhOjV,smtp_user=RhOjV,smtp_pass=RhOjV,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if RhOjK(recipients,RhOjX)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=RhOjV):
 ...
 result["From"]=formataddr((RhOjm(Header(from_name,"utf-8")),from_email))
 return result
