...
SENT_EMAILS=[]
EMAIL_BLACKLIST=SaMOr()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",SaMOW(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=SaMOV,from_name=SaMOV,smtp_host=SaMOV,smtp_user=SaMOV,smtp_pass=SaMOV,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if SaMOT(recipients,SaMOf)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=SaMOV):
 ...
 result["From"]=formataddr((SaMOW(Header(from_name,"utf-8")),from_email))
 return result
