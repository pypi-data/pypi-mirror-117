...
SENT_EMAILS=[]
EMAIL_BLACKLIST=uWCMo()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",uWCMw(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=uWCMl,from_name=uWCMl,smtp_host=uWCMl,smtp_user=uWCMl,smtp_pass=uWCMl,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if uWCMr(recipients,uWCMt)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=uWCMl):
 ...
 result["From"]=formataddr((uWCMw(Header(from_name,"utf-8")),from_email))
 return result
