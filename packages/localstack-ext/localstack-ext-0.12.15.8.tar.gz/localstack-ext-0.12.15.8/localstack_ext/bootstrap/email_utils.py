...
SENT_EMAILS=[]
EMAIL_BLACKLIST=sVNLU()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",sVNLo(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=sVNLH,from_name=sVNLH,smtp_host=sVNLH,smtp_user=sVNLH,smtp_pass=sVNLH,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if sVNLd(recipients,sVNLe)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=sVNLH):
 ...
 result["From"]=formataddr((sVNLo(Header(from_name,"utf-8")),from_email))
 return result
