...
SENT_EMAILS=[]
EMAIL_BLACKLIST=MucTm()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",MucTL(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=MucTE,from_name=MucTE,smtp_host=MucTE,smtp_user=MucTE,smtp_pass=MucTE,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if MucTQ(recipients,MucTN)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=MucTE):
 ...
 result["From"]=formataddr((MucTL(Header(from_name,"utf-8")),from_email))
 return result
