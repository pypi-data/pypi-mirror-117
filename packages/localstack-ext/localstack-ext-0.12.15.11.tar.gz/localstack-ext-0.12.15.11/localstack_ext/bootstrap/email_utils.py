...
SENT_EMAILS=[]
EMAIL_BLACKLIST=eloxa()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",eloxL(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=eloxb,from_name=eloxb,smtp_host=eloxb,smtp_user=eloxb,smtp_pass=eloxb,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if eloxR(recipients,eloxU)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=eloxb):
 ...
 result["From"]=formataddr((eloxL(Header(from_name,"utf-8")),from_email))
 return result
