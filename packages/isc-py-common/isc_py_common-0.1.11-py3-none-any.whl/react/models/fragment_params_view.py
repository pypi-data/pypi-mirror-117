...
from react.models.fragment_params import Fragment_paramsManager
...
class Fragment_params_viewManager(BaseRefManager):
    ...
    def getRecord(record):
        image_src = f'http://{settings.IMAGE_CONTENT_HOST}:{settings.IMAGE_CONTENT_PORT}/{record.image_src}&ws_host={settings.WS_HOST}&ws_port={settings.WS_PORT}&ws_channel={settings.WS_CHANNEL}'

        value_text = record.value_text
        if not value_text and record.image is not None:
            value_text = image_src
            Fragment_params_view.objects.filter(id=record.id).update(value_text=value_text)
