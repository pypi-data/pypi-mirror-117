...
class Users_old_imagesQuerySet(Model_imagesQuerySet):
    pass


class Users_old_imagesManager(Model_imagesManager):
    ...
    def get_queryset(self):
        return Users_old_imagesQuerySet(self.model, using=self._db)


class Users_old_images(Model_images, Model_withOldIds):
    image = ForeignKeyProtect(Images)
    main_model = ForeignKeyProtect(User)
    props = Users_old_imagesManager.props()

    objects = Users_old_imagesManager()
