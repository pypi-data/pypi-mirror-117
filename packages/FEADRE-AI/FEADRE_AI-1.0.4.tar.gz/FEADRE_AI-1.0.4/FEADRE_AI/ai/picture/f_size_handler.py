...
def resize_np_keep(img_np, new_size_hw, mode='lt', is_fill=True, fill_color=(128, 128, 128)):
    '''
    
    :param img_np: 一张图片 np图片 h,w,c
    :param new_size_hw: [h,w]
    :param mode: center lt
    :param is_fill: 是否用 fill_color 进行填充
    :param fill_color:
    :return:
        img, old_size list(h,w) 最后进行反向
        (top, bottom, left, right) # 填充宽度
    '''
    assert isinstance(img_np, np.ndarray), 'type(img_np = ' + str(type(img_np))
    old_size_wh = img_np.shape[0:2]  # hw
    # flog.debug('原尺寸 %s', img.shape)
    # 选择比例小的
    ratio = min(float(new_size_hw[i]) / (old_size_wh[i]) for i in range(len(old_size_wh)))
    # flog.debug('选择的缩放比例 %s', ratio)

    # 未填充时的尺寸需要保存下来
    h_new, w_new = tuple([int(i * ratio) for i in old_size_wh])
    ...
    if is_fill:
        ...
        # 图片填充
        _pad_w = new_size_hw[1] - w_new
        _pad_h = new_size_hw[0] - h_new
    ...
    return img_np, ratio, old_size_wh[::-1], (left, top, right, bottom)


def resize_img_tensor(image, new_size_wh):
    '''
    将图片缩放到指定的大小范围内，并处理对应缩放bboxes信息
    :param image: 一张已toTensor图片 c,h,w torch.Size([3, 335, 500])
    :param new_size_wh: [w,h]
    ...
    hw = [new_size_wh[1], new_size_wh[0]]
    old_size_hw = image.shape[-2:]  # (800 600) 这个取出来不是tensor
    ratio = min(np.array(hw) / old_size_hw)
    ...
    return image.squeeze(0), old_size_hw[::-1]


def resize_pil_keep(img_pil, new_size, is_fill=True):
    '''
    is_fill==False 自动判断长宽均满足 剪切填充 满足new_size的一边 确保图形的完整
    :param img_pil:
    :param new_size:
...
def letterbox(img_np, new_shape=(416, 416), color=(128, 128, 128), auto=True, scaleFill=False, scaleup=True):
...
if __name__ == '__main__':
    ...
    file_pic = r'/_test_pic/test_99.jpg'
    ...
    print(img_np.shape)
    # img_np, ratio, old_size_wh, (left, top, right, bottom) = resize_np_keep(img_np, (600, 400), mode='center')
    # print(ratio, old_size_wh, (left, top, right, bottom))
    img_np, ratio, old_size, _ = resize_np_keep(img_np, (416, 416), is_fill=True, )
    ...
    # img_tensor, old_size_wh = resize_img_tensor(img_tensor, (600, 200))
    # print(old_size_wh)
    # img_pil = F.to_pil_image(img_tensor).convert('RGB')  # 这个会还原

    # img_pil, ratio, (nw, nh) = resize_pil_keep(img_pil, (600, 200), is_fill=False)
    # print(ratio, (nw, nh))
    # img_pil.show()
