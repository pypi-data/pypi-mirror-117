...
class RetrieverThreadingActor(pykka.ThreadingActor):
    ...
    def log(self, detail, level: LogLevels = LogLevels.INFO):
        if not self.can_log:
            return
        ...
        assert isinstance(self.mem, SharedMemory), "The log() function only works for actors that have " \
                                                   ...
                                                   "type SharedMemory"
        # print(f"logging {detail} from {self.actor_urn}")
        self.mem.logging.log(self.actor_urn, detail, level)

    def trace(self, detail):
    ...
    def _before_on_start(self):
        self.log("on_start_triggered", LogLevels.TRACE)

    def _before_on_stop(self):
        self.log("on_stop_triggered", LogLevels.TRACE)
    ...
    def _before_on_failure(
        ...
        self.log(f"on_failure_triggered. {exception_type}, {exception_value}", LogLevels.TRACE)
