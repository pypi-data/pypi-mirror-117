...
def calc_h98_d3d(shot, calc_tau=False, calc_ptot=False, pinj_only=False, estimated_signals=None, data=None):
    ...
    :param calc_tau: bool
        Calculate tauth instead of gathering tauth. Might be useful if the confinement code stops early and doesn't
        write tauth.

    :param calc_ptot: bool
        Calculate total power instead of gathering it. Useful if ptot isn't written due to an error in another code

    :param pinj_only: bool
        Approximate ptot by pinj, ignoring ohmic and ECH power. This might be okay.

    :param estimated_signals: dict
        Data for estimating missing signals. Should contain a 'time' key whose value is a 1d float array.
        Should contain one additional key for each pointname that is being estimated, whose value is a
        1d float array with length matching that of time.

    :param data: dict [optional]
        Pass in an empty dict and data used in the calculation will be written to it.
        Intermediate quantities may be captured for inspection in this way.
