...
class Client(object):
    ...
    def add_agent_operations(self, agent_id, operations, useWebSocket=False):
        ...
        if not hasattr(operations, "shape") and not operations:
            raise CraftAiBadRequestError("Operations must be a non-empty list")
        ...
        if useWebSocket:
            webSocketEventLoop = asyncio.get_event_loop()
            headers = [
                ("Authorization", "Bearer " + self.config.get("token")),
                ("agent-name", agent_id),
                (
                    "expected-task-count",
                    ceil(len(operations) / self.config["operationsChunksSize"]),
                ),
            ]
            try:
                [protocol, uri] = self.config["url"].split("://")
            except ValueError:
                raise CraftAiBadRequestError("Invalid url provided")
            wsProtocol = ("wss://", "ws://")[protocol == "http"]
            wsUrl = wsProtocol + uri + "/api/ws"
            try:
                websocket = webSocketEventLoop.run_until_complete(
                    websockets.connect(
                        wsUrl, subprotocols=["echo-protocol"], extra_headers=headers
                    )
                )
        ...
        try:
            while is_looping:
                ...
                try:
                    ...
                    raise CraftAiBadRequestError(
                        "Invalid configuration or agent id given. {}".format(
                            err.__str__()
                        )
                    )
                if useWebSocket:
                    try:
                        webSocketEventLoop.run_until_complete(websocket.send(json_pl))
                    except websockets.exceptions.ConnectionClosedError as e:
                        try:
                            errorMessage = webSocketEventLoop.run_until_complete(
                                websocket.recv()
                            )
                        except websockets.exceptions.ConnectionClosedError as e:
                            raise CraftAiUnknownError(e.reason)
                        raise Client._get_error_from_websocket_status(
                            e.code, errorMessage
                        )
                else:
            ...
            if useWebSocket:
                try:
                    ...
                    webSocketEventLoop.run_until_complete(websocket.recv())
                except websockets.exceptions.ConnectionClosedError as e:
                    raise Client._get_error_from_websocket_status(e.code, errorMessage)
                except websockets.exceptions.ConnectionClosedOK as e:
                    added_operations_count = int(e.reason.split()[2])

        except Exception as e:
            if useWebSocket:
                webSocketEventLoop.run_until_complete(websocket.close(1003))
            raise e
        return {
    ...
    def get_sliding_window_scores_bulk(self, payload):
        """Get a group of validation scores of sliding windows.

        :param list payload: Contains the informations to get the validation score. It's in
        the form [{"id": generator_id, "test_from": test_from, "test_to": test_to, "step": step}]
        With parameters having the form given in the craft_ai documentation.
        ...
        # Create the json file with the agents with valid id and send it
        scores = self._create_and_send_json_bulk(
            payload, "{}/bulk/generators/score/sliding".format(self._base_url), "POST",
        )

        return scores

    def get_single_window_score_bulk(self, payload):
        """Get a group of validation scores of manual defined windows.

        :param list payload: Contains the informations to get the validation score. It's in
        the form [{"id": generator_id, "test_from": test_from, "test_to": test_to,
        ...
        # Create the json file with the agents with valid id and send it
        scores = self._create_and_send_json_bulk(
            payload, "{}/bulk/generators/score/single".format(self._base_url), "POST",
        )
    ...
    def _decode_response(response):
        ...
        if status_code in [200, 201, 204, 207]:
            ...
            raise Client._get_error_from_http_status(status_code, message)
        return None
    ...
    def _decode_response_bulk(response_bulk):
        ...
        for response in response_bulk:
            if ("status" in response) and (response.get("status") == 201):
                ...
                agent = {
                    "error": Client._get_error_from_http_status(status_code, message)
                }
                try:
    ...
    @staticmethod
    def _get_error_from_http_status(status_code, message):
    ...
    @staticmethod
    def _get_error_from_websocket_status(status_code, message):
