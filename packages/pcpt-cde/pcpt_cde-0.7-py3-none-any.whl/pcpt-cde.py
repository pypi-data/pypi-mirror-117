...
def get_asg_details(all_asg, cluster_name):
    ...
    for i in range(len(all_asg)):
        ...
        for j in range(len(all_tags)):
            if (
                all_tags[j]["Key"] == f"{config.K8S_CLUSTER_TAG_PREFIX}/{cluster_name}"
                and all_tags[j]["Value"] == "owned"
            ):
                cluster_asg = True
            if all_tags[j]["Key"] == f"{config.AWS_ASG_TAG}":
                node_group_asg = True
...
def get_timezone(region):
    ...
    if region in config.AWS_REGION_TIMEZONE:
        tz = config.AWS_REGION_TIMEZONE[region]
...
def main():
    ...
    try:
        ...
        instance_id = get_instance_id(session, config.AWS_INSTANCE_TAG)
        if instance_id is None:
            print(f"ERROR: Unable to find instance with tag: {config.AWS_INSTANCE_TAG}")
            sys.exit(1)
        ...
        if ssm_session:
            ...
            # send k8s command to get latest events
            cmd_id = send_command(session, instance_id, config.SSM_GET_EVENTS)

            # aws loadbalancer details
            elb_list = get_elb(session, aws_config)
            elb_table.field_names = [
                "Service",
                "Scheme",
                "State",
                "Type",
            ...
            # aws ssm details
            backup_bucket = get_ssm_parameter(session, config.AWS_SSM_KEY_BACKUP)
            log_bucket = get_ssm_parameter(session, config.AWS_SSM_KEY_LOGS)
            ...
            events = get_invocation(session, instance_id, cmd_id)
...
def get_invocation(session, instance_id, cmd_id):
    """
    Get command STDOUT
    """
...
def send_command(session, instance_id, command):
    """
    Run command on instance and return command id.
    """
...
def get_elb(session, aws_config):
    ...
    stdout_msg(
        f'Checking aws load balancer with tag: kubernetes.io/cluster/{aws_config["cluster_name"]}'
    )
        ...
        for elb in response["LoadBalancers"]:
            ...
            for tag_desc in tags["TagDescriptions"]:
                ...
                for tags in tag_desc["Tags"]:
                    if config.K8S_SERVICE_TAG_PREFIX in tags["Key"]:
                        service_name = tags["Value"]
                    if (
                        f"{config.K8S_CLUSTER_TAG_PREFIX}/{aws_config['cluster_name']}"
                        in tags["Key"]
                    ):
                        cluster_tag = True
