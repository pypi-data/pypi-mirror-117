...
def dict_to_wit_data(dict_data, default_value=0):
    wit_data = WitData(default_value)
    wit_data.init_with_dict(dict_data)
    return wit_data

def np_to_wit_data(dates, stock_codes, data, default_value=0):
    wit_data = WitData(default_value)
...
def db_rows_to_wit_data(rows, trade_date_column='tradeDate', stock_code_column='stockCode', value_column='itemValue', default_value=0):
    wit_data = None
    if len(rows) > 0:
        wit_data = WitData(default_value)
...
def dataframe_to_wit_data(df, default_value=0):
    wit_data = WitData(default_value)
    wit_data.init_with_dataframe(df)
    return wit_data

def wit_id_to_wit_data(api_key, wit_id):
    response = get_wit_descriptor(api_key, wit_id)

    return response['wit']

def get_wit_descriptor(api_key, wit_id):
    data = dict()
    data['apiKey'] = api_key
    data['witId'] = wit_id
