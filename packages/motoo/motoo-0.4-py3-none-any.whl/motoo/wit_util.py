...
def file_to_wit_data(filename, default_value=None):
    text = b''
    with open(filename, "rb") as f:
        text = f.read()
    
    return bytes_to_wit_data(text, 0.0)
...
class WitData:
    ...
    def to_bytes(self):
        ...
        return b''.join([len(bs[0]).to_bytes(8, byteorder='big'), len(bs[1]).to_bytes(8, byteorder='big'), len(bs[2]).to_bytes(8, byteorder='big'), \
            len(bs[3]).to_bytes(8, byteorder='big'), bs[0], bs[1], bs[2], bs[3]])
    ...
    def get_date_slicing(self, start_date=None, end_date=None):
        ...
        if end_date is None or end_date > self.dates[-1]:
            end_index = len(self.dates) - 1
        elif end_date < self.dates[0]:
            return None
        else:
            end_index = self.get_date_index(end_date) + 1
