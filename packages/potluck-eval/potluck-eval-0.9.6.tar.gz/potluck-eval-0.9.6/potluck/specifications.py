...
class HasPayload:
    ...
    def do_setup(self, setup_fn):
        ...
        Note: `do_setup` and `do_cleanup` each accumulate setup/cleanup
        functions instead of replacing the previous setup/cleanup
        function, with setup functions added later affecting the results
        of setup functions added earlier. Also, setup/cleanup functions
        are accumulated between child and parent payload-bearing objects,
        rather than child setups/cleanups overriding parent
        setups/cleanups.
        """
...
class HasGoal:
    ...
    def compare_using(self, comparator_fn):
        ...
        The comparator function should return a comparison result: a
        dictionary with "status" and "explanation" keys, where the status
        is one of "accomplished", "partial", or "failed".
    ...
    def compare_exactly(self):
        ...
        of any type for exact equality (the default omni_compare function
        has various grades of partial success and ignores things like
        floating point rounding error).
        Returns the `TestGroup` for chaining.
...
class AlterContext(RefinedTest):
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        A parent goal provider and a context builder function are
        necessary.
...
class Transform(RefinedTest):
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        A parent goal provider and a transformation function are
        necessary. A description for the result may be provided if a
        full custom description isn't being used. `refine_ref` may be
        set to False to avoid also transforming the equivalent reference
        slot.
...
def RequireExpectations(
    ...
    register_goal(rubrics.DefinesEnoughTests(**args))
...
def ObserveExpectationResults(
    ...
    register_goal(rubrics.TestsPassedInSubmission(**args))
...
def ValidateExpectations(
    ...
    register_goal(rubrics.TestsPassForSolution(**args))
