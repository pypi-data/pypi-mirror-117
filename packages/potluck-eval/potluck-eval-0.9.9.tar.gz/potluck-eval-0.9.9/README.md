...
`importlib_resources`, and `beautifulsoup4` packages. Tests depend on
`pytest`, and you can run them using `tox` if you want.
...
through PyPI. For full server security, you should also install
`pyopenssl`, `flask_talisman`, and `flask_seasurf`, but these are not
required for running the server and won't be used if they're not present
(although this introduces some extra security vulnerabilities).
