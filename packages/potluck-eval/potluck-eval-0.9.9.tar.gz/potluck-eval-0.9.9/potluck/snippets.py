...
class Snippet(specifications.TestGroup):
    ...
    def __init__(self, sid, title, caption, tests, postscript=''):
        """
        ...
        # We store our title, caption, and postscript
        self.title = title
...
class Variables(Snippet):
    ...
    def render_context(self, context):
        ...
        return '<span class="prompt">In [ ]:</span> {}\n'.format(markup)
...
class FunctionCalls(Snippet):
    ...
    def render_context(self, context):
        ...
        result = '<span class="prompt">In [ ]:</span> {}\n'.format(callrep)

        if captured:
            result += '<span class="printed">{}</span>\n'.format(captured)

        # Highlight the return value
        value = highlight_code(pprint_with_prefix(value, "Out [ ]:")[8:])

        result += '<span class="prompt">Out [ ]:</span> {}\n'.format(value)
