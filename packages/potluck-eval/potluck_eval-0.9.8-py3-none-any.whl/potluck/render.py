...
def render_instructions(
    ...
    """
    Renders instructions as HTML (converts instructions.md from markdown)
    and combines them with a rubric table and snippets into one document.
    """
    # Load our report template
    ...
    # Render HTML from our instructions markdown
...
def render_rubric_table(taskid, table, blank=False):
    ...
    Normally, you'd want to call `reset_row_ID_cache` before calling
    this to avoid over-zealous ID de-duplication.
...
ROW_IDS = {}
"""
Per-task and per-blank-status storage for observed row IDs, so that we
can prevent ID duplication by adding -number suffixes.
"""


def reset_row_ID_cache():
    """
    Resets the row ID cache. Use this before call render_table to ensure
    that we don't over-zealously try to de-duplicate IDs.
    """
    global ROW_IDS
    ROW_IDS = {}
...
def unique_id(taskid, identifier):
    """
    Given a task of interest and an identifier, keeps track of
    identifiers provided and returns them as-is, except when a duplicate
    is provided, in which case it appends a -number suffix to the
    duplicate to make it unique and returns that. The -number suffixes
    start at -2 for the second copy; -1 is never used because the first
    copy doesn't get a suffix added.

    Note: call `reset_row_ID_cache` to reset the cache of observed
    identifiers.
    """
    task_ids = ROW_IDS.setdefault(taskid, {})
    ...
    if seen_before == 0: # not seen before
        ...
        task_ids[identifier] += 1
        return identifier + '-' + str(seen_before + 1)
...
def render_rubric_row(taskid, row, blank=False):
    ...
    identifier = unique_id(taskid, row.get("id"))
    ...
    contents += (
        ...
        'id="{}" '.format(identifier) if identifier else "",
        topic_class,
