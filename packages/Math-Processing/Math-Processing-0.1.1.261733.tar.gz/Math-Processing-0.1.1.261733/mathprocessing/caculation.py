print('''
This is a caculate program template.
If you want to use one of function in it, you may get an error.
''')
...
def download():
    if p.system() == 'Windows':
        if p.release() == '11' or p.release() == '10' or p.release() == '8.1':
            print('''
We will close your Caculation 6.1 ten seconds soon
''')
            web.open('https://python.org/downloads/release/python-396/')
            time.sleep(10)
            raise SystemExit('Please restart Caculation 6.1')
        elif p.release() == '7' or p.release() == '8':
            print('''
We will close your Caculation 6.1 ten seconds soon
''')
            web.open('https://python.org/downloads/release/python-388')
            time.sleep(10)
            raise SystemExit('Please restart Caculation 6.1')
        elif p.release() == 'XP':
            print('''
I am sorry that your Windows is too old to run python 3.8.x
You may install new Windows likes Windows 10 or Windows 11''')
    elif p.system() == 'Darwin':
        print('''
Caution !
We cannot check your System, OS X, Mac OS X or macOS which can run python 3.8.x or not
Please download after reading the installation precautions
And we will close your Caculation 6.1 ten seconds soon

NOTE :
If your macOS or Mac OS X version higher than 10.9, you can download Python release ( 3.9.x )
''')
        web.open('https://python.org/downloads/release/python-396')
        time.sleep(10)
        raise SystemExit('Please restart Caculation 6.1')
        ...
        print('''
Maybe your PC System are not Windows or macOS,
please choose a python release that higher than 3.8 to download
And we will close your Caculation 6.1 ten seconds soon
''')
        web.open('https://python.org/download/other/')
        time.sleep(10)
        raise SystemExit('Please restart Caculation 6.1')

if sys.version_info[0] != 3:
    print('''Your Python version is lower than 3.x
Please download Python version 3.8.x or more later''')
    while True:
        try:
            data = input('Download ? [False/True] ')
        except NameError:
            pass
    ...
    print('''Your Python version is lower than 3.8
Please download Python version 3.8.x or more later''')
    while True:
        try:
            data = input('Download ? [False/True] ')
...
class Caculation():
    def __init__(self):
        ...
        # Value
        self.window = tk.Tk()
        self.window.title('Caculation 6.1 ( on %s %s )' % (p.system(), p.release()))
