...
__all__ = ['pg_get_locked_transactions', 'pg_cancel_backend', 'pg_cancel_all_backends', 'indices', 'compliance_check', 'as_column_name', 'to_sql', 'rename_schema', 'list_views', 'list_matviews', 'list_frames', 'list_all_frames', 'get_frame_length', 'get_frame_dependencies', 'get_view_sql_code', 'rename_table', 'drop_table', 'rename_view', 'drop_view', 'rename_matview', 'drop_matview', 'frame_exists', 'drop_frame', 'list_columns_ext', 'list_columns', 'list_primary_columns_ext', 'list_primary_columns', 'rename_column', 'drop_column', 'comparesync_table', 'readsync_table', 'writesync_table']
...
def to_sql(df, name, engine, schema=None, if_exists='fail', nb_trials=3, logger=None, **kwargs):
    ...
    if not frame_exists(name, engine, schema=schema, nb_trials=nb_trials, logger=logger):
        if_exists = 'replace'
    ...
    if if_exists != 'gently_replace':
        if not local_indices:
            return run_func(df.to_sql, name, engine, schema=schema, if_exists=if_exists, index=False, index_label=None, nb_trials=nb_trials, logger=logger, **kwargs)
        retval = run_func(df.to_sql, name, engine, schema=schema, if_exists=if_exists,
        ...
        if if_exists == 'replace':
            exec_sql("ALTER TABLE {} ADD PRIMARY KEY ({});".format(frame_sql_str, ','.join(
                local_indices)), engine, nb_trials=nb_trials, logger=logger)
        return retval
    ...
    if local_indices != remote_indices:
        raise se.ProgrammingError("SELECT * FROM {} LIMIT 1;".format(frame_sql_str), remote_indices,
    ...
    if columns != remote_columns:
        raise se.ProgrammingError("SELECT * FROM {} LIMIT 1;".format(frame_sql_str), "matching non-primary fields",
                                   "Local columns '{}' differ from remote columns '{}'.".format(columns, remote_columns))

    exec_sql("DELETE FROM {};".format(frame_sql_str),
             engine, nb_trials=nb_trials, logger=logger)
    return run_func(df.to_sql, name, engine, schema=schema, if_exists='append', index=bool(local_indices), index_label=None, nb_trials=nb_trials, logger=logger, **kwargs)
...
def list_frames(engine, schema=None, nb_trials=3, logger=None):
    ...
    for item in list_matviews(engine, schema=schema, nb_trials=nb_trials, logger=logger):
...
def list_all_frames(engine, schema=None, nb_trials=3, logger=None):
    ...
    for schema in list_schemas(engine, nb_trials=nb_trials, logger=logger):
        df = list_frames(engine, schema=schema,
...
def frame_exists(frame_name, engine, schema=None, nb_trials=3, logger=None):
    ...
    '''
    if frame_name in list_tables(engine, schema=schema, nb_trials=nb_trials, logger=logger):
        return True
    if frame_name in list_views(engine, schema=schema, nb_trials=nb_trials, logger=logger):
        return True
...
def drop_frame(frame_name, engine, schema=None, restrict=True, nb_trials=3, logger=None):
    ...
    '''
    if frame_name in list_tables(engine, schema=schema, nb_trials=nb_trials, logger=logger):
        return drop_table(frame_name, engine, schema=schema, restrict=restrict, nb_trials=nb_trials, logger=logger)
    if frame_name in list_views(engine, schema=schema, nb_trials=nb_trials, logger=logger):
        return drop_view(frame_name, engine, schema=schema, restrict=restrict, nb_trials=nb_trials, logger=logger)
    if frame_name in list_matviews(engine, schema=schema, nb_trials=nb_trials, logger=logger):
        return drop_matview(frame_name, engine, schema=schema, restrict=restrict, nb_trials=nb_trials, logger=logger)
    return False
...
def list_columns_ext(table_name, engine, schema=None, nb_trials=3, logger=None):
    ...
    '''
    if not frame_exists(table_name, engine, schema=schema, nb_trials=nb_trials, logger=logger):
        if schema is None:
...
def comparesync_table(engine, df_filepath, table_name, id_name, hash_name='hash', set_index_after=False, columns=['*'], schema=None, max_records_per_query=None, cond=None, reading_mode=True, nb_trials=3, logger=None):
    ...
    with logger.scoped_debug("Comparing table: local '{}' <-> remote '{}'".format(df_filepath, frame_sql_str), curly=False) if logger else dummy_scope:
        ...
        try:
            ...
            if hash_name in list_columns(table_name, engine, schema=schema, nb_trials=nb_trials, logger=logger):
            ...
            with tqdm(total=remaining, unit='val') as progress_bar:
                while remaining > record_cap:
                    ...
                    df = read_sql(qsql, engine, index_col=id_name,
...
def writesync_table(engine, df_filepath, table_name, id_name, hash_name='hash', schema=None, max_records_per_query=None, conn_ro=None, engine_ro=None, nb_trials=3, logger=None):
    ...
    if engine_ro is None:
        if conn_ro is None:
            ...
            if logger:
                logger.warn("Keyword argument 'conn_ro' is becoming deprecated. Please replace it 'engine_ro' instead.")
            engine_ro = conn_ro
    frame_sql_str = frame_sql(table_name, schema=schema)
    with logger.scoped_debug("Writing table: local '{}' -> remote '{}'".format(df_filepath, frame_sql_str), curly=False) if logger else dummy_scope:
        local_df, remote_md5_df, same_keys, diff_keys, local_only_keys, remote_only_keys = comparesync_table(
            engine_ro, df_filepath, table_name, id_name, hash_name=hash_name, columns=['*'], schema=schema, max_records_per_query=max_records_per_query, cond=None, reading_mode=False, nb_trials=nb_trials, logger=logger)
        ...
        if len(local_df) < 128:  # a small dataset
            to_sql(local_df, table_name, engine, schema=schema,
        ...
        if len(local_only_keys) > 0:
            ...
            with tqdm(total=len(local_only_keys)) as progress_bar:
                ...
                while len(df) > record_cap:
                    ...
                    start_time = pd.Timestamp.utcnow()
                    to_sql(df2, table_name, engine, schema=schema,
                ...
                to_sql(df, table_name, engine, schema=schema,
                       if_exists='append', nb_trials=nb_trials, logger=logger)
                progress_bar.update(len(df))

        # remove redundant remote records
        id_list = diff_keys + remote_only_keys
        if len(id_list) > 0 and table_name in list_tables(engine_ro, schema=schema, nb_trials=nb_trials, logger=logger):
            if logger:
...
def readsync_table(engine, df_filepath, table_name, id_name, hash_name='hash', set_index_after=False, columns=['*'], schema=None, cond=None, bg_write_csv=False, max_records_per_query=None, nb_trials=3, logger=None, raise_exception_upon_mismatch=True):
    ...
    with logger.scoped_debug("Reading table: local '{}' <- remote '{}'".format(df_filepath, frame_sql_str), curly=False) if logger else dummy_scope:
        local_df, remote_md5_df, same_keys, diff_keys, local_only_keys, remote_only_keys = comparesync_table(
            engine, df_filepath, table_name, id_name, hash_name=hash_name, columns=columns, schema=schema, max_records_per_query=max_records_per_query, cond=cond, nb_trials=nb_trials, logger=logger)
        ...
        if len(id_list) > 0:
            ...
            with tqdm(total=len(id_list)) as progress_bar:
                ...
                while len(id_list) > 0:
                    ...
                    start_time = pd.Timestamp.utcnow()
                    new_dfs.append(read_sql(query_str, engine, index_col=id_name,
                                            set_index_after=set_index_after, nb_trials=nb_trials, logger=logger))
                    # elapsed time is in seconds
                    elapsed_time = (pd.Timestamp.utcnow() -
                                    start_time).total_seconds()
