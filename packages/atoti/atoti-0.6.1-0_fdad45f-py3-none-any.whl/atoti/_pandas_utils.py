...
_ARROW_TYPES = {
    ...
    "string": pa.string(),
    "LocalDate[yyyy-MM-dd]": pa.date32(),
    "LocalDate[yyy-MM-dd'T'HH:mm:ss]": pa.date64(),
}
...
def pandas_to_temporary_parquet(
    ...
    *,
    ...
    tempdir: str = _make_atoti_tempdir()
    with tempfile.NamedTemporaryFile(
        ...
        table = pa.Table.from_pandas(
            dataframe, preserve_index=False, safe=False, schema=schema
        )
        ...
        pq.write_table(table, os.path.join(tempdir, file_path.name))
...
def _flatten_multilevel_columns(columns: pd.Index) -> pd.Index:  # type: ignore
    return pd.Index(
        (
...
def _get_date_without_time_column_names(dataframe: pd.DataFrame) -> Collection[str]:  # type: ignore
    return [
        column
...
def _contains_only_dates_without_time(values: pd.Series) -> bool:  # type: ignore
...
def _sanitize_column_names(dataframe: pd.DataFrame) -> Tuple[pd.DataFrame, Mapping[str, str]]:  # type: ignore
