...
class Cube(LocalCube[Hierarchies, Levels, Measures]):
    ...
    def create_parameter_simulation(
        ...
        **kwargs: Any,
        ...
        if any(level.name == "Scenario" for level in (levels or [])):
            raise ValueError(
    ...
    def create_parameter_hierarchy_from_column(self, name: str, column: Column):
...
class CubeContext(MutableMapping[str, str], ReprJsonable):
    ...
    @typechecked
    def __setitem__(self, key: str, value: Any):  # pylint: disable=redefined-outer-name
