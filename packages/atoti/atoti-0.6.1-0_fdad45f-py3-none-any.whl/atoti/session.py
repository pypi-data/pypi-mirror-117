...
from ._docs_utils import CSV_KWARGS, PARQUET_KWARGS, TABLE_CREATION_KWARGS, doc
...
from .table import Table, _create_table, _LoadParquetPrivateParameters
...
class _CreateTablePrivateParameters:
    columns_with_shared_dictionary: Mapping[str, Column] = field(default_factory=dict)
...
class _ReadParquetPrivateParameters(
    ...
    _types: Optional[Mapping[str, DataType]] = None
...
class Session(LocalSession[Cubes]):
    ...
    def tables(self) -> Tables:  # noqa: D401
        ...
        return Tables(
            self._java_api,
            {
                table: Table(table, self._java_api)
                for table in self._java_api.get_tables()
            },
        )
    ...
    def create_table(
        ...
        **kwargs: Any,
        ...
        self._java_api.create_table(
            ...
            columns_with_shared_dictionary=private_parameters.columns_with_shared_dictionary,
            is_parameter_table=private_parameters.is_parameter_table,
        )
        return _create_table(self._java_api, name)
    ...
    def read_pandas(
        ...
        **kwargs: Any,
        ...
        ) = pandas_to_temporary_parquet(dataframe, prefix=table_name, types=types)
    ...
    @doc(**{**TABLE_CREATION_KWARGS, **CSV_KWARGS})
    def read_csv(
        self,
        path: PathLike,
        *,
        ...
        hierarchized_columns: Optional[Collection[str]] = None,
        date_patterns: Optional[Mapping[str, str]] = None,
        **kwargs: Any,
    ) -> Table:
        ...
        private_parameters = _CreateTablePrivateParameters(**kwargs)
        full_path = path
        path, pattern = _split_path_and_pattern(path, ".csv")

        table_name = _infer_table_name(
            path=path, pattern=pattern, table_name=table_name
        )

        inferred_types = CsvDataSource(self._java_api).infer_csv_types(
            path=path,
            keys=keys,
            separator=separator,
            encoding=encoding,
        ...
        table.load_csv(
            ...
            separator=separator,
            encoding=encoding,
            process_quotes=process_quotes if process_quotes is not None else False,
            array_separator=array_separator,
            date_patterns=date_patterns,
        )
        return table

    @doc(**{**TABLE_CREATION_KWARGS, **PARQUET_KWARGS})
    def read_parquet(
        self,
        path: PathLike,
        *,
        ...
        **kwargs: Any,
        ...
        full_path = path
        path, pattern = _split_path_and_pattern(path, ".parquet")
        table_name = _infer_table_name(
            path=path, pattern=pattern, table_name=table_name
        )
        ...
        types = (
            {**inferred_types, **private_parameters._types}
            if private_parameters._types is not None
            else inferred_types
        )
        table = self.create_table(
            table_name,
    ...
    def start_transaction(self, scenario_name: str = BASE_SCENARIO_NAME) -> Transaction:
        ...
        * It avoids possibly incorrect intermediate states (e.g. if loading some new data requires dropping existing rows first).
        ...
        """
        return Transaction(self._java_api, scenario_name)
    ...
    def create_cube(
        ...
        *,
        ...
        self._java_api.create_cube_from_table(
            table=base_table, cube_name=name, creation_mode=mode.upper()
        )
    ...
    def create_scenario(self, name: str, *, origin: str = BASE_SCENARIO_NAME):
