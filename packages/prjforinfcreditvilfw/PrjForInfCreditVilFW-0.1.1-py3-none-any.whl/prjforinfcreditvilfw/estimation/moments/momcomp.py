                ...
                for momkey in moment_set:
                    ...
                    if (found_match):
                        if (isinstance(model_moments_cur, pd.Series)):
                            ...
                            '''
                            current_obj = ((abs(model_moments_cur - moments_data_cur)*moment_weight)**moment_power)
#                             current_obj = moment_weight*((abs(model_moments_cur - moments_data_cur))**moment_power)
        ...
        if (isinstance(model_moments_cur, pd.Series) and (momsets_ctr == 1)):            
            df_key = hardcode_estimation.connct_nested_dict_dot(momsetgroupkey = hardcode_estimation.string_estimation()['main_obj']['str']) 
