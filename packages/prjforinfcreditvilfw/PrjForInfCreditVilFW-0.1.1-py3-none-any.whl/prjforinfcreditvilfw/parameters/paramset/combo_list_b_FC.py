...
#         max_int = 1.2
        ...
        combo_list = \
            [{'param_update_dict':{'grid_type':['a', 20180512, {'std_eps':std, 'std_eps_E':std}],
                                   ...
                                   'data_type':['b', 20180512, {'A':A-((std**2)/2),'Region':0,'Year':0}],
                                   'model_type':['a',1],
                                   'interpolant_type':interpolant_type}
            ,'title':'(INF BORR+SAVE)+(0MIN)+(LOG,Angeletos)+i15+(R='+str(int(cur_rate*100))+',A='+str(A)+',S='+str(std)+')'
            ,'combo_desc':'Angeletos, INF BORR ZERO FIXED COSTS ZERO MIN BORR SAVE LOG'+str(int(cur_rate*100))
    ...
    if (sub_type == '20180513a'):
        ...
        combo_list = \
            ...
            ,'combo_desc':'Angeletos, INF BORR ZERO FIXED COSTS ZERO MIN BORR SAVE LOG'+str(int(inf_rate*100))
            ,'file_save_suffix':'_r'+str(int(inf_rate*100)) + 'ilf'+str(int(BNI_LEND_P*100)) + 'fsf'+str(int(BNF_SAVE_P*100))}
            for BNI_LEND_P in np.linspace(min_BNI_LEND_P, max_BNI_LEND_P, num=BNI_LEND_P_count)]
    ...
    if ("debug_" in sub_type):
        ...
        # SHOULD ALSO TEST:
        # interst rate simulation
        # b_20180702_ITG_DAprdMu generated results from A0 up, but no final results?
        # equi_20180702_ITG_fcfs_BNF_SAVE_P_10000_r12000_equitg_maxJitg, seems to have finished P_10000, but did not move forward to 10555?
...
#         'data_type':['a', '20180701']
    ...
    if ( any([main_type_str in sub_type for main_type_str in main_type_str_list])):
        ...
        if ('20181111a' in sub_type):
            ...
            # 'param_update_dict': {
            # 'grid_type': ['a', '20181024'],
            # 'esti_type': ['a', '20181021bench',
            #   {'logit_sd_scale': 0.9888888888888889,
            #    'BNI_BORR_P': 0.0,
            #    'BNI_LEND_P': 3.0,
            #    'BNF_BORR_P': 0.0,
            #    'BNF_SAVE_P': 1.0,
            #    'kappa': 0.2777777777777778,
            #    'R_FORMAL_BORR': 1.055222222222222,
            #    'R_FORMAL_SAVE': 1.027777777777778}],
            # 'data_type': ['a', '20181024'],
            # 'model_type': ['a', '20180701'],
            # 'interpolant_type': ['a', '20180607'],
            # 'dist_type': ['a', '20181025', {'epsA_frac_A': 0.15}],
            # 'minmax_type': ['a', 'B181107']},
            # 'title': 'BenchmarkT2', 'combo_desc': 'BenchmarkT2', 'file_save_suffix': '_T2'
            # }]
