...
def compute_set_gen(fargate=True):
    ...
    # This is original version, 2018 07 17, integration version created
    #     compute_detail_fargate_ge_par = {'cpu':str(1024*2),
    #                                      'memory':str(1024*8),
    #                                      'workers':2,
    ...
    # A version that works with integration, and that works without as well, 4 core parallel
    '''
        'int_rate_counts':4,
        'bisection_iter':4,
            - 4+4+4+4 = 16, for accuracy over 0.20 interest span: 0.20/(4*4*4*4)=0.00078125

        'int_rate_counts':4,
        'bisection_iter':4,
            - 5+5+5 = 15, for 0.20/(5*5*5)=0.0016
    '''

    """
    Group A, Parallize over interest rate vector:
        - useful (a tiny bit faster perhaps), when doing
    """
    ...
    #     fargate_ng_par_d = {'cpu':str(1024*8),
    #                         'memory':str(1024*45),
    #                         'workers':8,
    #                         'compute_param_vec_count':56,
    #                         'aws_fargate':fargate,
