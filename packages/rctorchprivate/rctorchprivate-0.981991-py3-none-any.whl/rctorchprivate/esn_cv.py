...
def execute_objective(parallel_arguments, parameters, id_):#arguments):
    ...
    if ode:
        ...
        return float(total_score), {"pred": results["ys"][best_idx].to("cpu"), "val_y" : results["ydots"][best_idx], "score" : best_batch_score}, id_
    else:
        return float(total_score), {"pred": pred_, "val_y" : val_input["y"], "score" : val_score}, id_
...
def eval_objective_remote(parallel_args_id, parameters, dtype = None, device = None, plot_type = "error",  *args):
        ...
        results = ray.get([execute_objective.remote(parallel_args_id, parameter_lst[i], i) for i in range(num_processes)])
...
class EchoStateNetworkCV:
    ...
    ######
    """
    def __init__(self, bounds, subsequence_prop,  model=EchoStateNetwork, initial_samples=50, #subsequence_length,
                 ...
                 device = None, success_tolerance = 4, dtype = torch.float32,
    ...
    def _train_plot_update(self, pred_, validate_y, steps_displayed, elastic_losses = None, restart_triggered = False):
        ...
        if self.interactive:
            ...
            for i in range(self.n_trust_regions):
                ...
                # if self.log_score:
                #     assert False
                self.ax[0].plot(10**np.array(self._errorz_step[i]), alpha = 0.5, color = "blue", label = labels_[0] )
                self.ax[0].plot(10**np.array(self._errorz[i]), alpha = 0.2, color = "green", label = labels_[1])
                #else:
                # self.ax[0].plot(np.array(self._errorz_step[i]), alpha = 0.5, color = "blue", label = labels_[0] )
                # self.ax[0].plot(np.array(self._errorz[i]), alpha = 0.2, color = "green", label = labels_[1])

                #plot 2: the turbo state
                self.ax[1].plot(np.log(self._length_progress[i])/self.log2, color = 'blue', label = plot2_label)

            if self.n_trust_regions > 1:
                pct_complete = (self.n_evals/self.max_evals) * 100
                self.ax[0].set_title(f'% complete: { pct_complete:.0f}')

            self.ax[1].axhline(np.log(len_max)/self.log2, color = 'green', label = 'max length')
            self.ax[1].set_title("TURBO state")
            
            self.ax[1].axhline(np.log(len_min)/self.log2, color = 'red', label = 'target length')
            self.ax[1].legend()  
            ...
            if self.ODE_order and pred_2plot.shape[1] == 2:
                self.ax[2].plot(pred_2plot[:,0], pred_2plot[:,1], alpha = 0.3, color = "red", label = "latest pred")
                self.ax[2].set_title("Phase space")
            else:
                if not self.ODE_order:
                ...
                self.ax[2].plot(pred_2plot, alpha = 0.3, color = "red", label = "latest pred")
                self.ax[2].set_title("Val Set Prediction")
                self.ax[2].set_ylabel("y")
                self.ax[2].set_xlabel(r'$t$')
            
            self.ax[1].legend(**legend_font_dict)
            self.ax[2].legend(**legend_font_dict)
    ...
    def eval_objective(self, parameters, plot_type = "error", *args):
        ...
        #breakpoint()
        results = ray.get([execute_objective.remote(self.parallel_args_id, parameter_lst[i], i) for i in range(num_processes)])
    ...
    def _turbo_1(self):
        ...
        #self._errorz_step[0] += [max(self._errorz[0])] * self.initial_samples #n_init
        ...
        while not self.state.restart_triggered: 
            ...
            self.updates(Y_next, updates_dict)
            ...
            self._errorz_step[0] += [min(self._errorz[0])] * self.batch_size
    ...
    def _turbo_initial_samples(self, X_init, dim, turbo_state_id):
    ...
    def updates(self, scores, batch_dict):
        ...
        trust_region_id = batch_dict["trust_region_id"]

        state = self.states[trust_region_id]
        for i, score in enumerate(scores):
            self._errorz[trust_region_id].append(score)
            self._length_progress[trust_region_id].append(state.length)
        if self.interactive:
            self._train_plot_update(pred_ = batch_dict["pred"], validate_y = batch_dict["y"], steps_displayed = batch_dict["pred"].shape[0])
    ...
    def _turbo_m(self):
        ...
        for i in range(self.n_trust_regions):
            ...
            self.states[i] = self._turbo_initial_samples(X_init = X_init, dim = dim, turbo_state_id = i)
        ...
        while self.n_evals < self.max_evals: #not self.state.restart_triggered: 
            ...
            if self.parallel_trust_regions:
                objective_inputs = [(self.convert_params(X_next), i) for i, X_next in enumerate(X_nexts)]
                results = ray.get([eval_objective_remote.remote(self.parallel_args_id, objective_input, self.dtype, self.device) for objective_input in objective_inputs])
                Y_nexts = [result[0] for result in results]
                [self.updates(result[0], result[1]) for result in results]
            ...
            for i in range(self.n_trust_regions):
                # Update state 
                Y_next = Y_nexts[i]
                self.states[i] = update_state(state=self.states[i], Y_next=Y_next)
            ...
            self.n_evals += self.batch_size * self.n_trust_regions
