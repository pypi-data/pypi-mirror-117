...
class EchoStateNetwork(nn.Module):
    ...
    def fit(self, y = None, X=None, burn_in=0, input_weight=None, verbose = False , 
            ...
            backprop_f = None, epochs = None, hamiltonian = None): #beta = None, 
        # if verbose:
        #     if nl:
        #         print("nonlinear!")
        #     else:
    ...
    def test(self, y, X=None, y_start=None, steps_ahead=None, scoring_method='nmse', 
        ...
        if steps_ahead is None:
            if not self.ODE_order:
                y_predicted = self.predict(n_steps = y.shape[0], X=X, y_start=y_start, scale = True)
                score = self.error(predicted = y_predicted, target = y, method = scoring_method, alpha=alpha)
                if self.id_ == None:
                    return score, y_predicted.data #{"yhat": y_predicted.data, "ytest": y}, X[self.burn_in:]
                else:
    ...
    #         # state_t, _  = self.forward(t, input_ = input_t, #+1
    #         #                                     current_state = state_t, 
    #         #                                     output_pattern = output_prev)

    #         next_state, _ = self.train_state(t, X = input_t, state = state_t, y = output_prev)

    #         extended_state_spec = cat([next_state, input_t])
            
    #         next_output = dot(self.LinOut.weight.squeeze(), extended_state_spec.squeeze())
            
    #         next_output = next_output + float(self.LinOut.bias) #

    #         # states = cat([states, state_t.view(-1, self.n_nodes)], axis = 0)

    #         # output_t = self.output_i(input_t, state_t)
    #         states = cat([states, next_state.view(1,-1)], axis = 0)
    #         outputs = cat([outputs, next_output.view(1,self.n_outputs)], axis = 0)
    #     return states, outputs
    ...
    def predict(self, n_steps, X=None, y_start=None, continuation = True, scale = True, continue_force = True):
        ...
        #X =X, y, self.dev, supervised = True)
        self.X_val_extended =  check_x(self.X_val) #self.extend_X(self.X_val)
