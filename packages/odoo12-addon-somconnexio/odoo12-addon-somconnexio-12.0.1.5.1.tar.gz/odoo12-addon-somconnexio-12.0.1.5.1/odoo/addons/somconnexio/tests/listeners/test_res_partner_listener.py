...
@patch("odoo.addons.somconnexio.listeners.partner_listener.Customer")
class TestResPartnerListener(SavepointCase, ComponentMixin):
    ...
    def test_res_partner_listener_edit_address_field(self, CustomerMock):
        CustomerMock.get.return_value = Mock()
        CustomerMock.get.return_value.customer.customerAccounts = {
            'customerAccount': [
                {'code': "1234_1"},
            ]
        }
    ...
    def test_res_partner_listener_edit_address_many_accounts_field(self, CustomerMock):
        CustomerMock.get.return_value = Mock()
        CustomerMock.get.return_value.customer.customerAccounts = {
            'customerAccount': [
                {'code': "1234_1"},
        ...
        queue_jobs_after = self.env['queue.job'].search([])
        self.assertEquals(3, len(queue_jobs_after - queue_jobs_before))

    def test_res_partner_listener_edit_not_address_field(self, *args):
        queue_jobs_before = self.env['queue.job'].search([])
        self.partner.write({
            "name": "test"
        })
    ...
    def test_res_partner_listener_edit_address_and_not_address_field(self, CustomerMock): # noqa
        CustomerMock.get.return_value = Mock()
        CustomerMock.get.return_value.customer.customerAccounts = {
            'customerAccount': [
                {'code': "1234_1"},
