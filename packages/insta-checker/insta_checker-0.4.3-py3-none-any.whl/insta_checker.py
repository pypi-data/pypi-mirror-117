...
class InstaChecker:
    ...
    def deserialize_user(
        ...
        try:
            ...
            result['errors'].append(f'User deserialization error at line {sys.exc_info()[-1].tb_lineno}. Maybe instagram changed  API, have returned only full')
    ...
    def deserialize_media(
        ...
        try:
            ...
            if media_type in ['image', 'sidecar']:
                if full:
                    if media_type == 'image':
                        result = {
                            'full': media,
                ...
                result['errors'].append(f'Media of type "{media_type}" not currently supported, returned only full')
        except BaseException as ex:
            result['errors'].append(f'Media deserialization error at line {sys.exc_info()[-1].tb_lineno}. Maybe instagram changed  API, returned only full')
        if not len(result['errors']):
            del result['errors']
        return result
                    ...
                    if len(response_result['data']['ProfilePage']):
                        ...
                        if 'graphql' in response_result['data'].keys():
                            ...
                            if 'user' in response_result['data'].keys():
                                ...
                                print(response_result['data'].keys())
        ...
        try:
            ...
            if 'graphql' in response_result['data'].keys():
                ...
                if 'shortcode_media' in response_result['data'].keys():
                    ...
                    if self.debug:
                        super_print(response_result['data'])
                    deserialized_data = self.deserialize_user(response_result['data'], full=True)
                    if 'errors' in deserialized_data.keys():
                        response_result['errors'] += deserialized_data['errors']
                        del deserialized_data['errors']
                    response_result['data'] = deserialized_data
