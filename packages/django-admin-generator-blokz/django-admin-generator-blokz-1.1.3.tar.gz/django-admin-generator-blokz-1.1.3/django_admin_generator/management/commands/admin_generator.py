...
class CustomSafeDeleteAdmin(SafeDeleteAdmin):
    ...
    Override it to add 'deleted' field to list_displays and list_filters 
    as well as highlighting deleted objects automatically.
...
class %(name)sAdmin(ObjectPermissionsModelAdmin, SafeDeleteAdmin):
%(class_)s
'''
