...
import boto3
import pandas as pd
def read_csv_from_s3(source_data_bucket_name, source_data_path_prefix):
    prefix_objs = list(boto3.resource('s3').Bucket(source_data_bucket_name).objects.filter(Prefix=''.join([source_data_path_prefix])))
    prefix_df = []
