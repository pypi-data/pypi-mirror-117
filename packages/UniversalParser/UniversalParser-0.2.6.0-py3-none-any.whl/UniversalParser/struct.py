...
class ChainXML:
    ...
    def signal_move_node(self, move_node, from_node, to_node) -> None:
        if id(from_node) != self._contrast_ids[id(move_node)]:
        ...
        while queue:
            ...
            if pop_tag not in parent_node:
                ...
                if isinstance(parent_node[pop_tag], ChainDict):
                    ...
                    self.signal_move_node(t_node, from_node=parent_node, to_node=temp_list)
                    temp_list.append(temp_node)
    ...
    def _build_by_copy(self, datas: Union[dict, list], root_node: ChainDict) -> None:
        ...
        while queue:
            ...
            if pop_tag not in parent_node:
                parent_node[pop_tag] = temp_node
                self.signal_add_node(temp_node, parent_node, tag=pop_tag, text=temp_node[self.real_cdata_key])
            else:
                if isinstance(parent_node[pop_tag], ChainDict):
                    t_node = parent_node[pop_tag]
                    temp_list = []
                    ...
                    self.signal_move_node(t_node, from_node=parent_node, to_node=temp_list)
                    temp_list.append(temp_node)
                    self.signal_add_node(temp_node, temp_list, tag=pop_tag, text=temp_node[self.real_cdata_key])
                elif isinstance(parent_node[pop_tag], list):
                    parent_node[pop_tag].append(temp_node)
                    self.signal_add_node(temp_node, parent_node, tag=pop_tag, text=temp_node[self.real_cdata_key])
                else:
