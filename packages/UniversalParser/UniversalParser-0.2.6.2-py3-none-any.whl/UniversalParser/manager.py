...
class ChainManager:
        ...
        if isinstance(node, ChainDict):
            if tag not in node:
                ...
                if isinstance(temp_node, ChainDict):
                    ...
                    self.objects.signal_move_node(temp_node, from_node=node, to_node=temp_list)
    ...
    def move(self, move_node, from_node, to_node) -> None:
        ...
        if tag not in from_node:
            ...
            if isinstance(to_node, ChainDict):
                if tag not in to_node:
                    self.update_attr(move_node, self.loc_key, self.objects._get_last_loc(to_node))
                    ...
                    if isinstance(to_node[tag], ChainDict):
                        ...
                        self.objects.signal_move_node(temp_node, from_node=to_node, to_node=temp_list)
                        temp_list.append(move_node)
                        to_node = temp_list
                    else:
                        self.update_attr(move_node, self.loc_key, self.objects._get_last_loc(to_node[tag]))
                        to_node[tag].append(move_node)
            else:
                self.update_attr(move_node, self.loc_key, self.objects._get_last_loc(to_node))
                to_node.append(move_node)
                
        self.objects.signal_move_node(move_node, from_node=from_node, to_node=to_node)
