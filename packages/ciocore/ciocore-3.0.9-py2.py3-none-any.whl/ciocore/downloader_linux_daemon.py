...
from ciocore import api_client, common, downloader_orig, exceptions, loggeria
...
# The frequency (in seconds) for which the Touch thread should report progress of the file (outside
# of start/finish)
TOUCH_INTERVAL = 120
...
# Global run-state variable that decorators and other functions can use to know when they should
# exit
 
if sys.version_info < (3,):
    RUN_STATE = multiprocessing.Array("c", "stoppingorstuff")
else:
    RUN_STATE = multiprocessing.Array("c" , "stoppingorstuff".encode('utf-8'))
...
class DecAuthorize(object):
    """
    Decorator that adds an authentication header to the wrapped function's "headers" argument.
    Automatically renews tokens when encountering 401 errors and retries the function
    """
    ...
    def __call__(self, function):
        ...
        def decorater_function(*args, **kwargs):
            ...
            # update the functions headers dictionary, overwriting/adding an "authorization"
            # key/value
            ...
            try:
                ...
                # If a 401 exception occurs, fetch a new token, update the auth header with it, and
...
class DecDownloaderRetry(common.DecRetry):
    ...
    def __init__(
        self, run_value, retry_exceptions=Exception, skip_exceptions=(), tries=8, static_sleep=None
    ):
        self.run_value = run_value
        super(DecDownloaderRetry, self).__init__(
    ...
    def sleep(self, seconds):
        ...
        Instead of doing one long sleep call, we make a loop of many short sleep calls. This gives
        the opportunity to check the running state, and exit the sleep process if necessary.
        """
        global RUN_STATE
...
class Downloader(object):
    """
    Top-level Downloader class that maintains all worker processes/threads and terminates them using
    shared state objects (signaled via SIGINT -KeyboardInterrupt).
    ...
    NOTE: For the sake of the brevity we'll refer to "threads" as "processes" for the rest of this
    documentation

    In order to shutdown these child processes, each process (including their own child processes
    has a corresponding multiprocessing.Array object that the parent uses to signal the child to
    shutdown.  Though processes can be terminated by having each one simply throwing an exception,
    we want to make sure that each process terminates cleanly (releasing resources and signaling to
    their own child processes). Therefore, every child process should only exit by returning from
    it's "run" method (don't raise an exception!).

    Also, every parent should "join" any of it's child processes before exiting itself.  This
    ensures that all children processes have indeed terminated properly. Otherwise this could lead
    to "zombie" processes (no parents).

    Life-cycle of a Process: 1. a process (Process1) is started by parent process (Parent1). Parent1
        retains a shared object to signal run-state change to Process1 2. Process1 creates and
        starts it's own child process (Child1) 3. Process1 runs indefinitely (looping within it's
        "run" method), constantly checking the status of it's run-state object. 4. a
        KeyboardInterupt (SIGINT) is triggered by the user, and is handled by Parent1.  Parent1
        signals to Process1 to shutdown (by changing the value of the run-state object). 5. Process1
        detects that the run-state value has changed, and calls it's own "_stop" method. 6. The stop
        method changes the run-state object for the Child1 process, (triggering Child1 to go through
        the same shutdown logic (step 5) 7. Before returning from the "run" method, Process1 "joins"
        the Child1 process so that it blocks (and waits for Child1 to exit).

    The parent/calling process uses a process-safe multiprocessing.Array object to communicate to
    this process when needing to shutdown/cleanup/exit. Available state values: - "running":
    all systems go. - "stopping":      stop everything and cleanup.

    The DownloadWorker process has a child thread of it's own (TouchWorker), which is responsible
    for communicating the progress of the current file download to the conductor backend.  It, too,
    uses the a multiprocessing.Array object to control signal when it should be stopped or not.
    """
    ...
    def __init__(self, args):
        ...
        # Create a results queue that will hold the results of all files that are downloaded from
        # each DownloadWorker
    ...
    def start_daemon(self):
        """
        Start the downloader daemon. Create all child worker processes, start them, and join them.
        This is blocking function and will not exit until all workers have exited their prococess.
    ...
    def stop_daemon(self):
        ...
        1. change the run_state value of each worker's shared object value. This will trigger each
           worker's stop/cleanup behaviors.
        2. Exit the application runtime by raising an exception (we must break out of the join that
           occurred in the start_daemon method).
        """

        # Cycle through each worker, and change the share object's state value to "stopping
        for worker in self._workers:
            run_state =self._workers[worker]
            LOGGER.debug(
                "changing %s from %s to %s", worker.name, run_state.value, self.STATE_STOPPING
            )
            run_state.value = self.STATE_STOPPING

        # Join the workers. It's generally good practice to do this. Otherwise the parent process
        # can exit (and return control back to shell) before the child processes exit (creating
    ...
    def _create_workers(self, start=True):
        """
        Create child worker processes.  For each worker, create a process/thread safe object that is
        used to communicate between this parent process and the worker.  Return a dictionary where
        the key is the process object, and the value its corresponding run_state state object.
    ...
    def sigint_handler(self, sig, frm):
        ...
        Note that when registering this function as the signint handler, this function gets called
        by ALL child processes as well.  Because of this, we check which process is calling it, and
        only execute it if it's the main (parent) process.
        """
...
class DownloadWorker(multiprocessing.Process):
    ...
    def __init__(
        ...
        # Set the bytes counter to 0 The bytes counter tracks the current amount of bytes have
        # processed (hashed or downloaded) for the current file download.
    ...
    def run(self):
        ...
        This serves as the outer/upper wrapper function which is responsible for: 1. Creating and
            starting any child workers of its own (TouchWorker, etc). 2. Calling the "inner" run
            function to continually query for and download files that are pending download. 3.
            Monitoring the run_state value to determine whether to exit the loop or not. 4. Catching
            all unexpected exceptions and preventing the process from dying
        ...
        In order to shutdown the Downloader in general, it's important that this method is exited
        properly (reaches the end of the method). i.e. don't allow any exceptions to be raised.
        """
        time.sleep(random.randint(0, 200) / 100.0)
        ...
        # While the downloader is in a running state, continue to try to download a file.  If an
        # exception occurs, catch and log it, and restart the loop
        while self._run_state.value == Downloader.STATE_RUNNING:
            ...
            # catch all other exceptions here. This should hopefully never happen. But catch it so
            # that the worker process doesn't die.
            except:
                LOGGER.exception("Preventing process from exiting due to Exception:\n")
                # wait a little to allow for exception recovery . TODO:(lws) this may be totally
                # stupid/unnecessary)
                self._wait()
        ...
        # Drain the queue. Note that the process may not die properly unless the queue is emptied
        # first. Hang forever from parent's call to Join.
    ...
    def _create_workers(self, start=True):
        ...
        WORKERS: - TouchWorker # Reports live progress of current file download to conductor backend
        """
        workers = {}
    ...
    def _run(self):
        ...
        id_, jid, tid, destination, file_size = (
            dl_info["id"],
            dl_info["jid"],
            dl_info["tid"],
            dl_info["destination"],
        ...
        # Catch all other exceptions for the file download, and report the failure via http call.
        ...
        if success:
            ...
            Backend.fail(
                id_,
                bytes_downloaded=self._bytes_counter.value,
                account=self.account,
                location=self.location,
                project=self.project,
            )
        ...
        result = self._construct_result_dict(
            dl_info, id_, local_file, action, time_started, time_ended
        )
    ...
    def _get_next_download(self):
        ...
        downloads = (
            Backend.next(self.account, location=self.location, project=self.project, number=1) or []
        )
        if downloads:
    ...
    def _adapt_payload(payload):
        ...
        """
        # Change the id to use an underscore so that it doesn't conflict with python built-in "id"
    ...
    def _maybe_download_file(self, local_file, id_, url, dl_info):
        """
        checks for existing file and validates md5, if valid, skip download. Return True if the file
        is actually downloaded (and None if it is skipped)
        ...
        # If the file doesn't already exists on disk and have the proper hash, then download the
        # file
    ...
    @DecDownloaderRetry(
        run_value=Downloader.STATE_RUNNING,
        skip_exceptions=(exceptions.DownloaderExit, exceptions.FailDownload),
        tries=MAX_DOWNLOAD_RETRIES,
    )
    def download(self, id_, local_file, url, dl_info):
        """
        The "outer" download function that wraps the "real" download function in retries (decorator)
        and md5 verification.  All exceptions that are encountered will be automatically
        retried...except for DownloaderExit
        ...
        # Validate that the new md5 matches the expected md5 If the md5 does not match, then delete
        # the file that was just downloaded and raise an exception.  This will trigger a retry (via
        # the decorator).
        if new_md5 != md5:
            try:
                self._log_msg(jid, tid, "Removing bad file", local_file, log_level=logging.DEBUG)
                os.remove(local_file)
            except:
                self._log_msg(
                    jid, tid, "Could not cleanup bad file", local_file, log_level=logging.WARNING
                )

            # Raise an exception. This will cause a retry
            raise Exception(
                "Downloaded file does not have expected md5. "
    ...
    def _download(self, id_, local_file, url, dl_info, progress_queue):
        """
        Download the given file url to the given local_file path. Return the md5 (base64) of the
        downloaded file.

        Per each chunk of the file, increment the bytes_counter and add the total bytes to the
        progress_queue object.  The progress_queue is read  by the TouchWorker thread  to indicate
        whether to "touch" the backend, and if so, how many bytes to report as downloaded.
        """
        # unpack some values for convenience
        ...
        with open(local_file, "wb") as file_obj:
            for chunk in response.iter_content(chunk_size=DOWNLOAD_CHUNK_SIZE):
                ...
                if chunk:
                    ...
                    # Put the the bytes counter value into the progress queue
        ...
        if close_duration > 30:
            msg = (
                "File closing took: %s seconds, yikes! This may cause a file "
                "download to get reset.  This is likely due to poor disk "
                "performance. Consider reducing thread thread count for "
                "Downloader or use a more performant file system!"
            ) % common.get_human_duration(close_duration)
    ...
    def _file_exists_and_is_valid(self, id_, local_file, md5, jid, tid):
        """
        Validate that the give filepath exists on disk and its md5 matches that of the given one.
        Return True if that's the case.

        While md5 hashing an existing file, update the _bytes_counter and _progress_queue so that
        the TouchWorker can report the progress to the conductor backend
        """
        ...
        if local_md5 != md5:
            self._log_msg(
                jid,
                tid,
                "Md5 mismatch (%s vs %s)" % (local_md5, md5),
                local_file,
        ...
        self._log_msg(
            jid, tid, "File already downloaded and verified.", local_file, log_level=logging.DEBUG
        )
        return True

    def _cleanup_download(self, id_, jid, tid, destination, local_file):
        """
        Cleanup the download.  This is currently a No-op, but serves as a slot for any cleanup that
        may need to happen for the download before for the download worker process exits. As of now,
        a download file will get automatically reset on the backend if the downloader hasn't
        reported activitiy about it after a few mins.
        """
    ...
    def _reset_progress(self):
        """
        Reset the state data of the current file download. Reset the bytes counter and empty the
        progress queue.
        """
    ...
    def _log_msg(self, jid, tid, message, local_file, ljust=37, log_level=logging.INFO):
        """
        Log a message about the given file.  This is a convenience function that creates a message
        that is structured and consistent throughout the downloading processes. This makes
        readability easier when trolling through logs.

        example output: <jid>|<tid>  <message  <file path>
        ...
        LOGGER.log(
            log_level,
            msg_template,
            {"message": message.ljust(ljust), "jid": jid, "tid": tid, "filepath": local_file},
        )

    def _log_progress(self, local_file, jid, tid, file_size, log_level=logging.INFO):
        """
        Log a message about the download progress of the given file. This uses the current state
        info (bytes_transferred) to calculate progress.
        ...
        ratio_str = "%s/%s" % (
            common.get_human_bytes(self._bytes_counter.value),
    ...
    def _construct_result_dict(self, dl_info, id_, local_file, action, time_started, time_ended):
        """
        Construct a "result" dictionary that contains information about how the download was
        handled. Contains the following keys:
    ...
    def _update_bytes_counter_callback(self, id_, filepath, file_size, bytes_processed, log_level):
        """
        A callback function that the md5 hashing function uses to update the _byte_counter and
        progress_queue.

        Check the run_state value and raise a DownloaderExit exception in order to exit/break out of
    ...
    def _stop(self):
        """
        Cycle through all child worker processes and change their run_state objects to signal them
        to stop. Join all child processes to block until they all exit properly
        """
        for worker in self._workers:
            state = self._workers[worker]
            LOGGER.debug(
                "changing %s from %s to %s", worker.name, state.value, Downloader.STATE_STOPPING
            )
            state.value = Downloader.STATE_STOPPING

        LOGGER.debug(
            "waiting for procs to exit:\n\t%s", "\n\t".join(sorted([w.name for w in self._workers]))
        )
...
class TouchWorker(threading.Thread):
    """
    A Thread that periodically reports/"touches" file progress to the Conductor backend. Aside from
    reporting the byte progress of a file, it also serves as a "heartbeat" for a file download,
    indicating to the backend that the file is still in progress and to not consider it a
    stranded/dead file download.
    ...
    2. However, the touch will only be executed  if there is data in the progress_queue. If there is
       no data in the progress queue then a "touch" will not not be issued. The progress queueu
       should have constant data streaming through it (either when md5 hashing or downloading the
       file). If there is no data getting put into it, then it means that the DownloadWorker has
       hung on that file for some reason.  Eventually the backend will "reset" that download because
       it will not have been touched after a few minutes.
    """
    ...
    def __init__(
        ...
        """
        run_state:    multiprocessing.Array object. Used by the parent calling process to
                      communicate when to shutdown/exit this Process.

        progress_queue: multiprocessing.Queue object.

        interval: int/float. The frequency to potentially issue a Touch to the backend process_name:
        str. the name of the parent process. This will serve as a prefix for the thread's name.
        """
    ...
    def run(self):
        ...
        Report on the last entry found in the progress queue. If there are no entries, then do not
        report anything. We want the backend to reset any "stranded"/"stuck" downloads, so only
        report progress when there IS progress. Progress entries only get added to the queue when a
        file download is being actively hashed or downloaded.
        """
        self._run_state.value = Downloader.STATE_RUNNING
        ...
        while self._run_state.value == Downloader.STATE_RUNNING:
            ...
            self._wait(seconds=0.2)

        LOGGER.debug("Exiting thread")

    def _touch(self, file_id, action, bytes_processed):
        LOGGER.debug(
            "Touching backend: id=%s _bytes_action=%s bytes_processed=%s"
            % (file_id, action, bytes_processed)
        )
        Backend.touch(
            file_id,
    ...
    @classmethod
    def _get_last_file_progress(cls, progress_queue):
        """
        Get the last item from the given queue object. We're only interested in the most recent item
        in the queue (all older ones are superseded by newer entries), but unfortunately there are
        no FILO queues for multiprocessing. No matter, we need to clear the contents of the queue
        anyway, so we'll remove all items and return the last item
        """
        progress_items = empty_queue(progress_queue)
        if progress_items:
...
class HistoryWorker(multiprocessing.Process):
    """
    A process that periodically print/logs the "history" of the last x files that have been
    downloaded/uploaded.
    """
    ...
    def __init__(
        ...
        results_queue: multiprocessing.Queue object.  Contains an entry for each file that was
                       handled by the worker and what happened with it.
    ...
    def run(self):
        """
        Start and return a thread that is responsible for pinging the app for files to transfer (and
    ...
    def _construct_history_summary(self, results_list):
        """
        For the given results list, return a printable/loggable history summary for them.

        """
        title = " %s HISTORY %s " % (
            self._label,
            ("(last %s files)" % self._history_max) if self._history_max else "",
        )
        table = downloader_orig.HistoryTableStr(
...
class Backend:
    ...
    def headers(cls):
        bearer = api_client.get_bearer_token()
        return {"accept-version": "v1", "authorization": "Bearer %s" % bearer.value}
...
class Counter(object):
    """
    Acts as a mutable integer variable, so that adding/subtracting can be done to the same object
    (retaining pointers to same object/varaiable in memory)
...
def empty_queue(progress_queue):
    """
...
def safe_mkdirs(dirpath):
    """
    Create the given directory.  If it already exists, suppress the exception. This function is
    useful when handling concurrency issues where it's not possible to reliably check whether a
    ...
    # only catch OSERrror exceptions
        ...
        # This exception might happen for various reasons. So to be sure that it's due to the path
        # already existing, check the path existence. If the path doesn't exist, then raise the
        # original exception. Otherwise ignore the exception bc the path exists.
