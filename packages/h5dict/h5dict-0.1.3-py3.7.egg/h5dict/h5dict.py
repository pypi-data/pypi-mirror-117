...
class h5dict:
    ...
    def __setitem__(self, key, value):
        """Tryes to create a dataset with informed value, if unable creates
            groups and datasets indide thoes groups.
        ...
        if self.in_write_mode():
            # If key is not string convert it and stores original type for
            # back conversion on load
    ...
    def instance(self, hdf5, key):
        ...
        self.check_closed()
        return h5dict(hdf5, self.name, self.mode, self.conversions,
                      parent=self, key=key)
    ...
    def convert_key(self, key):
        ...
        if key != str:
            ...
            try:
                ...
                raise TypeError("Type {} of key {} can not be used.".format(
    ...
    def to_dict(self):
        """Converts all the data the current HDF5 file or group in dicts
            and lists.
        ...
        if self.parent is not None:
            if self.key+TYPE_SUFIX in self.parent.keys():
                d = try_convert_to(d, self.parent[self.key+TYPE_SUFIX],
                                   self.conversions)
        return d
