...
@author: npkn
"""
def interpolate(Xdata,Ydata,Xnew,iter):
    def datamodel(Xdata,Ydata,iter):    
        import numpy as np
        ...
        if LenX==LenY:
            if round(np.max(Xdata)) >= round(np.min(Xdata)):
                ...
                deg=np.min(Xdata)
            maxK=5
            maxM=int(np.floor(iter/maxK))
            for m in range(1,maxM+1):
                for k in range(1,maxK+1):
                    print("calculating iteration {} out of {} iterations".format((m-1)*maxK+k,maxK*maxM))
    ...
    ParaSet=datamodel(Xdata,Ydata,iter)
