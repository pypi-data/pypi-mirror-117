...
class RuleBasedDisambiguation:
    ...
    def __auxiliary_verb_or_alt_decision(self, sentence: tp.List[str], word_index: int,
        ...
        :rtype: `tp.List[str]`
        """
        if self.__is_main_verb(sentence[word_index + steps_forward]):
            return ['VERB']
        else:
            return alt_label
    ...
    def __check_label_after(labels: tp.List[tp.List[str]], word_index: int,
        ...
        :rtype: `bool`
        """
        label_to_check = labels[word_index + steps_back]
        if word_index < len(labels) - steps_back and any([label in target_label for label in label_to_check]):
            return True
        else:
            return False
    ...
    def __check_label_before(labels: tp.List[tp.List[str]], word_index: int,
        ...
        """
        label_to_check = labels[word_index - steps_back]
        if (word_index - steps_back >= 0) and any([label in target_label for label in label_to_check]):
            return True
        else:
            return False
    ...
    def __check_word_before(sentence: tp.List[str], word_index: int,
        ...
        :rtype: `bool`
        """
        word_to_check = sentence[word_index - steps_back]
        if (word_index - steps_back >= 0) and (word_to_check in target_words):
            return True
        else:
            return False
    ...
    def __case_adjective_substantive(self, sentence, labels, word_index) -> tp.List[tp.List[str]]:
        ...
        if word_index != 0:
            ...
            elif sentence[word_index] in ['plano', 'planos', 'acordo', 'seguro', 'celular']:
                labels[word_index] = ['SUBS']
            
            elif sentence[word_index] in ['novo']:
                labels[word_index] = ['ADJ']
