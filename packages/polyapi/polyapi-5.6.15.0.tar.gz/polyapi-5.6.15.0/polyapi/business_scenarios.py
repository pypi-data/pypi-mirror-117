...
from .common import log, timing, raise_exception, TypeConverter, MULTISPHERE_ID, GRAPH_ID, CODE_NAME_MAP, OPERANDS
...
class BusinessLogic:
    ...
    def __init__(self, login: str, url: str, password: str = None, session_id: str = None,
        ...
        # предварительный URL стенда Полиматики (точный URL будет сформирован после авторизации)
        url = self._get_prepare_url(url)
        ...
        # таблица команд и состояний
        self.server_codes = Executor.get_server_codes(url)
        ...
        suffix_url = self._login(password, url, suffixes_url)

        # получаем все необходимые урлы (URL для API-запросов, URL для ресурсов)
        self.url, self.resources_url, self.login_url = self._get_urls(url, suffix_url)
    ...
    def _get_urls(self, url: str, suffix_url: str) -> [str, str, str]:
        """
        ...
        :param url: (str) предварительный URL-адрес стенда Полиматики.
        ...
        """
        return "{}{}".format(url, suffix_url), "{}resources".format(url), "{}login".format(url)
    ...
    @timing
    def _login(self, password: str, url: str, suffixes_url: List) -> str:
        """
    ...
    def filter_pattern_change(self, dim_id: str, num: int, pattern_list: list) -> Dict:
        ...
        :param pattern_list: (list) список паттернов; должен включать в себя словари формата
            {"pattern": <value>, "type": <value>}, где:
                pattern - любой паттерн поиска в строковом виде;
        ...
        for current_pattern in pattern_list:
            ...
            # проверка pattern на строковый тип
    ...
    def get_current_datetime_format(self) -> str:
        ...
        return self.h.parse_result(config_data, 'configuration', 'datetime_format')

    @timing
    def put_dim_filter(self, dim_name: str, filter_name: Union[str, None] = None, start_date: Union[int, str] = None,
                       end_date: Union[int, str] = None, filter_field_format: Union[str, None] = None) -> Dict:
        """
        Установка фильтра по размерностям.
        :param dim_name: (str) Название размерности.
        :param filter_name: (str) Название фильтра; в случае, если нужно указать интервал дат, необходимо передать None.
        :params start_date, end_date: (int, datetime.datetime) Начальная и конечная дата фильтрации соответственно.
        ...
        :filter_field_format (str): формат даты-времени, в котором фильтруемая размерность представлена в Полиматике.
            Актуально только в том случае, если в параметрах start_date и end_date указаны строки-даты.
    ...
    @timing
    def measure_rename_group(self, group: str, new_name: str, module: str = None) -> Dict:
        """
    ...
    @timing
    def measure_remove_group(self, group: str, module: str = None) -> Dict:
        """
    ...
    def create_calculated_measure(self, new_name: str, formula: str) -> Dict:
        ...
        Правила написания формул:
            1. При объявлении фунции между именем функции и открываюшей скобкой не должно быть пробелов.
                Верное написание: "top(", "total(", "corr(".
                Неверное использование: "top (", "total (", "corr (".
            2. Факты необходимо указывать в квадратных скобках. Например, [Сумма], [Количество].
            3. В качестве разделителя параметров (фактов) в методах top и corr необходимо использовать точку с запятой.
                Верное написание: "top([Сумма];100)", "corr([Сумма];[Выручка])".
                Неверное написание: "top([Сумма],100)", "corr([Сумма],[Выручка])".
            4. Все использующиеся операнды и функции должны быть разделены пробелами.
                Верное использование: "100 + [Сумма]", "corr([Сумма];[Выручка])"
        Примеры корректных формул:
            top([Сумма долга];1000) + total([Остаток])
            100 + [Больницы] / [Количество вызовов врача] * 2 + corr([Количество вызовов врача];[Больницы])
        :param new_name: (str) название создаваемого вычислимого факта.
        :param formula: (str) формула, соответствующая описанным выше правилам.
        ...
        output, opening_brackets, closing_brackets = str(), 0, 0
        ...
        try:
            for i in formula_lst:
                if i in ["(", ")", "not", "and", "or"]:
                    ...
                    # total([факт])
                    m = re.search(r'\[(.*?)\]', i)
                    total_content = m.group(0).strip()
                    measure_id = get_measure_id(get_measure_name(total_content))
                    output += "total({})".format(measure_id)
                    continue
                elif "top(" in i:
                    # top([факт];сколько)
                    m = re.search(r'\[(.*?)\]', i)
                    top_content = m.group(0).strip()
                    measure_id = get_measure_id(get_measure_name(top_content))
                    m = re.search(r'\d+', i)
                    int_value = m.group(0).strip()
                    output += "top( fact({}) ; {} )".format(measure_id, int_value)
                    continue
                elif "if(" in i:
                    ...
                    measure1 = get_measure_id(get_measure_name(measure1))
                    measure2 = get_measure_id(get_measure_name(measure2))
                    output += "corr( fact({}) ; fact({}) )".format(measure1, measure2)
                    continue
                elif i[0] == "[":
                    ...
                    if "," in i:
                        ...
                        elem = measure_content[0].strip()
                        measure_name = measure_content[1].strip()
                        ...
                        output += " fact({}; {}) ".format(measure_id, top_dim_dict[elem])
                        continue
                    measure_id = get_measure_id(get_measure_name(i))
                    output += " fact({}) ".format(measure_id)
                    continue
                elif (i in OPERANDS) or self._is_numeric(i):
                    output += i
                    continue
                else:
    ...
    @timing
    def run_scenario_by_user(self, scenario_name: str, user_name: str, units: int = 500, timeout: int = None):
        """
    ...
    @timing
    def clone_olap_module(self, module: str = None) -> Union[str, str]:
        """
        ...
        :param module: название/идентификатор клонируемого OLAP-модуля;
            если модуль указан, но такого нет - сгенерируется исключение;
            если модуль не указан, то берётся текущий (активный) модуль (если его нет - сгенерируется исключение).
        :return: (str) идентификатор нового модуля.
        ...
        :call_example:
            ...
            2. Открываем произвольный куб:
                bl_test.get_cube("cube_name").
            3. Вызов метода:
                new_module_uuid, new_module_name = bl_test.clone_olap_module()
            4. Вызов метода с передачей идентификатора/названия модуля:
                module = "module_id_or_name"
                new_module_uuid, new_module_name = bl_test.clone_olap_module(module=module).
            5. Вызов метода с передачей невалидного идентификатора/названия модуля:
                module = "invalid_module_id_or_name"
                new_module_uuid, new_module_name = bl_test.clone_olap_module(module=module).
        """
    ...
    def sort_measure(self, measure_name: str, sort_type: str) -> Dict:
        ...
        measure_data = self.execute_olap_command(command_name="fact", state="list_rq")
        all_measures = self.h.parse_result(measure_data, 'facts')
    ...
    def create_sphere(self, cube_name: str, source_name: str, file_type: str, update_params: Dict,
        ...
        :param sql_params: (Dict) параметры для источника данных SQL.
            Поля, передаваемые в словарь: "server", "login", "passwd", "sql_query"
            Пример:
                {
                    "server": "10.8.0.115",
                    "login": "your_user",
        ...
        # соединиться с бд в тестовом режиме
        self.execute_manager_command(
        ...
        # TODO: непонятно, зачем на этом этапе делать эту на первый взгляд бессмысленную операцию; нужно разобраться
        # if file_type == "csv":
        #     self.execute_manager_command(
    ...
    def wait_cube_loading(self, cube_name: str) -> str:
        ...
        file_url = "{}/log?cube_id={}".format(self.resources_url, self.cube_id)
    ...
    def group_dimensions(self, selected_dims: List) -> Dict:
        ...
        :param selected_dims: (List) список выбранных для группировки размерностей;
            регистр написания важен, а в списке ожидаются только строки.
        :return: (Dict) результат команды ("view", "group").
        """
        # проверки selected_dims
        if not selected_dims:
            return self._raise_exception(ValueError, 'Param "selected_dims" not set!')
        if not isinstance(selected_dims, list):
            return self._raise_exception(ValueError, 'Param "selected_dims" is not "list" type!')

        # удаляем все пробельные символы в selected_dims и проверяем, что все элементы имеют тип str
        for i, selected_dim in enumerate(selected_dims):
            if not isinstance(selected_dim, str):
                return self._raise_exception(ValueError, 'Expected "str" type in "selected_dims" param!')
            selected_dims[i] = selected_dim.strip()

        # получение количества верхних размерностей
        result = self.execute_olap_command(
            command_name="view", state="get", from_row=0, from_col=0, num_row=500, num_col=100)
        top_dims_qty = len(self.h.parse_result(result, "top_dims"))
        ...
        # получаем самые левые размерности элементов
        left_dim_values = [lst[0].strip() for lst in data]

        # проходим по списку selected_dims;
        # если элемент размерности есть в мультисфере - добавляем его, в противном случае генерируем ошибку
        # таким образом гарантируется, что в selected_indexes после прохождения цикла будет хотя бы один элемент
        selected_indexes = set()
        for current_dim_value in selected_dims:
            if current_dim_value in left_dim_values:
                selected_indexes.add(left_dim_values.index(current_dim_value))
            else:
                return self._raise_exception(ValueError, 'Invalid dimension element name: "{}"'.format(
                    current_dim_value))
        ...
        result = self.execute_olap_command(
            command_name="view", state="group", position=1, line=sorted_indexes[0], level=0)

        # обновить total_row и вернуть результат
        self.update_total_row()
        return result
    ...
    def get_module_list(self) -> List:
        """
        [ID-3123] Возвращает список модулей в активном слое текущей сессии.
        :return: (list) список вида [[module_id, module_name, module_type], [...], ...],
    ...
    @timing
    def column_resize(self, module: str = None, width: int = 200, olap_resize: bool = False) -> Dict:
        """
    ...
    def set_measure_direction(self, measure_name: str, is_horizontal: bool) -> Dict:
        """
        Установка расчёта по горизонтали для заданного факта.
        :param measure_name: (str) название факта.
        ...
        # получаем идентификатор факта по его имени и исполняем команду
        measure_id = self.get_measure_id(measure_name)
        return self.execute_olap_command(
...
class GetDataChunk:
    ...
    def load_sphere_chunk(self, units: int = 100, convert_type: bool = False, default_value: Any = None) -> Dict:
        """
        Генератор, подгружающий мультисферу постранично (порциями строк). При этом в мультисфере не должно быть
        вынесенных вверх размерностей - в противном случае будет сгенерировано исключение. Важно, чтобы перед вызовом
        метода были развёрнуты все узлы мультисферы - это необходимо для корректного получения данных.
        :param units: (int) количество подгружаемых строк; по-умолчанию 100.
        :param convert_type: (bool) нужно ли преобразовывать данные из типов, определённых Полиматикой, к Python-типам;
            по-умолчанию False (т.е. не нужно).
        :param default_value: (Any) актуален только при convert_type = True;
            используется в случае, если не удалось преобразовать исходные данные к нужному типу; по-умолчанию None.
        ...
        :call_example:
            ...
            gen = gdc.load_sphere_chunk(units="units", convert_type="convert_type", default_value="default_value")
        ...
        while total_row > 0:
            ...
            for item in rows_data[1:]:
                ms_data = type_converter.convert_data_types(self.column_types, item, default_value) \
                    if convert_type \
                    else item
