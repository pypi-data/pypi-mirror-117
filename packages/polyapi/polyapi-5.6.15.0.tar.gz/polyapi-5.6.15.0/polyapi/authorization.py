...
class Authorization:
        ...
        if password is None:
            ...
            if not r.ok:
                ...
                try:
                    ...
                    msg = 'Stand "{}" not supporting non-password authorization. Please, specify the password!'.format(
                        url)
                    raise AuthError(msg)
            if len(r.history) > 0:
    ...
    def _check_language(self, language: str):
        ...
        """
        if language not in ("ru", "en", "de", "fr"):
            msg = 'Invalid language! Expected: "ru"/"en"/"de"/"fr", found: "{}"'.format(language)
            raise AuthError(msg)
