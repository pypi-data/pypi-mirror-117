...
from .common import raise_exception, FUNCS
...
class Helper:
    ...
    def parse_formula(self, formula: str) -> List:
        ...
        Примеры:
            1. "5 + [Сумма] * 2 + corr([Сумма];[ID])" -> ["5", "+", "[Сумма]", "*", "2", "+", "corr([Сумма];[ID])"]
        ...
        for formula_part in splitted_formula:
            # определяем, является ли текущая часть формулы функцией
            if not in_func:
            ...
            # определяем, является ли текущая часть формулы фактом
            if not in_measure:
                if '[' in formula_part:
                    in_measure = True
            if in_measure:
    ...
    def parse_result(self, result: Dict, key: str, nested_key: str = None) -> Any:
        """
        ...
        :return: (Dict) Значение заданного поля.
        """
