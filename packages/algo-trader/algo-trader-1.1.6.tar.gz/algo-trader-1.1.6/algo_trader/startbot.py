...
def file_create_input(check_exist=True, symbols=['XBTUSD'], broker='bitmex', strategy='lotus'):
    print("creating file input symbols", symbols)
    setting_name = '{}_{}_settings.pickle'.format(broker, strategy)
    print('setting name is', setting_name)
    setting_path = os.path.join(settings_path, setting_name)
    print('setting path is', setting_path)
    if check_exist and os.path.exists(setting_path):
        ...
        for symbol in symbols:
            print("asking input for this symbol")
            pos_size = input(
                "Choose your Position Size for {} per trade in %: ".format(symbol))
            pos_size = check_pos_size(pos_size)
            print("appending to symbol_settings")
            new_symbol = Symbol(name=symbol, position_size=pos_size)
            symbol_settings.append(new_symbol)
            print("added", new_symbol)
            print("symbol_settings are now", symbol_settings)
...
def make_pickle_file(filename, token, api_key, api_secret, symbol_settings):
    ...
    for setting in symbol_settings:
        print("for setting in symbol setting we have this setting", setting)
        settings.symbols.append(setting)
    pickle.dump(settings, file=open(
        os.path.join(settings_path, filename), 'wb'))
    print("saved the stuff and returned it, saved to", os.path.join(settings_path, filename))
    return settings
