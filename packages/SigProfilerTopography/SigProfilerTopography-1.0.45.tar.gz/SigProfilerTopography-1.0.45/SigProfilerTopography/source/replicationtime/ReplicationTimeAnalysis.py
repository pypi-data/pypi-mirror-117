...
def search_for_each_mutation_using_list_comprehension_using_numpy_array(mutation_row,
    ...
    if (uniqueIndexesArray.size>0):
        ...
        # if (sim_num==0) and (decile_index_array[0] or decile_index_array[1] or decile_index_array[2] or decile_index_array[3] or decile_index_array[4] ):
        ...
        # if (sim_num==0) and (decile_index_array[9] or decile_index_array[8] or decile_index_array[7] or decile_index_array[6] or decile_index_array[5]):
        #     my_dict = {a: b for a, b in zip(df_columns, mutation_row)}
        #     list_of_late_dict.append(my_dict)
        #     # print('DEBUG2 len(list_of_late_dict): ', len(list_of_late_dict))
        if (sim_num==0):
            my_dict = {a: b for a, b in zip(df_columns, mutation_row)}
            list_of_early_late_dict.append(my_dict)
...
def searchforAllMutations_using_numpy_array(sim_num,
    ...
    list_of_early_late_dict = [] # All ten deciles
    ...
    return sim_num, subs_signature_decile_index_accumulated_np_array, dinucs_signature_decile_index_accumulated_np_array, indels_signature_decile_index_accumulated_np_array, list_of_early_late_dict
##################################################################
...
def calculateCountsForMutationsFillingReplicationTimeNPArrayRuntime_using_numpy_array(computationType,
    ...
    def accumulate_np_arrays(result_tuple):
        ...
        list_of_early_late_dicts = result_tuple[4]

        # print('MONITOR ACCUMULATE', flush=True)

        list_of_all_early_late_dicts.extend(list_of_early_late_dicts)
...
def replicationTimeAnalysis(computationType,
    ...
    all_sims_dinucs_signature_decile_index_accumulated_np_array, \
    all_sims_indels_signature_decile_index_accumulated_np_array, \
    list_of_all_early_late_dicts  = calculateCountsForMutationsFillingReplicationTimeNPArrayRuntime_using_numpy_array(
        computationType,
        outputDir,
        jobname,
        numofSimulations,
    ...
    filename = 'Mutations_in_early_late_replicating_regions.txt'
    mutations_in_early_late_replicating_regions_df = pd.DataFrame(list_of_all_early_late_dicts)
    mutations_in_early_late_replicating_regions_df.to_csv(os.path.join(outputDir, jobname, filename), sep='\t', header=True, index=False)
