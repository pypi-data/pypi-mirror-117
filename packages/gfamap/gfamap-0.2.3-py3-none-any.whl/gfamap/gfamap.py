...
def create_parser():
    parser = argparse.ArgumentParser(prog = 'gfamap',
                                     usage = '%(prog)s [options] --graph assembly_graph.gfa --reference genome.fasta',
    ...
    optional.add_argument('--version', action = 'version', version = '0.2.3')
    optional.add_argument('-e', '--edges', metavar = '', help = 'comma-separated names of edges in the assembly graph to be mapped')
...
def annotate_gfa(gfa, mapping, prefix, verbose):
    ...
    annotated_gfa = 'annotated_' + os.path.basename(gfa)
