...
from dagster.utils import DEFAULT_WORKSPACE_YAML_FILENAME, merge_dicts
from dagster_cloud.headers.impl import get_dagster_cloud_api_headers
...
def _dagit_client(url, api_token):
    url = f"{url}/graphql"
    ...
    return _graphql_client(transport)


def _graphql_client(transport):
...
def add_command(url, api_token, location, **kwargs):
    execute_add_command(_dagit_client(url, api_token), location, kwargs)
...
def update_command(url, api_token, location, **kwargs):
    execute_update_command(_dagit_client(url, api_token), location, kwargs)
...
def delete_command(url, api_token, location):
    execute_delete_command(_dagit_client(url, api_token), location)
...
def list_command(url, api_token):
    execute_list_command(_dagit_client(url, api_token))
...
def sync_command(url, api_token, workspace):
    execute_sync_command(_dagit_client(url, api_token), workspace)
