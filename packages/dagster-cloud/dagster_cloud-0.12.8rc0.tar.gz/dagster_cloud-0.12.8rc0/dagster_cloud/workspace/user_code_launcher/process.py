...
class ProcessUserCodeLauncher(ReconcileUserCodeLauncher, ConfigurableClass):
    ...
    def start(self):
        # Ensure that the ephemeral subprocesses spun up for runs and steps
        # don't leave zombie processes in the process table after they finish
        # See https://www.geeksforgeeks.org/zombie-processes-prevention/
        self._original_signal_handler = signal.signal(signal.SIGCHLD, signal.SIG_IGN)
        super().start()
    ...
    def __exit__(self, exception_type, exception_value, traceback):
        if self._started:
            ...
            signal.signal(signal.SIGCHLD, self._original_signal_handler)
