...
# kaldi scp style reader
def read_scp(scp, sep=" ", clean=False):
    data = open(scp, encoding="utf-8").readlines()
    uttid2text = {}
    for i in data:
        uttid = i.strip().split(sep)[0]
        text = i.strip().split(sep)[1:]
        if clean:
            text = "".join(text).replace(".", "").replace("，", "").replace("<UNK>", "").replace("。", "").replace("、", "").replace(" ", "").replace(",", "")
            uttid = os.path.basename(uttid).replace(".wav", "")
        uttid2text[uttid] = text
    return uttid2text

def read_tsv(tsv):
    uttid2text = read_scp(tsv, sep="\t")
    return uttid2text
