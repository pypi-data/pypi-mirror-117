...
def pull_all(db, course_, dry_run):
    # TODO: when pulling an assignment group, it saves the canvas id and the
    # yaml file, but does not store the object in the db. It should though
    # right? Because if not, and you want to push an assignment, it looks up
    # the assignment group by searching the db for the AG object by name
