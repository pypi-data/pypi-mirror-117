...
def cmd_pull(db, args):
    ...
    for component_filepath in args.components:
        ...
        if os.path.isdir(component_filepath):
            ...
            # local versions
            for child_path in os.listdir(component_filepath):
                # TODO: anytime we call listdir like this, it might
                # unintentionally pull in files other than the component yaml
                # files. this would cause an error so you probably want to only
                # listdir on yaml files or something like that
                if not component_filepath.startswith("files"):
                    component = helpers_yaml.read(component_filepath + '/' +
                            child_path)
                    # TODO: also, if the above file doesn't exist, helpers_yaml
                    # will return None and break the next line
