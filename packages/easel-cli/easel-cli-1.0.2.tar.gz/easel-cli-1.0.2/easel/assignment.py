...
class Assignment(component.Component):
    ...
    def get_assignment_group_id(self, db, course_id):
        if self.assignment_group:
            ...
            # TODO: if a user initially pulls everything from a past course and
            # then, instead of pushing everything at once to the new course,
            # pushes an individual assignment, the assignment group it belongs
            # to will not be found and throw the below error. But it should be
            # capable of resolving the id for the name at that point because it
            # pulled the info to the file. The solution is probably just to
            # save the component to the db (in easel/commands.py in cmd_pull
            # about line 183 or so.
