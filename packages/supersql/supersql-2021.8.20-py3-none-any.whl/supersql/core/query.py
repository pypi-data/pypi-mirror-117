...
class Query(object):
    ...
    def _clone(self) -> "Query":
        ...
        """
        if self._pause_cloning: return self
        clone = type(self)(
            engine=self._engine,
            dsn=self._dsn,
            user=self._user,
        ...
        # if connected we don't want to reconnect or lose access to db that holds engine reference
        clone._db = self._db
        return clone
    ...
    async def connect(self, pooled=False):
        if pooled: self._connection = await self._db.connect()
        else: self._connection = await self._db.connect()
        self._pooled = pooled
    ...
    async def run(self, *args, **kwargs) -> Results:
        ...
        results = await self._db.executes(self)
        return Results(results)

    async def sql(self, statement: str):
        results = await self._db.raw(statement)
        return results
