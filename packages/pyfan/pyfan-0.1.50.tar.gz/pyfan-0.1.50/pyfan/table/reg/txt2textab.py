...
def tab_txt2tex(ls_st_txt_regs,
                ...
                dc_fmd_sym_sig=None, dc_tex_sig_sym=None):
    """Markov conditional transition probability check

    DECIMAL CONOVERSION:

    STAR CONVERSION:
        Assume that the table can come with 3 star, 2 stars or 1 star as superscripts next to numbers.
        These stars could be interpreted as representing different sig. levels.

    Parameters
    ----------
    it_col_count : int
        Number of latex table columns
    it_or_dc_round_decimal : `float`, optional
        Tolerance for the difference between 1 and each row sum
    dc_fmd_sym_sig : dict
        Translate between symbol in fmd for sig to numerical values. Keys are fmd symbols, values are sig.
    dc_tex_sig_sym : dict
        Translates between significance numbers and latex output. Keys are the sig, values are latex strings.
    ...
    if dc_fmd_sym_sig is None:
        # what do symbols in fmd mean, standard:
        dc_fmd_sym_sig = {'***': 0.001, '**': 0.01, '*': 0.05}
    if dc_tex_sig_sym is None:
        # what should symbols translate to
        dc_tex_sig_sym = {0.001: '\\sym{***}',
                          0.01: '\\sym{**}',
                          0.05: '\\sym{*}',
                          0.10: '\\dagger'}
    ...
    for st_txt_regq_line in ls_st_txt_regs:
        ...
        if st_N5_char == '###> ':
            ...
            for it_esti_ctr, st_esti in enumerate(ls_st_estimates):
                ...
                fl_sig_esti = np.nan
                for st_sym, fl_sig in dc_fmd_sym_sig.items():
                    if st_sym in st_esti:
                        st_esti = st_esti.replace(st_sym, "")
                        fl_sig_esti = fl_sig
                ...
                if it_bracket_count is np.nan:
                    ...
                    if fl_sig_esti is not np.nan:
                        st_esti_starred = st_esti_rounded + dc_tex_sig_sym[fl_sig_esti]
...
if __name__ == '__main__':
    ...
    # dc_fmd_sym_sig_xyl_202011 = {'***': 0.01, '**': 0.05, '*': 0.10}
    # dc_tex_sig_sym_demography = {0.001: '\\sym{***}',
    #                              0.01: '\\sym{**}',
    #                              0.05: '\\sym{*}',
    #                              0.10: '$^\\dagger$'}
    # kwargs_common = {'dc_fmd_sym_sig': dc_fmd_sym_sig_xyl_202011,
    #                  'dc_tex_sig_sym': dc_tex_sig_sym_demography}
    #
    ...
    #                     fl_adj_box_maxwidth=1.1, **kwargs_common)
    #
    ...
    # for st_rglob in ls_st_rglob:
    #     tab_txt2tex_f2f(spt_root=spt_root, st_rglob=st_rglob,
    #                     it_col_count=6, fl_col_label_width_cm=5.7,
    #                     **kwargs_common)
    #
    # ls_st_rglob = ['table_main3_fmd.md', 'table_app1_fmd.md']
    # for st_rglob in ls_st_rglob:
    #     tab_txt2tex_f2f(spt_root=spt_root, st_rglob=st_rglob,
    #                     it_col_count=5, fl_col_label_width_cm=5.7,
    #                     **kwargs_common)

    # ** kwargs_common
    spt_root = 'D:/Dropbox (UH-ECON)/Profile Paper/Paper Profile April 2016/tab/estimates/'
    ls_st_rglob = ['estimates_tmd.md']
    for st_rglob in ls_st_rglob:
        tab_txt2tex_f2f(spt_root=spt_root, st_rglob=st_rglob,
                        it_col_count=9, fl_col_label_width_cm=2,
                        it_or_dc_round_decimal=4)
