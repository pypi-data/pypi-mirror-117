...
It's possible to query available services (it's also possible to filter by
environment and/or tags). Example:
```python
import akerbp.mlops.cdf.helpers as cdf
cdf.set_up_cdf_client('deploy')
all_services = cdf.list_services()
test_services = cdf.list_services(env="test")
tag_services = cdf.list_services(tags["well_interpretation"])
```
Services can be deleted. Example:
```python
import akerbp.mlops.cdf.helpers as cdf
cdf.set_up_cdf_client('deploy')
