...
def delete_service(external_id, confirm=True):
    """
    Delete a service (cognite function) 

    Input:
        - external_id: (string) function name (external id), with the format
            'model_name-service-environment'. Use `list_services` to get an
            overview of deployed services
        - confirm: (bool) whether the user will be asked to confirm deletion
    """
    try:
        model, service, environment = external_id.split("-")
    except ValueError:
        m = f"Expected external id format: 'model-service-environment'"
        logging.error(m)
        return
    ...
    if not confirm or confirmed:
        ...
        try:
            ...
            logging.debug(f"Deleted function with {external_id}")
        except CogniteNotFoundError:
            logging.error(f"Couldn't find function with '{external_id=}'")
...
def list_services(tags=[], env=None):
    """
    List available services (i.e. CDF Functions), optionally filtering by 
    environment (dev, test or prod) or set of tags. 
    ...
    """ 
    def is_mlops_function(f):
        try:
            model,service,env = f.external_id.split("-")
            return True
        except ValueError:
            return False
    ...
    functions = filter(is_mlops_function, functions)
    if env:
