...
class SQLConnection:
    ...
    def update(self, data, update_list, on_list, schema, table, bool_cols=None, print_sql=False, batch_size=100,
               output=None, bools2bits=True, nullable=True):
                        ...
                        if nullable:
                            ...
                            if pd.notna(batch_update.loc[iindex, col]):
                                if isinstance(batch_update.loc[iindex, col], Number):
                                    ...
                                    sql_statement += " [%s] = '%s' ," % (col, batch_update.loc[iindex, col])
                            if pd.isna(batch_update.loc[iindex, col]) and col in ['bwg_blocked']:
                                if '_bwg_blocked_value' in batch_update.columns:
                                    if pd.isna(batch_update.loc[iindex, '_bwg_blocked_value']):
                                        sql_statement += " [_%s_value] = NULL ," % col
                                else:
                                    sql_statement += " [_%s_value] = NULL ," % col
                            if pd.isna(batch_update.loc[iindex, col]) and col in ['bwg_blockedaction']:
                                if '_bwg_blockedaction_value' in batch_update.columns:
                                    if pd.isna(batch_update.loc[iindex, '_bwg_blockedaction_value']):
                                        sql_statement += " [_%s_value] = NULL ," % col
                                else:
                                    sql_statement += " [_%s_value] = NULL ," % col
