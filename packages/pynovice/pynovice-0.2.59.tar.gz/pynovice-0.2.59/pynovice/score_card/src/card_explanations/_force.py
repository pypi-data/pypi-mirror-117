...
import warnings
import re
from .utils._legacy import convert_to_link, Instance, Model, Data, DenseData, Link, hclust_ordering, draw_additive_plot
...
def force(base_value, shap_values=None, features=None, feature_names=None, out_names=None, link="identity",
    ...
    if shap_values.shape[0] == 1:
        if feature_names is None:
            feature_names = ["Feature %s" % str(i) for i in range(shap_values.shape[1])]
        if features is None:
