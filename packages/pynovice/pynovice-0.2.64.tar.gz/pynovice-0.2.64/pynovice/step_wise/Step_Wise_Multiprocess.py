...
class Regression():
    def __init__(self, X:pd.DataFrame, Y, init_in_vars=[], model=None, sample_weight = None, max_related_level=1, iter_num=20,
                ...
                n_core=None, logger_info=True, logger_file=None, observation_model='normal'):
    ...
    def _check(self, in_vars, current_perf):
        ...
        perfs = np.max(perf_list) if self.is_check_overfit else np.mean(perf_list)
    ...
    def fit_observation(self,iter_idx,_in_vars,logger_ch):
        ...
        while (True):
            ...
            if add_var is not None:
                ...
                current_perf = perf
            if logger_ch:
                logger_ch.info('此轮迭代完成(全量观察)%d，入模变量为：%s。 当前模型性能%s为:%f' % (iter_idx, add_var, self.measure, current_perf))
