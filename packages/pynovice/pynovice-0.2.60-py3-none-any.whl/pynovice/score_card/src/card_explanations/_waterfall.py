...
try:
    ...
    pass
from .utils._utils import safe_isinstance, format_value
from pynovice.score_card.src.card_explanations.utils import _colors as colors
...
def waterfall(shap_values, max_display=10, show=True):
    ...
    if feature_names is None:
        feature_names = np.array(["Feature %s" % str(i) for i in range(len(values))])
...
def waterfall_legacy(base_value, shap_values=None, features=None, feature_names=None, max_display=10, show=True):
