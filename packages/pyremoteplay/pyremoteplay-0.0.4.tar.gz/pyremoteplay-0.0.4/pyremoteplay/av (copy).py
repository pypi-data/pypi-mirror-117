...
class ProcessReceiver(AVReceiver):
    ...
    def process(pipe, address, authkey, video_format="h264", use_hw=False):
        codec = AVReceiver.video_codec(video_format=video_format, use_hw=use_hw)
        manager = SharedMemoryManager(address=address, authkey=authkey)
        manager.connect()
        ...
        while True:
            ...
            if frame is None:
                continue
            plane = bytearray(frame.planes[0])
            out_shm = manager.SharedMemory(len(plane))
            out_shm.buf[:] = plane
            try:
                pipe.send((out_shm.name, frame.width, frame.height))
                out_shm.close()
            except BrokenPipeError:
                break
        codec.close()
    ...
    def read(self):
        _LOGGER.error("Recv")
        frame = self.pipe1.recv()
        self.v_queue.append(frame)
        event_emitter.emit("frame")

    def listener(self):
        while not self._session.is_stopped:
            if self.pipe1.poll(0.0001):
                frame = self.pipe1.recv()
                self.v_queue.append(frame)
                event_emitter.emit("frame")
            else:
    ...
    def start(self):
        ...
        self.manager = SharedMemoryManager(authkey=authkey)
        self.manager.start()
        self._worker = multiprocessing.Process(
            target=ProcessReceiver.process,
            args=(self.pipe2, self.manager.address, authkey, self._session.video_format, self._session.use_hw),
            daemon=True,
        )
        self._worker.start()
        _LOGGER.info("Process Start")
        # self._listener = threading.Thread(target=self.listener, daemon=True)
        # self._listener.start()
        asyncio.get_event_loop().add_reader(self.pipe1.fileno(), self.read)
        self.notify_started()

    def get_video_frame(self):
        frame = None
        shm_name, width, height = self.v_queue.popleft()
        shm = multiprocessing.shared_memory.SharedMemory(shm_name)
        frame = ProcessReceiver.FrameMinimal(bytes(shm.buf), width, height)
        shm.close()
        shm.unlink()
        return frame

    def handle_video(self, buf):
        shm = self.manager.SharedMemory(len(buf))
        shm.buf[:] = buf
        self.pipe1.send(shm.name)
        shm.close()
