...
class NevFile:
    ...
    def getdata(self, elec_ids="all", wave_read="read"):
        ...
        while self.datafile.tell() != ospath.getsize(self.datafile.name):
            ...
            if not (
                elec_ids == "all"
                or ((packet_id in elec_ids) and NEURAL_PACKET_ID_MIN <= packet_id <= NEURAL_PACKET_ID_MAX)
            ):
            ...
            # For neural waveforms, read classifier, skip one byte (reserved), and read waveform data
                ...
                if wave_read == "read":
                    if idx == -1:
                        ...
                        output["spike_events"]["Waveforms"][idx] = np.append(
            ...
            # For configuration log event
                ...
                output["configuration_events"]["ConfigChanged"].append(
                    unpack(("<" + str(samples) + "s"), self.datafile.read(samples))[0]
                )
    ...
    def __init__(self, datafile=""):
        ...
        if self.basic_header["FileTypeID"] == "NEURALSG":
            ...
            self.basic_header["ChannelID"] = list(
                np.fromfile(file=self.datafile, dtype=np.uint32, count=self.basic_header["ChannelCount"]).reshape(
                    shape
                )[0]
            )
        else:
    ...
    def getdata(self, elec_ids="all", start_time_s=0, data_time_s="all", downsample=1):
        ...
        if self.basic_header["FileSpec"] != "2.1":
            for i in range(num_elecs):
                idx = next(
                    item for (item, d) in enumerate(self.extended_headers) if d["ElectrodeID"] == output["elec_ids"][i]
                )
        ...
        while not hit_stop:
            ...
            if self.basic_header["FileSpec"] != "2.1":
                ...
                if output["data_headers"][-1]["NumDataPoints"] < downsample:
                    self.datafile.seek(
                        ...
                        * output["data_headers"][-1]["NumDataPoints"]
                        * DATA_BYTE_SIZE,
                        1,
                    )
                    continue
            ...
            curr_file_pos += (
                self.basic_header["ChannelCount"] * output["data_headers"][-1]["NumDataPoints"] * DATA_BYTE_SIZE
            )
        ...
        if self.basic_header["FileSpec"] == "2.1":
            ...
            if front_end_idxs:
                if front_end_idx_cont:
                    output["data"][front_end_idxs[0] : front_end_idxs[-1] + 1] *= getdigfactor(
                        self.extended_headers, output["ExtendedHeaderIndices"][front_end_idxs[0]]
                    )
                else:
            ...
            if analog_input_idxs:
                if analog_input_idx_cont:
                    output["data"][analog_input_idxs[0] : analog_input_idxs[-1] + 1] *= getdigfactor(
                        self.extended_headers, output["ExtendedHeaderIndices"][analog_input_idxs[0]]
                    )
                else:
