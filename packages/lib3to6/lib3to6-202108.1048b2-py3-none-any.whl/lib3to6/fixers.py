...
class _FRAFContext:
    ...
    def update_index_elts(self, elts: Elts) ->None:
        for i in range(len(elts)):
            ...
            if isinstance(elt, ast.Name):
                ...
                msg = (
                    f'Error fixing index element with forward ref of type {type(elt)}'
                    )
    ...
    def update_subscript(self, val: ast.Subscript) ->None:
        ...
        if isinstance(idx, ast.Tuple):
            ...
            msg = (
                f'Error fixing annotation of forward ref with type {type(idx)}'
                )
    ...
    def update_annotation_refs(self, node: AnnoNode, attrname: str) ->None:
        ...
        if isinstance(anno, ast.Name):
            ...
            msg = (
                f'Error fixing annotation of forward ref with type {type(anno)}'
                )
...
class ForwardReferenceAnnotationsFixer(fb.FixerBase):
    ...
    def apply_fix(self, ctx: common.BuildContext, tree: ast.Module
        ) ->ast.Module:
    ...
    def visit_FunctionDef(node: ast.FunctionDef) ->ast.FunctionDef:
        ...
        for arg, default in zip(kwonlyargs, kw_defaults):
            ...
            if default is None:
                ...
                msg = (
                    f'Keyword only arguments must be immutable. Found: {default} for {arg_name}'
                    )
    ...
    def visit_Name(self, node: ast.Name) ->typ.Union[ast.Name, ast.Attribute]:
        if isinstance(node.ctx, ast.Load) and node.id in ('map', 'zip',
            ...
            self.required_imports.add(common.ImportDecl('itertools', None,
                None))
            global_decl = (
                f"{node.id} = getattr(itertools, 'i{node.id}', {node.id})")
...
class NamedTupleClassToAssignFixer(fb.TransformerFixerBase):
    ...
    def visit_ClassDef(self, node: ast.ClassDef) ->typ.Union[ast.ClassDef,
        ast.Assign]:
        self.generic_visit(node)
        if len(node.bases) == 0:
            return node
