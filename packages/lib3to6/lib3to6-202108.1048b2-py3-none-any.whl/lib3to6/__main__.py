...
@click.option('--default-mode', default='enabled', metavar='<mode>', help=
    __DEFAULT_MODE_HELP.strip())
@click.argument('source_files', metavar='<source_file>', nargs=-1, type=
    click.File(mode='r'))
def main(target_version: str, diff: bool, in_place: bool, install_requires:
    typ.Optional[str], source_files: typ.Sequence[io.TextIOWrapper],
    default_mode: str='enabled', verbose: int=0) ->None:
