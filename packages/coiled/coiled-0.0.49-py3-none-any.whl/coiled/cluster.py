...
class Cluster(distributed.deploy.Cluster, Generic[IsAsynchronous]):
    ...
    def __init__(
        ...
        *,
        ...
        self._requested = set()
        self._plan = set()
        # TODO: Should we update `maximum` based on user limit
        # and cluster configuration? ie:
        # limit 200 cores and cluster configuration has 4 cpus per worker
        # maximum = 50
    ...
    async def _start(self):
        ...
        with console.status(
            ...
            if should_create is False:
                self._requested = self.observed
                self._plan = self.observed
    ...
    def adapt(self, Adaptive=CoiledAdaptive, **kwargs) -> Adaptive:
        raise NotImplementedError("Adaptive Scaling is currently under development")
