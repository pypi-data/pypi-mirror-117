...
class LazyMapped:
    """A lazily mapped sequence

    :param seq:
        Any object that exposes the sequence protocol
    :param mapper:
        A single or sequence of mapping functions. If multiple functions are
        passed, they are applied in turn. If not specified, the item of the
        sequence is returned as-is.

    To materialize the sequence, you can use for example list::

        mapped_seq = LazyMapped(range(4), lambda x: x ** 2)
        materialized = list(mapped_seq)

        assert materialized == [0, 1, 4, 9]

    """

    def __init__(self, seq, mapper: Union[callable, Sequence[callable]] = ()):
        if callable(mapper):
            mapper = [mapper]

        self._seq = seq
        self._mapper = list(mapper)

    def map(self, func):
        """Return a new mapped sequence with the additional mapping function"""
        return type(self)(self._seq, [*self._mapper, func])
    ...
    def __getitem__(self, idx):
        return self._map(self._seq[idx])

    def _map(self, item):
        for func in self._mapper:
            item = func(item)
...
def fix_categories(
    ...
    if not pd_types.is_categorical_dtype(s):
        if inplace:
...
def to_start_of_month(s):
    import pandas as pd

    s = pd.to_datetime({"year": s.dt.year, "month": s.dt.month, "day": 1})
    s = pd.Series(s)
    return s
