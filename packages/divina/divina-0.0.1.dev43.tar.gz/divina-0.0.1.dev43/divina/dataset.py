...
def get_dataset(vision_definition):
    ...
    df = dd.read_parquet(
        "{}/{}/data/*".format(
            vision_definition["dataset_directory"],
    ...
    if "joins" in vision_definition:
        for i, join in enumerate(vision_definition["joins"]):
            join_df = dd.read_parquet(
                "{}/{}/data/*".format(
                    join["dataset_directory"], join["dataset_id"]
                )
            )
            join_df.columns = [
            ...
            join_profile.columns = [
                "{}_{}".format(join["dataset_id"], c)
                for c in join_profile.columns
            ]
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                raise Exception(
                    "Could not parse data at path: {}".format(read_path)
                )
