...
class LinkInventModel(GenerativeModelBase):
    def __init__(self, vocabulary: PairedModelVocabulary, network: EncoderDecoder,
                 max_sequence_length: int = 256, no_cuda: bool = False, mode: str = "training"):
    ...
    @classmethod
    def load_from_file(cls, path_to_file, mode: str = 'training') -> Union[Any, GenerativeModelBase] :
        """
    ...
    def sample(self, inputs, input_seq_lengths) -> List[SampledSequencesDTO]:
        ...
        input_vector = torch.full((batch_size, 1), self.vocabulary.output.vocabulary["^"], dtype=torch.long).cuda() # (batch, 1)
        ...
        linker_smiles_list = [self.vocabulary.output.decode(seq) for seq in torch.cat(sequences, 1).data.cpu().numpy()]
