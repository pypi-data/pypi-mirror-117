...
from openlxp_xia.models import XIAConfiguration, MetadataFieldOverwrite
...
def get_metadata_fields_to_fill():
    xia_data = XIAConfiguration.objects.first()
    target_metadata_schema = xia_data.source_target_mapping
    mapping = read_json_data(target_metadata_schema)
    flatten_mapping_schema = dict_flatten(mapping, None)

    xia_data = XIAConfiguration.objects.first()
    target_validation_schema = xia_data.target_metadata_schema
    target = read_json_data(target_validation_schema)
    ...
    for item in required_target_list:
        if item in flatten_mapping_schema:
            MetadataFieldOverwrite.field_name = flatten_mapping_schema[item]
        else:
