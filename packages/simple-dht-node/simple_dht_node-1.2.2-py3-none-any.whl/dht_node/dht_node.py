...
class DHTNode:
    ...
    def __init__(
        ...
        self.blocked_ips: TTLCache = TTLCache(maxsize=1000, ttl=3600 * 24)
    ...
    def _process_messages(self):
        ...
        while not self._stop_requested.is_set():
            ...
            try:
                ...
                logging.debug("Cannot decode message: %r", msg_raw)
                continue
    ...
    def _on_find_node_response(self, message: dict, _node: Node):
        ...
        for node in utils.parse_compact_node_info(message[b"r"][b"nodes"]):
            # Skip invalid nodes
            if not utils.is_valid_node(node, self.id):  # pragma: no cover
                self.block_ip(node.ip)
                continue
    ...
    def _ping_questionable_nodes(self):  # pragma: no cover
    ...
    def block_ip(self, node_ip: str):
        """Block node by IP for 24 hours"""
        with self._lock:
            self.blocked_ips[node_ip] = datetime.now()
