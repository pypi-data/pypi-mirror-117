...
if dd.is_installed('vat'):
                ...
                if self.origin.lower_bound:
                    if self.origin.lower_bound > len(vn_dict['number']):
                        raise ValidationError(
                            _(f"VAT id must contain at least {self.origin.lower_bound} digits."))
                else:
                    raise ValidationError(
                        _(f"VAT id must contain {self.origin.length} digits."))

            if hasattr(vn_dict, 'country_code') and vn_dict['country_code'] != self.origin.country_code:
                raise ValidationError(
                    _(f"Country code ({self.origin.country_code}) does not match with {vn_dict['country_code']}."))
    ...
    class VatNumberManager:
        ...
        def set_origin(self):
            self.origin = self._vat_origins.get(self.country_code)

        def validate(self, vn_dict=None, local=True):
            if local:
                if not vn_dict:
                ...
                if result.is_valid is False:
                    msg = gettext("Invalid VAT identification number:\n")
                    logs = [l for l in result.log_lines if 'xml' not in l]
                    if not logs:
                        msg += gettext("Not found in the registry.")
                    else:
                        msg += "\n".join(logs)
                    raise ValidationError(msg)
...
if dd.is_installed('vat'):
    ...
    class VatIdChecker(Checker):
        ...
        def get_checkdata_problems(self, obj, fix=False):
            if obj.vat_id:
                try:
                    obj.vid_manager.validate(local=False)
