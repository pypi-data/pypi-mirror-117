...
class OMMItemStream(OMMStream):
    ...
    async def _do_open_async(self, with_updates=True):
        ...
        """
        self._session.debug(
            f"Open asynchronously OMMItemStream {self.stream_id} to {self._name}"
        )
        if self._name is None:
            ...
            if self.state is not StreamState.Closed:
                if self._callbacks.on_refresh:
                    try:
                        ...
                        self._session.log(
                            logging.ERROR,
                            f"OMMItemStream on_refresh callback raised exception: {e!r}",
                        )
        ...
        with self.__item_stream_lock:
            ...
            if self._callbacks.on_status:
                try:
                    ...
                    self._session.log(
                        logging.ERROR,
                        f"OMMItemStream on_status callback raised exception: {e!r}",
                    )
                    self._session.log(1, f"{traceback.format_exc()}")
            ...
            if self.state is not StreamState.Closed:
                if self._callbacks.on_update:
                    try:
                        ...
                        self._session.log(
                            logging.ERROR,
                            f"OMMItemStream on_update callback raised exception: {e!r}",
                        )
                        self._session.log(1, f"{traceback.format_exc()}")
        ...
        with self.__item_stream_lock:
            ...
            if self.state is not StreamState.Closed:
                if self._callbacks.on_complete:
                    try:
                        self._session.log(
                            1, "OMMItemStream : call on_complete callback"
                        )
                        self._callbacks.on_complete(self)
                    except Exception as e:
                        self._session.log(
                            logging.ERROR,
                            f"OMMItemStream on_complete callback raised exception: {e!r}",
                        )
                        self._session.log(1, f"{traceback.format_exc()}")
            ...
            if self.state is not StreamState.Closed:
                ...
                if self._callbacks.on_error:
                    try:
                        ...
                        self._session.log(
                            logging.ERROR,
                            f"OMMItemStream on_error callback raised an exception: {e!r}",
                        )
                        self._session.log(1, f"{traceback.format_exc()}")
