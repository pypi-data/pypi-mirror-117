...
class TwoToneSpectroscopy:
    ...
    def __init__(self, *, x_min, x_max, x_step, y_min, y_max, y_step):
        ...
        self.heatt = np.zeros(len(self.x_list) * len(self.y_list))
        self.heatt[:] = np.nan
        
        self.xxx = np.zeros(len(self.x_list) * len(self.y_list))
        self.yyy = np.zeros(len(self.x_list) * len(self.y_list))
    ...
    def get_result(self, *, imshow: bool = False) -> pd.DataFrame:
        ...
        for i in range(len(self.__x_raw)):
            heat_list[int((self.xxx[i] - self.x_min) // self.x_step)][int((self.yyy[i] - self.y_min) // self.y_step)] = self.heatt[i]
