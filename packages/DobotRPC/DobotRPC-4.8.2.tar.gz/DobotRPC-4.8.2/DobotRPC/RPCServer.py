...
class RPCServer(object):
    ...
    async def __call_worker(self, websocket, rpc_id, method, **params) -> None:
        ...
        remote_address = "%s:%s" % (
            str(websocket.remote_address[0]), str(websocket.remote_address[1]))

        # todo: 暂时使用, 不应该在这里解释数据
        try:
            json_data = json.loads(feedback)
            if "params" in json_data and len(json_data["params"]) > 100:
                json_data["params"] = "..."
    ...
    async def __on_new_onnection(self, websocket: WebSocketServerProtocol,
        ...
        if ip in self.__ban_list:
            ...
            loggers.get(LOGGER_NAME).debug(
                "S(%s) >> C(%s): %s" % (str(address), str(remote_address), str(send_str)))
            await websocket.send(send_str)
            return

        loggers.get(LOGGER_NAME).info(
            "Client(%s:%s) connected." % (str(ip), str(port)))
        ...
        while True:
            ...
            try:
                ...
                loggers.get(LOGGER_NAME).error(e, exc_info=True)
                loggers.get(LOGGER_NAME).debug(
                    "S(%s) >> C(%s): %s" % (str(address), str(remote_address), str(e)))
                await websocket.send(str(e))
                continue

        loggers.get(LOGGER_NAME).warning(
            "%s:%s had disconnected!!!!!!!!" % (str(ip), str(port)))
