...
def firewall(stem, fw_sn_id, fwm_sn_id, private_ranges, depends_on=None):
    fw_pip = network.PublicIPAddress(
        f'{preffix}{s}{stem}{s}fwpubip',
        public_ip_address_name=f'{preffix}{s}{stem}{s}fwpubip',
    ...
    fwm_pip = network.PublicIPAddress(
        f'{preffix}{s}{stem}{s}fwmpubip',
        public_ip_address_name=f'{preffix}{s}{stem}{s}fwmpubip',
        resource_group_name=resource_group_name,
        sku=network.PublicIPAddressSkuArgs(
            name=network.PublicIPAddressSkuName.STANDARD,
        ),
    ...
    fw = network.AzureFirewall(
        f'{preffix}{s}{stem}{s}firewall',
        azure_firewall_name=f'{preffix}{s}{stem}{s}fwmpubip',
        resource_group_name=resource_group_name,
        ...
        ip_configurations=[network.AzureFirewallIPConfigurationArgs(
            name=f'{preffix}{s}{stem}{s}fwipconfig',
        ...
        management_ip_configuration=network.AzureFirewallIPConfigurationArgs(
            name=f'{preffix}{s}{stem}{s}fwmipconfig',
            public_ip_address=network.PublicIPAddressArgs(
                id=fwm_pip.id,
            ),
            subnet=network.SubnetArgs(
