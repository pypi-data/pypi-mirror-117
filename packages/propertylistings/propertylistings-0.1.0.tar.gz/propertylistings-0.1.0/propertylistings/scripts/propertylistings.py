import click, csv
from propertylistings.archive import logger
from propertylistings.resources import regions, region_options
...
@click.option(
        ...
        default=None,
        type=int,
        help='Provide the region code to search, if it is currently not documented in the propertylistings (--region) selection.'
        )
@pass_config
def cli(config, save_file, filepath, log, add_region):
...
def search(
    ...
    '''
    Scrape RightMove for-sale property data property listings.
    '''

    if config.add_region is None: region = regions[region]
    if config.add_region: region = str(config.add_region)

    scrape = PropertiesWebTool(
            pages,
            region,
            min_price,
            max_price,
    ...
    property_listings = scrape.generate_data()

    # Qualify if save-file option has been provided, otherwise echo gathered dataset.
    if config.save_file:

        if config.filepath is None: config.filepath = './scraped_data.csv'

        with open(file=config.filepath, mode="w", newline="") as f:
            write = csv.writer(f)
            write.writerows(property_listings)

    else:
        click.echo(property_listings)
