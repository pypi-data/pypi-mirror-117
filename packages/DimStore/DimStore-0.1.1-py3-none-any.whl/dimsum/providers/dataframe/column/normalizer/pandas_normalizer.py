...
class PandasNormalizer():
    ...
    def qualify_column(cls,df,feature,**kwargs):
        ...
        """
        # check edge case with duck typing
        if not hasattr(df, 'columns'):
            raise TypeError('> PandasNormalizer.qualify_column(): the input dataframe is not an instance of pandas dataframe.')
        if not hasattr(feature, 'namespace'):
            raise TypeError('> PandasNormalizer.qualify_column(): the feature object is not an instance of FeatureMetaData class.')
