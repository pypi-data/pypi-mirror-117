...
class FeatureSet():
    ...
    def update(self, key_values=None, updater=None, strict_mode=True, verbose=True, **kwargs):
        ...
        # validate udf
        if not self.__is_valid_ufd__(ufd):
            print('> The feature name must be unique in the specified namespace.')
            return 
    ...
    """
    "
    " check if the udf is valid, ie. duplication in each name space
    " O(n) time and space complexity
    "
    """
    def __is_valid_ufd__(self, udf):
        if not hasattr(udf, 'items'):
            raise TypeError('> the udf instance is not in valid type.')
        lookup = {}
        for _, feature in udf.items():
            if feature.namespace not in lookup:
                lookup[feature.namespace] = set([])
            if feature.name in lookup[feature.namespace]:
                return False
            lookup[feature.namespace].add(feature.name)
        return True
