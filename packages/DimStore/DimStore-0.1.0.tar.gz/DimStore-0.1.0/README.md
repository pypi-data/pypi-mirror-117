# DimSum()
DimSum() is a lightweight __feature store__ designed to streamline the DS working pipeline by sharing high-quality features among team members and enable feature reuse with minimal effort. It designs to be extensible and versatile to accommodate different teams set up on different computation platform.

# Design

<img style="float: center;" src="docs/diagrams/dimsum_design_diagram.jpg">  

##### Intro 
Dimsum is built for sharing features in pandas or pyspark dataframes; it could be extended to any arbitrary dataset format. The core idea is to share the feature extraction logic (_pipeline_) instead of the actual dataset. The _provider_ pattern enables Dimsum to work with different back-end layers such as object storage, IBM knowledge catalog, etc. 
...
"meta_manager_providers": {
    "flat_file_meta_manager":{
            "root_dir": "/Users/kai/repository/dimsum/example/storage",
            "folder_name":"catalog",
...
# Dev Enviroment
how to setup the devlopment enviroment (more content will be available)
