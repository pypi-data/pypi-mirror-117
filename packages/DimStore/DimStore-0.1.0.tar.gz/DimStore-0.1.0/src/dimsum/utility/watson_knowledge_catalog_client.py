...
"""
"
" The client interact with watson Kownledge Catalog using watson data API(beta)
...
class WatsonKnowledgeCatalogClient():
    ...
    def __init__(self, catalog_name, host, uid=None, pwd=None, token=None, verbose=True):
        ...
        # retrieve auth token
        if token == None:
            self.token = self.__get_auth_token__(uid,pwd)
        else:
            self.token = token        
        ...
        # debug info
        if verbose:
            print('Initialize watson Knowledge Catalog: sucessfully!')
    ...
    def create_asset(self, metadata):
        ...
        return response from the watson Data API
        """
        if metadata == None:
    ...
    "
    """
    def __get_auth_token__(self, uid, pwd):
        """
        @param::uid: username
        @param::pwd: password
        ...
        token = self.__jsonify__(data)['token']
        return token
            
        
    def __get_catalogs__(self):
        """
