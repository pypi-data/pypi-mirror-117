from dimsum.providers.dataframe.jointer.jointer_base import JointerBase
...
class PySparkJointer(JointerBase):
    ...
    def try_join(cls, left_df, right_df, left_idx, right_idx, **kwargs):
        ...
        """
        # check edge case
        if not hasattr(left_df, 'withColumnRenamed') or not hasattr(left_df, 'join'):
            raise TypeError('> PySparkJointer try_join(): left dataframe is not a instance of pyspark.sql.dataframe.DataFrame class.')
        if not hasattr(right_df, 'withColumnRenamed') or not hasattr(right_df, 'join'):
            raise TypeError('> PySparkJointer try_join(): left dataframe is not a instance of pyspark.sql.dataframe.DataFrame class.')
