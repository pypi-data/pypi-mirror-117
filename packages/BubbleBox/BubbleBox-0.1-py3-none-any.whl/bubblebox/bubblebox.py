...
def lj_force(eps, sig, rr):
    return -12*eps*(sig**12*rr**-7 - sig**6*rr**-4)
...
class box():
    ...
    def advance_vverlet(self):
        ...
        """
        #Fn = self.forces()
        Fn = self.forces(self.pos, self.interactions, self.L2x, self.L2y, self.r2_cut, self.force)
        ...
        self.vel_ = self.vel_ + .5*(self.forces(pos_new, self.interactions, self.L2x, self.L2y, self.r2_cut, self.force) + Fn)*self.dt*self.masses_inv
