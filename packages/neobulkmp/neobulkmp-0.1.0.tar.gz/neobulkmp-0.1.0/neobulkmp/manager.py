...
class Manager:
    ...
    def log_status(self):
        ...
        sourcing_worker_finished_count = len(
            self.manager_sourcing._get_workers(progress="finished")
        )
        ...
        waiting_rels_worker_count = len(
            self.manager_loading._get_workers(
                status="drain_ordered", tag=self.manager_loading.worker_tag_relsets
            )
        )
...
class ManagerWorkersSourcing(ManagerWorkersBase):
    ...
    def is_done(self):
        ...
        if len(self.workers) == len(self._get_workers(status="finished")):
            return True
        else:
            return False
        ...
        if len(cached_sets_worked_on) < assigned_no_of_cores:
            ...
            if cached_sets_not_worked_on:
                ...
                for next_set in cached_sets_not_worked_on:
                    ...
                    current_ns_labels = {
                        label
                        for cs in cached_sets_worked_on
