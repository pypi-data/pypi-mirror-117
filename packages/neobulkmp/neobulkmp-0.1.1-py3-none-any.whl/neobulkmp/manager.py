...
class Progress(str, Enum):
    # str mix in, to be able to compare enums to strings. cool trick! see https://stackoverflow.com/a/63028809/12438690
    STARTED = "start"
    DRAIN_ORDERED = "drain"
    DRAIN_READY = "drained"
    LOADING = "load"
...
class ManagerWorkersLoading(ManagerWorkersBase):
    ...
    def manage_nodeset_loading(self, assigned_no_of_cores):
        ...
        cached_sets_worked_on: List[SetsMetaBase] = [
            ...
            for worker in self._get_workers(
                progress=(Progress.STARTED, Progress.LOADING),
        ...
        if no_of_free_cores > 0:
            ...
            for next_worker in [
                ...
                for w in self._get_workers(
                    progress=Progress.STARTED, tag=self.worker_tag_nodesets
                )
            ]:
    ...
    def manage_relation_loading(self, assigned_no_of_cores):
        log.debug(
            ...
            + f" and {len(self._get_workers('running',tag=self.worker_tag_relsets))} worker running atm."
            + f" {len(self._get_workers(progress='drain_ordered', tag=self.worker_tag_relsets))} workers are waiting for a drain"
        )
        log.debug(
            f"DRAINING WAIT: {self._get_workers(progress='drain_ordered', tag=self.worker_tag_relsets)}"
        )
        ...
        log.debug(f"relsets free cores: {no_of_free_cores}")

        # START initialized and drained workers
        new_worker_started = False
        if no_of_free_cores > 0:
            # start one new worker
    ...
    def _order_drains(self, assigned_no_of_cores):
        ...
        next_relset_workers = [
            w
            for w in self._get_workers(
                progress=Progress.STARTED, tag=self.worker_tag_relsets
            )
        ]
