    ...
    def open_instrument_gpib(self, gpib_address):
        if self.test_except:
            ...
            if checker.find("ANRITSU") != -1:
                ...
                self.hp = False
                self.thandar = False
            elif checker.find("Hewlett-Packard") != -1:
                self.anritsu = False
                self.agilent = False
                self.hp = True
                self.thandar = False
    ...
    def read_raw_instrument(self):
        if self.test_except:
            ...
            if self.opened:
                ...
                pg.alert(text="Read Raw Error",
                         title="Error",
                         button="확인")
                return
            return self.com_inst.query_binary_values("CURV?", datatype="d", is_big_endian=True)

    def call_read_raw(self):
        if self.test_except:
            pass
        else:
            if self.opened:
                ...
                pg.alert(text="Call Read Raw Error",
                         title="Error",
                         button="확인")
                return
            self.com_inst.write("CURV?")

    def get_after_call_read_raw(self):
        if self.test_except:
            pass
        else:
            if self.opened:
...
class Source(Instrument):
    ...
    def set_imd_config(self, tone_num=2, freq_spacing=1000000):
        if self.agilent is True:
            pass
        elif self.anritsu is True:
            pass
...
class Spectrum(Instrument):
    def __init__(self):
        Instrument.__init__(self)
        self.instrument_name = "Spectrum"

    def set_frequency_span(self, freq_span):
        if self.agilent is True:
            print("agilent not done")
        if self.anritsu is True:
            print("anritsu not done")
        if self.hp is True:
            self.write_instrument(command="FREQ:SPAN {0}".format(freq_span))
    ...
    def get_frequency_span(self):
        ...
        if self.anritsu is True:
            print("anritsu not done")
        if self.hp is True:
            return self.query_instrument(command="FREQ:SPAN?")

    def set_video_bandwidth(self, video_bw):
        if self.agilent is True:
            print("agilent not done")
        if self.anritsu is True:
            print("anritsu not done")
        if self.hp is True:
            self.write_instrument(command="BAND:VID {0}".format(video_bw))
    ...
    def get_video_bandwidth(self):
        ...
        if self.anritsu is True:
            print("anritsu not done")
        if self.hp is True:
            return self.query_instrument(command="BAND:VID?")

    def set_res_bandwidth(self, res_bw):
        if self.agilent is True:
            print("agilent not done")
        if self.anritsu is True:
            print("anritsu not done")
        if self.hp is True:
            self.write_instrument(command="BWID {0}".format(res_bw))
    ...
    def get_res_bandwidth(self):
        ...
        if self.anritsu is True:
            print("anritsu not done")
        if self.hp is True:
            return self.query_instrument(command="BWID?")

    def set_center_freq(self, center_freq=None, up=False, down=False):
        if self.agilent is True:
            print("agilent not done")
        if self.anritsu is True:
            print("anritsu not done")
        if self.hp is True:
            if center_freq is not None:
                self.write_instrument(command="FREQ:CENT {0}".format(center_freq))
            else:
                if up is True:
                    self.write_instrument(command="FREQ:CENT UP")
                if down is True:
                    self.write_instrument(command="FREQ:CENT DOWN")
    ...
    def get_center_freq(self):
        ...
        if self.anritsu is True:
            print("anritsu not done")
        if self.hp is True:
            return self.query_instrument(command="FREQ:CENT?")

    def set_center_freq_step(self, step_freq):
        if self.agilent is True:
            print("agilent not done")
        if self.anritsu is True:
            print("anritsu not done")
        if self.hp is True:
            self.write_instrument(command="FREQ:CENT:STEP {0}".format(step_freq))
    ...
    def get_center_freq_step(self):
        ...
        if self.anritsu is True:
            print("anritsu not done")
        if self.hp is True:
            return self.query_instrument(command="FREQ:CENT:STEP?")

    def set_rel_level_offset(self, rel_offset):
        ...
        if self.anritsu is True:
            print("anritsu not done")
        if self.hp is True:
            self.write_instrument(command="DISP:WIND:TRAC:Y:RLEV:OFFS {0}".format(rel_offset))

    def get_rel_level_offset(self):
        if self.agilent is True:
            print("agilent not done")
        if self.anritsu is True:
            print("anritsu not done")
        if self.hp is True:
            return self.query_instrument(command="RLEV:OFFS?")

    def set_marker_freq(self, marker_num=1, freq=None):
        if self.agilent is True:
            print("agilent not done")
        if self.anritsu is True:
            print("anritsu not done")
        if self.hp is True:
            if freq is not None:
                self.write_instrument(command="CALC:MARK{0}:X {1}".format(marker_num, freq))
            else:
                print("set market freq error")

    def get_marker_freq(self, marker_num=1):
        if self.agilent is True:
            print("agilent not done")
        if self.anritsu is True:
            print("anritsu not done")
        if self.hp is True:
            return self.write_instrument(command="CALC:MARK{0}:X?".format(marker_num))

    def set_marker_center(self, marker_num=1):
        if self.agilent is True:
            print("agilent not done")
        if self.anritsu is True:
            print("anritsu not done")
        if self.hp is True:
            self.write_instrument(command="CALC:MARK{0}:SET:CENT".format(marker_num))

    def set_marker_state(self, state, marker_num=1):
        if self.agilent is True:
            print("agilent not done")
        if self.anritsu is True:
            print("anritsu not done")
        if self.hp is True:
            if state is True:  # marker on
                self.write_instrument(command="CALC:MARK{0}:STAT ON".format(marker_num))
            else:  # marker off
                self.write_instrument(command="CALC:MARK{0}:STAT OFF".format(marker_num))

    def get_marker_state(self, marker_num=1):
        if self.agilent is True:
            print("agilent not done")
        if self.anritsu is True:
            print("anritsu not done")
        if self.hp is True:
            return self.query_instrument(command="CALC:MARK{0}:STAT?".format(marker_num))

    def set_marker_all_off(self):
        if self.agilent is True:
            print("agilent not done")
        if self.anritsu is True:
            print("anritsu not done")
        if self.hp is True:
            self.write_instrument(command="CALC:MARK:AOFF")

    def set_marker_peak_search(self, marker_num=1, left=False, right=False, __next=False):
        if self.agilent is True:
            print("agilent not done")
        if self.anritsu is True:
            print("anritsu not done")
        if self.hp is True:
            if left is False and right is False and __next is False:
                self.write_instrument(command="CALC:MARK{0}:MAX".format(marker_num))
            else:
                if left is True:
                    self.write_instrument(command="CALC:MARK{0}:MAX:LEFT".format(marker_num))
                    return
                if right is True:
                    self.write_instrument(command="CALC:MARK{0}:MAX:RIGH".format(marker_num))
                    return
                if __next is True:
                    self.write_instrument(command="CALC:MARK{0}:MAX:NEXT".format(marker_num))
                    return

    def set_marker_mode(self, mode, marker_num=1):
        if self.agilent is True:
            print("agilent not done")
        if self.anritsu is True:
            print("anritsu not done")
        if self.hp is True:
            if mode == "POS":
                self.write_instrument(command="CALC:MARK{0}:MODE POS".format(marker_num))
            elif mode == "DELT":
                self.write_instrument(command="CALC:MARK{0}:MODE DELT".format(marker_num))
            elif mode == "BAND":
                self.write_instrument(command="CALC:MARK{0}:MODE BAND".format(marker_num))
            elif mode == "SPAN":
                self.write_instrument(command="CALC:MARK{0}:MODE SPAN".format(marker_num))

    def get_marker_mode(self, marker_num=1):
        if self.agilent is True:
            print("agilent not done")
        if self.anritsu is True:
            print("anritsu not done")
        if self.hp is True:
            return self.query_instrument(command="CALC:MARK{0}:MODE?".format(marker_num))

    def set_marker_move_according_center_freq(self, marker_num=1):
        if self.agilent is True:
            print("agilent not done")
        if self.anritsu is True:
            print("anritsu not done")
        if self.hp is True:
            self.write_instrument(command="CALC:MARK{0}:SET:STEP".format(marker_num))

    def get_read_marker(self, marker_num=1, x=False, y=False):
        if self.agilent is True:
            print("agilent not done")
        if self.anritsu is True:
            print("anritsu not done")
        if self.hp is True:
            if x is True:
                return self.query_instrument(command="CALC:MARK{0}:X?".format(marker_num))
            if y is True:
                return self.query_instrument(command="CALC:MARK{0}:Y?".format(marker_num))
...
class Network(Instrument):
    ...
    def get_cal_para_name(self, cal_num=1):
        if self.agilent is True:
            return self.query_instrument("CALC{0}:PAR:CAT?".format(cal_num))
        else:
            return "not done except agilent network"

    def reset(self, full_reset=False):
        if self.agilent is True:
            if full_reset is True:
                self.write_instrument(command="SYST:FPRESET")
            else:
                self.write_instrument(command="SYST:PRESET")
        else:
            return "not done except agilent network"

    def get_s_data(self, mem=False):
        if self.agilent is True:
            if mem is False:
                return self.query_instrument("CALC:DATA? SDATA")
            else:
                return self.query_instrument("CALC:DATA? SMEM")
        else:
            return "unknown_0"
...
# if __name__ == "__main__":
    # network = Network()
    # network.open_instrument_gpib(16)
    # print(network.query_instrument("*IDN?"))

    # test = Spectrum()
    # test.open_instrument_gpib(18)
    #
    # test.set_frequency_span(freq_span=5000000)
    # print(test.get_frequency_span())
    # test.set_res_bandwidth(res_bw=3000)
    # print(test.get_res_bandwidth())
    # test.set_video_bandwidth(video_bw=3000000)
    # print(test.get_video_bandwidth())
    #
    # test.set_center_freq(center_freq=2000000000)
    # test.set_center_freq_step(step_freq=2000000000)
    # # test.set_center_freq(up=True)
    # test.set_marker_state(state=True, marker_num=1)
    # print(test.get_marker_state())
    # test.set_marker_center(marker_num=1)
