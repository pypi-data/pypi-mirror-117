...
#         stream = pkg_resources.resource_stream(__name__, 'data/filename.txt')
        my_resources = importlib_resources.files("agri-tech")
        data = (my_resources / "data" / "filename.txt").read_bytes()
       
        # Define function to bind value of the input to the output variable 
        def f(x):
            self.output_slider_variable.value = str(x)

        dropdown = interact(f, x=read_text(data))
