    ...
    async def request(self, route: str, meth: str, body: typing.Any = None, *, is_json: bool = False, reason_header: str = None, retry: int = None, **kwargs) -> dict:
        ...
        for x in range(retry):
            ...
            if 200 <= code < 300:
                ...
                raise _exception.BadRequest(route, code, resp)
            elif code == 403:
                raise _exception.Forbidden(route, code, resp)
            elif code == 404:
                raise _exception.NotFound(route, code, resp)
            elif code == 429:
                wait_sec = resp["retry_after"]
                await asyncio.sleep(wait_sec)
                continue
            elif 500 <= code < 600:
                raise _exception.DiscordError(code)
            else:
                raise _exception.Unknown(code)
        raise _exception.RateLimited(route, code, resp)
                ...
                if res["op"] == Opcodes.DISPATCH:
                    if res["t"] == "READY":
                        self.session_id = res["d"].get(
                            "session_id", self.session_id)
                        if "ON_READY".lower() in _cmd.event_dict.keys():
                            func = _cmd.event_dict["on_ready"]
                            await func()
                    elif res["t"] == "INTERACTION_CREATE":
                        if "INTERACTION_CREATE".lower() in _cmd.event_dict.keys():
                            func = _cmd.event_dict["interaction_create"]
                            e = _models.Interaction(**res["d"])
                            await func(e)  # TODO: call function with info
                    elif res["t"] == "MESSAGE_CREATE":
                        if "MESSAGE_CREATE".lower() in _cmd.event_dict.keys():
                            func = _cmd.event_dict["message_create"]
                            e = _models.Message(**res["d"])
                            await func(e)
                        else:
                            e = _models.Message(**res["d"])
                            for i in _cmd.command_dict.keys():
                                if str(e.content) == i:
                                    func = _cmd.command_dict[e.content]
                                    await func(e)
                                elif str(e.content).startswith(i + ' '):
                                    func = _cmd.command_dict[i]
                                    await func(e)
                    else:
                        if res["t"].lower() in _cmd.event_dict.keys():
                            func = _cmd.event_dict[res["t"]]
                            await func(**res["d"])
