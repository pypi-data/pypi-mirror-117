...
def get_current_user():
    return _models.User(**_requests.get('/users/@me').json())


def get_user(id_):  # TODO: Check if ** works or not
    id_ = str(id_)
    return _models.User(**_requests.get(f'users/{id_}').json())


def modify_current_user(username=None, avatar=None):
    return _models.User(**_requests.patch('users/@me', {'username': username, 'avatar': avatar}).json())


def get_current_user_guilds():  # Change to _models.Guild() when finished
    return _requests.get('/users/@me/guilds').json()


def leave_guild(id_):
    return _requests.delete(f'/users/@me/guilds/{id_}').json()


def create_dm(id_):
    dm_channel = _requests.post(
        f'/users/@me/channels', {'recipient_id': id_}).json()
    return dm_channel


def create_group_dm(id_, acc_token: list, nicks: dict):
    group_channel = _requests.post(
        f'/users/@me/channels', {'access_tokens': acc_token, 'nicks': nicks})
    return group_channel


def get_user_connections():
    return _requests.get('/users/@me/connections')


def activity_type_converter(act):
    if isinstance(act, _models.Activity):
        return {
...
def get_channel(channel_id=None):
    return Channel(**_requests.get(f"/channels/{channel_id}").json())
