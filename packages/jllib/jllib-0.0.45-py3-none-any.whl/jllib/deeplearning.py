...
class AutoAuthenticator(AutoEvaluator):
    ...
    def fit(self) -> None:
        ...
        for i in range(self.nb_classes):
            ...
            history = model.fit(
                ...
                epochs=25,
                callbacks=[
                    tensorflow.keras.callbacks.EarlyStopping(monitor="mae",
                                                             patience=25,
                                                             mode="min",
            ...
            for pred, x in zip(preds, x_train):
                mae_list.append(self._get_mae(pred, x))
    ...
    def _determine_distances_to_cluster_border(self, sample: np.ndarray):
        ...
        for i in range(self.nb_classes):
            ...
            pred_mae = self._get_mae(pred, sample)

            distance = pred_mae - class_threshold
    ...
    def _check_sample_against_all_autoencoders(self, sample: np.ndarray, t: int = 0, impl: int = 1):
        ...
        for i in range(self.nb_classes):
            ...
            if pred_mae + t <= class_threshold:
                possible_matches.append({'class': i, 'pred_mae': pred_mae, 'class_threshold': class_threshold})
        ...
        if len(possible_matches) == 0:
            ...
            # impl == 1: sort by mae and return first element with smallest error.
            # impl == 2: sort by maximum distance between pred_mae and class_threshold
            assert impl in {1, 2}
            if impl == 1:
                possible_matches = sorted(possible_matches, key=lambda k: k['pred_mae'])
            elif impl == 2:
                possible_matches = sorted(possible_matches, key=lambda k: abs(k['class_threshold'] - k['pred_mae']),
                                          reverse=True)
            return True, possible_matches[0]['class'], possible_matches
    ...
    def evaluate(self, x_val=None, y_val=None):
        ...
        possible_thresholds = Parallel(n_jobs=os.cpu_count())(delayed(self._determine_distances_to_cluster_border)(x_val_sample) for x_val_sample in x_val)
        ...
        results_lst = Parallel(n_jobs=os.cpu_count())(delayed(self._do_eval)(t, x_val, y_val) for t in possible_thresholds)
        return results_lst
