...
def buy(isin, order_count, order_price, timeout=None):
    response = None
    try:
        ...
        response = utils.request(method="POST", url=url, headers=headers, data=payload, timeout=timeout)
        if response.status_code == 200:
            response = utils.convert_json(response)
            logger.info(
                f"buy request sent successfully, {isin}, order price {order_price}, order count {order_count}: "
                f"{response}")
            return response['IsSuccessfull']
        else:
            logger.warning(
                f"buy request failed, {isin}, order price {order_price}, order count {order_count}: "
                f"status_code:{response.status_code}, response_content={response.text}")
            return False

    except Exception:
        logger.error(
            f"error during sell {isin}, order price {order_price}, order count {order_count}, response={response.text if response is not None else None}"
            f" {response.status_code if response is not None else None}",
            exc_info=True)
        raise


def sell(isin, order_count, order_price, timeout=None):
    response = None
    try:
        url = "https://api3.mobinsb.ir/web/v1/Order/Post"
        ...
        response = utils.request(method="POST", url=url, headers=headers, data=payload, timeout=timeout)
        if response.status_code == 200:  # 200 series mean ok!
            response = utils.convert_json(response)
            logger.info(
                f"sell request sent successfully, {isin}, order price {order_price}, order count {order_count}: {response}")
            return response['IsSuccessfull']
        else:
            logger.warning(
                f"sell request failed, {isin}, order price {order_price}, order count {order_count}: "
                f"status_code:{response.status_code}, response_content={response.text}")
            return False
    except Exception:
        logger.error(
...
def get_detail(isin):
    ...
    response = utils.request(method="GET", url=url)
    response = utils.convert_json(response)
    data = {'symbol': response['symbolinfo']['ect'], 'company': response['symbolinfo']['est'],
            'isin': response['symbolinfo']['nc'],
...
def open_order():
    ...
    for item in response['Data']:
        data = {'symbol': item['symbol'], 'company': item['cn'], 'isin': item['nsccode'], 'time': item['time'],
                'date': item['dtime'], 'order price': item['orderprice'], 'orderid': item['orderid'],
