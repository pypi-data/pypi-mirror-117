...
class OAuth2(object):
    ...
    def redirect_url(self, request: Request, redirect_uri: str):
        base = urljoin(str(request.url), 'auth')
        return f"{base}?{urlencode({'redirect_uri': redirect_uri})}"
    ...
    def bind(self, router: APIRouter, verify: Callable[[Request, Any, str], Any]):
        @router.get("/login")
        def login(request: Request, redirect_uri: str = ""):
            redirect_uri = (
                redirect_uri
                if redirect_uri
                else self.redirect_url(request, redirect_uri)
            )
