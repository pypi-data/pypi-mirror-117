...
log_file = os.path.join(TEMP_FOLDER, 'socket_first.txt')
svars = variables()
...
class Socket:
    ...
    def check_alarm(self, data):
        if data == 'alarm_on':
            svars[f'{self.base_name}off'] = True
        elif data == 'alarm_off':
            svars[f'{self.base_name}off'] = False
