...
class Graph(BasicGraph):
	...
	def render(
		...
		if path is None:
			...
			to_save = self.get_graphviz_source(format=output_format)
			to_save.render(filename=filename, view=view)
			graphviz_str_to_display = self.get_graphviz_source(direction=direction, pad=pad, dpi=None)
	...
	def get_graphviz_graph(self, direction=None, dpi=300, height=None, width=None, pad=None, output_format=None):
		"""
		:type direction: NoneType or str
		:type dpi: NoneType or int
		:type height: NoneType of int or float
		...
		:type pad: NoneType or int or float
		:type output_format: str or NoneType
		:rtype: GraphvizGraph or Digraph
		"""
		source = self.get_graphviz_source(
			direction=direction, dpi=dpi, height=height, width=width, pad=pad, output_format=output_format
		)
		lst = str(source).splitlines()
		has_comment = (lst[0].find('//') != -1)
		is_direct_graph = False
		skip_index = 0

		if has_comment:
			skip_index = 1
			if lst[skip_index].find('graph {') != -1:
				is_direct_graph = False
		else:
		...
		for i in range(len(lst)):
			if ((i > skip_index) and (i != len(lst) - 1)):
				if has_comment:
					graph.comment = lst[0].replace('//', '')
				graph.body.append(lst[i])
		return graph
