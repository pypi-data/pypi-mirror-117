...
class ESClient(BaseArgsParsing):
                    ...
                    for range_item in body:
                        if key in range_item.keys():
                            if itme[1] in range_item[key].keys():
                                range_item[key][itme[1]] = self.build(range_item[key], ".".join(itme[1:]), field, vaule)
                                return body
                else:
                    for item in body:
                        if key in item.keys():
    ...
    def must_and_should(self, key, value1, value2, body=None):
        body = self.build(body, "query.bool.must.bool.should.match_phrase", key, value1)
        body = self.build(body, "query.bool.must.bool.should.match_phrase", key, value2)
        return body

    def search(self, body, index, page=1, size=20):
        from_ = (page - 1) * size
        if page * size > 10000:
