...
async def get_languages() -> Dict[Text, Any]:
    console.print("> [deep_sky_blue2]INFO[/deep_sky_blue2] ⬇️ Fetching all supported languages for NLU")
    ...
    async with get_async_http_session().get(
        ...
        if is_success_status(response.status):
            console.print("> [deep_sky_blue2]INFO[/deep_sky_blue2] ✅ Successfully created project")
            console.print(f"{json.dumps(json_response, indent=4)}")
        else:
            logger.error("> [red]ERROR[/red] ❌ Failed to create project")
...
async def delete_project(project_id: Text) -> Dict[Text, Any]:
    console.print(f"> [deep_sky_blue2]INFO[/deep_sky_blue2] 🗑️ Deleting project with id: {project_id}")
    ...
    async with get_async_http_session().delete(
        ...
        json_response = await response.json(encoding="utf-8")
        if is_success_status(response.status):
            console.print("> [deep_sky_blue2]INFO[/deep_sky_blue2] ✅ Successfully deleted project")
        else:
            logger.error("> [red]ERROR[/red] ❌ Failed to delete projects")
            logger.error(f"😔 Platform response: \n{json.dumps(json_response, indent=4)}")
    return json_response
...
def print_examples_table(examples: Dict[Text, Any]):
    ...
    console.print(f"> [deep_sky_blue2]INFO[/deep_sky_blue2] 🔢 Total Examples Count: {examples[DATA][COUNT]}")
...
def print_models_table(models: Dict[Text, Any]):
    ...
    console.print(f" [deep_sky_blue2]INFO[/deep_sky_blue2] 🔢 Total Models Count: {models[DATA][COUNT]}")
...
async def deploy(model_id: Text, n_replicas: int) -> Dict[Text, Any]:
    console.print(f"> [deep_sky_blue2]INFO[/deep_sky_blue2] ⤴️ Deploying: Model ID: {model_id}; Replicas: {n_replicas};")
    ...
    async with get_async_http_session().post(
        ...
        json_response = await response.json(encoding="utf-8")
        if is_success_status(response.status):
            console.print("> [deep_sky_blue2]INFO[/deep_sky_blue2] ✅ Model deployed successfully")
            console.print(f"> [deep_sky_blue2]INFO[/deep_sky_blue2] Platform Response: \n{json.dumps(json_response, indent=4)}")
        else:
