# coding: utf-8

"""
    Dockstore API

    This describes the dockstore API, a webservice that manages pairs of Docker images and associated metadata such as CWL documents and Dockerfiles used to build those images. Explore swagger.json for a Swagger 2.0 description of our API and explore openapi.yaml for OpenAPI 3.0 descriptions.  # noqa: E501

    OpenAPI spec version: 1.12.0-alpha.1-SNAPSHOT
    Contact: theglobalalliance@genomicsandhealth.org
    Generated by: https://github.com/swagger-api/swagger-codegen.git
"""


from __future__ import absolute_import

import unittest

import jgreenepack
from jgreenepack.jgreenepack.api.ga4_ghv1_api import GA4GHV1Api  # noqa: E501
from jgreenepack.rest import ApiException


class TestGA4GHV1Api(unittest.TestCase):
    """GA4GHV1Api unit test stubs"""

    def setUp(self):
        self.api = jgreenepack.jgreenepack.api.ga4_ghv1_api.GA4GHV1Api()  # noqa: E501

    def tearDown(self):
        pass

    def test_metadata_get(self):
        """Test case for metadata_get

        Return some metadata that is useful for describing this registry  # noqa: E501
        """
        pass

    def test_tool_classes_get(self):
        """Test case for tool_classes_get

        List all tool types  # noqa: E501
        """
        pass

    def test_tools_get(self):
        """Test case for tools_get

        List all tools  # noqa: E501
        """
        pass

    def test_tools_id_get(self):
        """Test case for tools_id_get

        List one specific tool, acts as an anchor for self references  # noqa: E501
        """
        pass

    def test_tools_id_versions_get(self):
        """Test case for tools_id_versions_get

        List versions of a tool  # noqa: E501
        """
        pass

    def test_tools_id_versions_version_id_dockerfile_get(self):
        """Test case for tools_id_versions_version_id_dockerfile_get

        Get the dockerfile for the specified image.  # noqa: E501
        """
        pass

    def test_tools_id_versions_version_id_get(self):
        """Test case for tools_id_versions_version_id_get

        List one specific tool version, acts as an anchor for self references  # noqa: E501
        """
        pass

    def test_tools_id_versions_version_id_type_descriptor_get(self):
        """Test case for tools_id_versions_version_id_type_descriptor_get

        Get the tool descriptor (CWL/WDL) for the specified tool.  # noqa: E501
        """
        pass

    def test_tools_id_versions_version_id_type_descriptor_relative_path_get(self):
        """Test case for tools_id_versions_version_id_type_descriptor_relative_path_get

        Get additional tool descriptor files (CWL/WDL) relative to the main file  # noqa: E501
        """
        pass

    def test_tools_id_versions_version_id_type_tests_get(self):
        """Test case for tools_id_versions_version_id_type_tests_get

        Get an array of test JSONs suitable for use with this descriptor type.  # noqa: E501
        """
        pass


if __name__ == '__main__':
    unittest.main()
