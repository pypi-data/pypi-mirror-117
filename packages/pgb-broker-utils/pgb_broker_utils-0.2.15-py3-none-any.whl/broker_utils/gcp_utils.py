...
def load_dataframe_bigquery(
    table_id: str, df: pd.DataFrame, infer_schema: bool = True,
):
    ...
    if infer_schema:
        ...
        for col in missing:
            df[col] = None
        # drop extra columns
        dropped = list(set(df.columns) - set(bq_col_names))
        if len(dropped) > 0:
            print(f'Dropping columns not in the table schema: {dropped}')
        df = df[bq_col_names]
