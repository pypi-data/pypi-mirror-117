...
class MllabHttpClient(MlPlatformHttpClient):
    ...
    def get_latest_model_id(self, experiment_id=int, auth=MlPlatformUserAuth) -> ModelDto:
        ...
        return from_dict(ModelDto,res.data)

    def get_inference_by_id(self, experiment_id: int, inference_id:int, auth: MlPlatformUserAuth) -> InferenceDto:
        res = self.send_request(
            "",
    ...
    def update_inference(
        self, experiment_id: int, dto: InferenceDto, auth: MlPlatformUserAuth
    ) -> dict:
        res = self.send_request(
            "inferences",
            {"experiments": experiment_id},
