...
class ModelQueryBuilder(QueryBuilder, ModelDBMixin):
    ...
    def drop_table(self) -> ModelDropQueryBuilder:
        return ModelDropQueryBuilder(
            db=self._db, dialect=self.dialect).drop_table(self._model.meta.table)
...
class ModelDropQueryBuilder(DropQueryBuilder, ModelDBMixin):

    def __init__(self, db: Database = None, **kwargs):
        super(ModelDropQueryBuilder, self).__init__(**kwargs)
        self._db = db
