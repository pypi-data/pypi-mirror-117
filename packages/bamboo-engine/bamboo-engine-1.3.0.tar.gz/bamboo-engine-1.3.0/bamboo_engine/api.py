...
def preview_node_inputs(
    ...
    :param root_pipeline_data: 流程数据
    :param current_constants: 当前变量
    :return: 执行结果
    :rtype: EngineAPIResult
    """
    total_constants = {**current_constants, **pipeline["data"].get("inputs", {})}
    context_values = [
        ContextValue(key=key, type=VAR_CONTEXT_MAPPING[info["type"]], value=info["value"], code=info.get("custom_type"))
        for key, info in total_constants.items()
    ]
    ...
    if subprocess_stack:
        ...
        param_data = {key: context.variables[key] for key, info in total_constants.items() if info["is_param"]}
        return preview_node_inputs(
            runtime=runtime,
            pipeline=child_pipeline,
            node_id=node_id,
            subprocess_stack=subprocess_stack[1:],
            root_pipeline_data={**root_pipeline_data, **param_data},
            current_constants=total_constants,
        )
