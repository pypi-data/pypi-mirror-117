...
class QxRequest(object):
  ...
  @property
  def sessionid(self):
    return self.headers.get('X-QX-SESSIONID', self.cookies.get('QXSESSIONID'))
...
class QxServerHook(object):
  ...
  def init_context(self) -> NoReturn:
    pass

  def load_context(self, request: QxRequest) -> Optional[Dict]:
    pass

  def init_session(self) -> NoReturn:
    pass
...
class QxServer(object):
  ...
  def init(self):
    HANDLERS_HOME = os.path.join(os.getcwd(), 'handlers')
    if os.path.isdir(HANDLERS_HOME):
      try:
        importlib.import_module('handlers')
        logging.info('handlers : loaded')
      except ImportError:
        logging.error('handlers : failed')

      handlers = set()

      for item in os.listdir(HANDLERS_HOME):
      ...
      for handler in handlers:
        try:
          importlib.import_module(f'handlers.{handler}')
          logging.info(f'handlers.{handler} : loaded')
        except ImportError:
          logging.error(f'handlers.{handler} : failed')

    if isinstance(self.hook, QxServerHook):
      self.hook.init_context()
      self.hook.init_session()
  ...
  def _target(self, target: Any) -> Any:
    ...
    if isinstance(path, str) and path:
      ...
      for method in meta.methods:
        ...
        logging.info(f'{server}{method} {path} : {func}')
