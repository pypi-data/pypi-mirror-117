...
class QxServer(object):
  ...
  def render_template(self, v: Callable):
    self._render_template = v
  ...
  def handle(self, request: QxRequest):
    ...
    if isinstance(result, QxHttpResult):
      response = QxResponse(
        ...
        body=self.render_template(**result.content) if isinstance(result, QxTemplate) and callable(self.render_template) else result.content,
...
class QxServerDecoratorHook(QxDecoratorHook):
  ...
  def _target(self, target: Any) -> Any:
    ...
    if isinstance(path, str) and path:
      ...
      name = self.arg('server')

      s = QxServer(name)
      s.handlers.register(QxHandler(
        func=func,
        meta=meta,
      ))
