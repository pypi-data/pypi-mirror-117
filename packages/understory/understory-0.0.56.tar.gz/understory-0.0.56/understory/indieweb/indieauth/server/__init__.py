...
class AuthorizationEndpoint:
    ...
    def get(self):
        ...
        return templates.signin(client, developer, form.scope.split(), supported_scopes)
    ...
    def post(self):
        try:
            ...
            # handle "Profile URL" response
            if "profile" not in auth_response["scopes"]:
                raise web.BadRequest("Profile URL request requires the `profile` scope")
...
def redeem_authorization_code():
    ...
    form = web.form("grant_type", "code", "client_id", "redirect_uri", "code_verifier")
    ...
    computed_code_challenge = base64.urlsafe_b64encode(
