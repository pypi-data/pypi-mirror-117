...
def init():
    ...
    try:
        ...
        HOST = config('HOST')  # server ip (E.g. - ngrok tunnel)
        print("hi")
        header = {
        ...
        try:
          repo.create_hook("web", header, EVENTS, active=True)
         
        #   pull
          for i in range(config('TIME')):
                response = requests.get(config('PUSH_COMMIT_URL'))
          ...
          response2 = requests.post(config('FETCH_LOG_URL'),data={'project_id':project_id,'repo_name':repo_name,'Time ':curtime,'user_name':username})           #success , project id , repo name , current date and time , email id
        #   print("status code is ",response2.status_code)
            ...
            for i in range(config('TIME')):
                ...
                if val > 0 and res==repo_name:
                    ...
                    time.sleep(60)
                    curtime = datetime.datetime.now()
                    response2 = requests.post(config('FETCH_LOG_URL'),data={'project_id':project_id,'repo_name':repo_name,'Time ':curtime,'user_name':username,'message':"pull success",'status':'pull operation performed'})
                    ...
                    flag=0
                    curtime = datetime.datetime.now()
                    response2 = requests.post(config('FETCH_LOG_URL'),data={'project_id':project_id,'repo_name':repo_name,'Time ':curtime,'user_name':username,'message': "pull failed",'status':'pull operation performed'})
