...
class AbstractGraphQLTestMixin(GraphQLFileUploadTestMixin, abc.ABC):
    ...
    def assert_json_path_of_type(self, graphql_response: GraphQLResponseInfo, path: str, allowed_type: Union[type, List[type]]):
        """
        ...
        def criterion(aresponse: GraphQLResponseInfo) -> Tuple[bool, str]:
            ...
            return type(actual) in allowed_type, f"in path {path}: actual was of type {type(actual)} but only {', '.join(map(lambda x: x.__name__, allowed_type))} are allowed!"
