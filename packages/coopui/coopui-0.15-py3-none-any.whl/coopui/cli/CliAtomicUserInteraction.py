...
logger = logging.getLogger('coopui')
...
class CliAtomicUserInteraction(IAtomicUserInteraction):
    ...
    def request_from_dict(cls, selectionDict: Dict[int, str], prompt=None,
        ...
        while True:
            ...
            if (inp or type(inp) == int) and selectionDict.get(inp, None) is not None:
                logger.info(f"User selected value: \"{inp}\" ({selectionDict[inp]}) from dictionary . PROMPT: {prompt}")
                return inp
            else:
                print("Invalid Entry...")

    @classmethod
    def request_index_from_df(cls, df: pd.DataFrame, prompt: str = None, cancel_text: str = 'CANCEL SELECTION',
        ...
        while True:
            ...
            cls.pretty_print_dataframe(print_out, prompt)
            ...
            inp = input("").upper()
            if cancel_text is not None and inp == cancel:
                logger.info(f"User cancelled selction from dataframe . PROMPT: {prompt}")
                return None
    ...
    @classmethod
    def request_open_filepath(cls, prompt: str = None, title: str = None, filetypes: Tuple[Tuple[str, str]] = None,
                              **open_file_kwargs):
        root = tkinter.Tk()
        ...
        while True:
            ...
            try:
                return clean_a_dataframe(df,
                                         ...
                                         column_name_replacement=lambda: cls.pd_column_replacement(df,
                                                                                                    required_columns))
            except Exception as e:
                cls.notify_user(text=f"Error when reading the csv file: {str(e)}")
                return None
    ...
    @classmethod
    def request_yes_no(cls, prompt: str = None, cancel_text: str = 'CANCEL SELECTION') -> bool:
    ...
    def int_tryParse(cls, value):
        try:
            ...
            logging.info(f"Unable to parse the value {value} to type <int>")
            return None
