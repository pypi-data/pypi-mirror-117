...
kron,arcsinh,arange,roll,ceil,sum,abs,cumsum,cumprod,zeros,prod,angle,average
...
def SNR_iGN(Incoh_Accessories,NLI_terms=None,_NMC=10e6,printlog=False):
    ...
    for Link_Accessories in Incoh_Accessories:
#        SNR_SingleLink_dB,_,__,___=SNR_EGN(iLink_EGN_Accessories,NLI_terms,_NMC,printlog)
        SNR_SingleLink_dB=SNR_GN(Link_Accessories,NLI_terms,_NMC,printlog)
        one_to_SNR_Linear+=1/10**(SNR_SingleLink_dB/10)
...
#                            LPLaunchPowerDict_Linear[tup2]
    ...
    SNR_dB_coherent=COILPLaunchPower_dBm-30-10*log10(G_ASE*COILPSymbolRate+NLI_var_GN)
#    SNR_dB_link_level=COILPLaunchPower_dBm-30-10*log10(G_ASE*COILPSymbolRate+NLI_var_link_level)
...
#    tqdm(range(NumofFiberSections),position=0,leave=True):
    ...
    DoubleNumLinks=[(i,j) for i in range(NumLinks) for j in range(NumLinks)]
    
    print('EGN in progress...')
    for k,kp in tqdm(DoubleNumLinks,position=0,leave=True):
