...
"""
The currently-active track. By default, this is the track named
'default'.
"""
...
def printTrack():
    """
    Prints out the log of the current track. When notes overlap, prints
    their start times, but otherwise just prints each note.
    TODO: Watch out for gaps?
    """
    thisTrack = TRACKS[ACTIVE_TRACK]
    skip = 0
    log = thisTrack["log"]
    ...
    for i, entry in enumerate(log):
        ...
        if len(describe) == 1:
            if first:
                ...
                lastSkip = False
                print("at {:.3g}s".format(describe[0][0]), describe[0][2])
            else:
                print("and", describe[0][2])
        else:
            first = False
            skip = len(describe) - 1
            for start, duration, description in describe:
                if not description.endswith("rest"):
                    print("at {:.3g}s".format(start), description)
