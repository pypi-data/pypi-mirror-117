...
class LanguageSpecificRulesAnalyzer(RulesAnalyzer):
    ...
    def is_potential_anaphoric_pair(self, referred:Mention, referring:Token, directly:bool) -> bool:
            ...
            if token.tag_ != 'PPOSAT':
                if self.has_morph(token, 'Number', 'Sing'):
                    ...
                    if self.has_morph(token, 'Gender', 'Neut'):
                        neut = True
                        if lemma_ends_with_word_in_list(token, self.neuter_person_words):
                            masc = True
                            fem = True
                        if lemma_ends_with_word_in_list(token, self.neuter_male_words):
                            masc = True
                        if lemma_ends_with_word_in_list(token, self.neuter_female_words):
                            fem = True
                        if not masc and not fem and (token.lemma_.lower().endswith('chen') or \
                                token.lemma_.lower().endswith('lein') and len(token.lemma_) > 6):
                            masc = True
                            fem = True
                    if token.pos_ == 'PROPN':
                        ...
                        if token.lemma_ in self.female_names:
                            fem = True
                        if token.lemma_ not in self.male_names and \
                                token.lemma_ not in self.female_names:
                            masc = fem = neut = True
            ...
            if self.is_potential_anaphor(token):
                if token.tag_ in ('PROAV', 'PRF'):
                    ...
                    if self.has_morph(token, 'Number', 'Sing') and \
                            ...
                            (self.has_morph(token, 'Case', 'Dat') or
                            self.has_morph(token, 'Case', 'Gen')):
                        neut = True
                    elif self.has_morph(token, 'Number', 'Sing') and \
                            self.has_morph(token, 'Gender', 'Fem') and \
        ...
        referring_masc, referring_fem, referring_neut, referring_plur = \
        ...
        for working_token in (doc[index] for index in referred.token_indexes):
            working_masc, working_fem, working_neut, working_plur = get_gender_number_info(
                working_token)
        ...
        if not (referred_masc and referring_masc) and not (referred_fem and referring_fem) \
                and not (referred_neut and referring_neut) and not (referred_plur and
                referring_plur):
            return 0
        ...
        if referring.tag_ == 'PROAV':
            ...
            # 'damit' etc. does not refer to nouns over several sentences
            if referring._.coref_chains.temp_sent_index - \
                    referred_root._.coref_chains.temp_sent_index > 1:
                return 0
            ...
            for working_token in (doc[index] for index in referred.token_indexes):
                if working_token.lemma_ in self.male_names or \
                        ...
                        working_token.ent_type_ in ('PER', 'LOC', 'ORG'):
                    return 0
                if self.is_potential_anaphor(working_token) and (referred_masc or referred_fem):
                    return 0
            ...
            if referring.tag_ == 'PPOSAT':
                ...
                while working_token.dep_ != self.root_dep:
                    ...
                    if working_token.dep_ not in self.conjunction_deps and \
                            ...
                            working_token.dep_ != 'ag' and working_token.tag_ != 'PPOSAT':
                        break
        ...
        if referring_governing_sibling.dep_ == 'sb' and \
