...
class ModelGenerator:
    ...
    def generate_feature_table(self, docs:list) -> FeatureTable:
        ...
        for doc in docs:
            for token in (token for token in doc if
                    self.rules_analyzer.is_independent_noun(token) or
    ...
    def generate_keras_model(self, docs: list, tendencies_analyzer, ensemble_size:int):
        ...
        def create_vector_squeezer(name, ensemble_index):
            ...
            """
            input_layer = keras.Input(shape=(vector_width,), name='_'.join((name, 'vector_input',
                str(ensemble_index))))
            layer = layers.Dense(24, activation="relu", name='_'.join((name,
                'vector_hidden_0_', str(ensemble_index))))(input_layer)
            output_layer = layers.Dense(3, activation="relu",
                name='_'.join((name, 'vector_output', str(ensemble_index))))(layer)
        ...
        feature_map = tendencies_analyzer.get_feature_map(helpful_docs[0][0], helpful_docs[0])

        feature_map_width = len(feature_map)
        position_map_width = len(tendencies_analyzer.get_position_map(helpful_docs[0][0],
            helpful_docs[0]))
        compatibility_map_width = len(tendencies_analyzer.get_compatibility_map(
            Mention(helpful_docs[0][0], False), helpful_docs[0][1]))
        overall_input_width = (2*feature_map_width) + (2*position_map_width) + \
            compatibility_map_width + 12 # each vector is squeezed to 3 neurons

        keras_inputs = []
        keras_outputs = []
        for ensemble_index in range(ensemble_size):
            referred_vector_input, referred_vector_output = create_vector_squeezer(
                'referred', ensemble_index)
            keras_inputs.append(referred_vector_input)
            referred_head_vector_input, referred_head_vector_output = \
                create_vector_squeezer('referred_head', ensemble_index)
            keras_inputs.append(referred_head_vector_input)
            referred_feature_map_input = keras.Input(shape=(feature_map_width,),
                name='_'.join(("referred_feature_map_input", str(ensemble_index))))
            keras_inputs.append(referred_feature_map_input)
            referred_position_map_input = keras.Input(shape=(position_map_width,),
                name='_'.join(("referred_position_map_input", str(ensemble_index))))
            keras_inputs.append(referred_position_map_input)
            referring_vector_input, referring_vector_output = create_vector_squeezer(
                'referring', ensemble_index)
            keras_inputs.append(referring_vector_input)
            referring_head_vector_input, referring_head_vector_output = \
                create_vector_squeezer('referring_head', ensemble_index)
            keras_inputs.append(referring_head_vector_input)
            referring_feature_map_input = keras.Input(shape=(feature_map_width,),
                name='_'.join(("referring_feature_map_input", str(ensemble_index))))
            keras_inputs.append(referring_feature_map_input)
            referring_position_map_input = keras.Input(shape=(position_map_width,),
                name='_'.join(("referring_position_map_input", str(ensemble_index))))
            keras_inputs.append(referring_position_map_input)
            compatibility_map_input = keras.Input(shape=(compatibility_map_width,),
                name='_'.join(("compatibility_map_input", str(ensemble_index))))
            keras_inputs.append(compatibility_map_input)
            layer = layers.Concatenate(axis=1, name='_'.join(("combined_input",
                str(ensemble_index))))([referred_vector_output,
                referred_head_vector_output, referred_feature_map_input,
                referred_position_map_input, referring_vector_output,
                referring_head_vector_output, referring_feature_map_input,
                referring_position_map_input, compatibility_map_input])
            layer = layers.Dense(overall_input_width, activation="relu",
                name='_'.join(("combined_hidden_0", str(ensemble_index))))(layer)
            layer = layers.Dense(20, activation="relu",
                name='_'.join(("combined_hidden_1", str(ensemble_index))))(layer)
            output = layers.Dense(1, activation="sigmoid",
                name='_'.join(("output", str(ensemble_index))))(layer)
            keras_outputs.append(output)
        keras_model = keras.Model(inputs=keras_inputs, outputs=keras_outputs, name='_'.join((
    ...
    def train_keras_model(self, docs: list, tendencies_analyzer, keras_model):
        ...
        for doc in docs:
            for token in (t for t in doc if hasattr(t._.coref_chains, 'temp_potential_referreds')):
                for index, referred in enumerate(r for r in
                        token._.coref_chains.temp_potential_referreds if
                        hasattr(r, 'true_in_training')):
                    while True:
                        working_referring = doc[referred.root_index]
                        if not hasattr(working_referring._.coref_chains,
                                'temp_potential_referreds'):
                            break
                        for index, working_referred in enumerate(r for r in
                                working_referring._.coref_chains.temp_potential_referreds if
                                hasattr(r, 'true_in_training')):
                            assert index == 0 # should only be one
                            for index, spanning_referred in enumerate(r for r in
                                    token._.coref_chains.temp_potential_referreds if
                                    r == working_referred):
                                assert index == 0 # should only be one
                            ...
                            break
        keras_inputs, _, keras_outputs = tendencies_analyzer.prepare_keras_data(docs,
            return_outputs=True)
        return keras_model.fit(x=keras_inputs, y=keras_outputs,
