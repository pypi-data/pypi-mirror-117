...
class LanguageSpecificRulesAnalyzer(RulesAnalyzer):
    ...
    def is_independent_noun(self, token:Token) -> bool:
        if not ((token.pos_ in self.noun_pos and token.dep_ not in ('compound', 'npadvmod')) or
                (token.tag_ == 'CD' and token.dep_ != 'nummod') or
                (token.tag_ == 'DT' and token.dep_ != 'det') or
                (token.pos_ == 'PRON' and token.tag_ == 'NN')):
            return False
    ...
    def is_potential_anaphor(self, token:Token) -> bool:
        ...
        if token.tag_ not in ('PRP', 'PRP$') or not self.has_morph(token, 'Person', '3'):
            return False
        ...
        if len(token._.coref_chains.temp_dependent_siblings) > 0 or \
        ...
        for child in (child for child in token.children if child.dep_ == 'amod'):
            if len([grandchild for grandchild in child.children if grandchild.dep_
                    not in ('acomp', 'xcomp')]) == 0:
                return False

        # e.g. '*It* seems to be believed that he has done it'
        if token.dep_ != 'ROOT' and \
                (token.head.lemma_ in ('be', 'seem') or token.dep_ == 'nsubjpass') and \
                len([child for child in token.head.children if child.dep_ in
                     ('advcl', 'ccomp', 'xcomp')]) > 0:
            return False

        # e.g. 'This makes *it* unlikely that he has done it'
        if token.dep_ != 'ROOT' and token.i > 0 and \
                token.doc[token.i - 1].lemma_ == 'make' and \
        ...
        if token.dep_ in ('nsubj', 'nsubjpass') and token.head.lemma_ == 'be':
            for child in token.head.children:
                if len([grandchild for grandchild in child.children if grandchild.dep_ == 'relcl'])\
                        > 0:
                    return False

        # Avalent verbs, e.g. '*it* is snowing'
        if token.dep_ != self.root_dep and token.head.pos_ == 'VERB' and len(
    ...
    def is_potential_anaphoric_pair(self, referred:Mention, referring:Token, directly:bool) -> bool:
        ...
        if len(referred.token_indexes) == 1 and self.has_morph(referring, 'Number', 'Plur') and \
        ...
        if len(referred.token_indexes) == 1 and \
                self.is_potential_anaphor(referred_root) and \
                (referred_root.morph.get('Number') != referring.morph.get('Number') or
                referred_root.morph.get('Gender') != referring.morph.get('Gender')):
            return 0

        # Singular anaphor, plural referent
        if self.has_morph(referring, 'Number', 'Sing') and (len(referred.token_indexes) > 1 or
                self.has_morph(referred_root, 'Number', 'Plur')):
            return 0
        ...
        if not self.is_potential_anaphor(referred_root):
            ...
            if self.has_morph(referring, 'Number', 'Plur') and len(referred.token_indexes) == 1 \
                    and self.has_morph(referred_root, 'Number', 'Sing'):
                if referred_lemma not in self.person_words:
                    if referred_root.tag_ != 'NNP' and referred_root.ent_type_ != 'PERSON':
                        return 0
                    else:
                        # named people who choose to refer to themselves with 'they'
                        uncertain = True
                if referred_lemma in self.exclusively_male_words or \
                        referred_lemma in self.exclusively_female_words:
                    uncertain = True

            # 'he' or 'she' referring to non-person, non-animal noun
            if (self.has_morph(referring, 'Gender', 'Masc') or \
                    self.has_morph(referring, 'Gender', 'Fem')) and \
                    referred_lemma not in self.exclusively_person_words and \
                    referred_lemma not in self.animal_words and \
                    referred_lemma not in self.male_names and \
                    referred_lemma not in self.female_names and \
                    referred_root.ent_type_ != 'PERSON':
                if referred_root.tag_ != 'NNP' and referred_lemma not in self.person_words:
                    return 0
                else:
                    uncertain = True

            # 'it' referring to person noun or entity
            if self.has_morph(referring, 'Gender', 'Neut') and (referred_lemma in
                    self.exclusively_person_words or referred_root.ent_type_ == 'PERSON'):
                return 0

            # 'it' referring to plural proper name
            if self.has_morph(referring, 'Gender', 'Neut') and referred_root.tag_ == 'NNPS':
                uncertain = True

            # 'he' referring to female noun
            if self.has_morph(referring, 'Gender', 'Masc') and \
                    referred_lemma in self.exclusively_female_words and \
                    referred_lemma not in self.animal_words:
                return 0

            # 'she' referring to male noun
            if self.has_morph(referring, 'Gender', 'Fem') and \
                    referred_lemma in self.exclusively_male_words and \
                    referred_lemma not in self.animal_words:
                return 0

            # 'it' referring to name
            if self.has_morph(referring, 'Gender', 'Neut') and referred_root.tag_ == 'NNP' and \
                    (referred_lemma in self.male_names or
                    referred_lemma in self.female_names):
                return 0

            # 'he' referring to female name
            if self.has_morph(referring, 'Gender', 'Masc') and referred_root.tag_ == 'NNP' and \
                    self.has_list_member_in_propn_subtree(
                    doc[referred.root_index], self.exclusively_female_names):
                uncertain = True

            # 'she' referring to male name
            if self.has_morph(referring, 'Gender', 'Fem') and referred_root.tag_ == 'NNP' and \
                    self.has_list_member_in_propn_subtree(
        ...
        if directly:
            ...
            if not uncertain and not self.is_potential_reflexive_pair(referred, referring) and \
    ...
    def is_potential_reflexive_pair(self, referred:Mention, referring:Token) -> bool:
        ...
        if referred_root.dep_ in syntactic_subject_dep:
            for referring_ancestor in referring.ancestors:
                ...
                if len([t for t in referring_ancestor.children if t.dep_ in
                        syntactic_subject_dep and t != referred_root]) > 0:
                    return False
            return False
        else:
