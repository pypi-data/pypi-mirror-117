...
class KafkaClient:
    ...
    def set_offset(self, topic: str, partition: int, offset: int):
        if self.consumer:
            ...
            self.consumer.seek(topic_p, offset)
        ...
        if self.consumer:
            ...
            while True:
                try:
                    ...
                    logger.log_err(f"Message deserialization failed: {e}")
                    continue
                if msg is None:
                    continue
                if msg.error():
                    logger.log_err(f"{'Avro' if self.avro_consumer else ''}Consumer error: {msg.error()}")
                    continue
