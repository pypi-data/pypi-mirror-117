...
class ALGymEnv(gym.Env, utils.EzPickle):
    ...
    def __init__(
        ...
        if not hasattr(roms, game):
            raise error.Error(
                f"Unable to find game \"{game}\", did you import \"{game}\" with ale-import-roms?"
            )
        ...
        if self._obs_type == "ram":
            self._obs = np.empty((self.ale.getRAMSize(),), dtype=np.uint8)
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        """
        if self._obs_type == "ram":
            self.ale.getRAM(self._obs)
        elif self._obs_type == "rgb":
            self.ale.getScreenRGB(self._obs)
        elif self._obs_type == "grayscale":
            self.ale.getScreenGrayscale(self._obs)
        else:
            raise error.Error(f"Unrecognized observation type: {self._obs_type}")
    ...
    def get_action_meanings(self) -> List[str]:
        ...
        values = map(lambda action: str(action), Action.__members__.values())
