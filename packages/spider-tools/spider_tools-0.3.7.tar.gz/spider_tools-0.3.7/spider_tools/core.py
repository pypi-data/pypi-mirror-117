...
cchardet = LazyImport("import cchardet")
json = LazyImport("import json")
oss2 = LazyImport("import oss2")
csv = LazyImport("import csv")
...
arrow = LazyImport("import arrow")
execjs = LazyImport("import execjs")
Process = LazyImport("from multiprocessing import Process")
pytesseract = LazyImport("import pytesseract")
pseg = LazyImport("import jieba.posseg as pseg")
BytesIO = LazyImport("from io import BytesIO")
xlrd = LazyImport("import xlrd")
PooledDB = LazyImport("from DBUtils.PooledDB import PooledDB")
listdir = LazyImport("from os import listdir")
threading = LazyImport("import threading")
Selector=LazyImport("from scrapy import Selector")
ThreadPool = LazyImport("from multiprocessing.dummy import Pool as ThreadPool")
        ...
        if item and sql_conn and cursor:
            ...
            if item_key:
                ...
                insert = 'insert ignore into {}('.format(table_name) + ','.join(item_key) + ')' + 'values({})'.format(
                    sss[:-1])
                data = '(' + ','.join(['%r' % str(i) for i in item_values]) + ')'
                data_list = []
                for i in eval(data):
                    if i == 'None':
                        i = None
...
class Task_Allocation():
    def __init__(self, redis_db, mysql_db, new_company_task):
        ...
        self.sql_pool = PooledDB(pymysql, mysql_db['pool_num'], host=mysql_db['host'], user=mysql_db['user'],
                                 passwd=mysql_db['passwd'], db=mysql_db['db'], port=3306,
                                 charset='utf8', use_unicode=True)
...
class UPLOAD_FILE():
    def __init__(self, subfilename, key="", password="", net_address="", db=""):
        ...
        self.subfilename = subfilename + '/' + datetime.datetime.now().strftime("%Y%m/") + '{}'.format(
            datetime.datetime.now().day)  # oss 路径# oss 路径
    ...
    def extract_online(self, url):
        ...
        if r.status_code == 200:
            if r.encoding == 'ISO-8859-1':
                r.encoding = cchardet.detect(r.content)['encoding']  # 确定网页编码
            html = r.text
            title = self.extract_title(html)
            description = self.extract_description(html)
            keyword = self.extract_keywords(html)
...
def Thread_Grab(func, task_list, thread_num=4):
    ...
    for i in tqdm(range(len(task_list)),
                  desc=f'执行函数：{func.__name__} 任务列表:[{str(task_list[0]) + ",...," + str(task_list[-1])}]'):
        Lock.acquire()
        args = task_list.pop()
        Lock.release()
