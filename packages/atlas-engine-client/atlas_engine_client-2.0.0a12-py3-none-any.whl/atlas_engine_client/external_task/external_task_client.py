...
class ExternalTaskClient(BaseClient):
    ...
    def subscribe_to_external_task_topic(self, topic, handler, **options):
        ...
        if task_options.get('long_polling_timeout', None) is not None:
            logging.warning('deprecated long_polling_timeout, please use long_polling_timeout_in_ms')
        ...
        if task_options.get('lock_duration', None) is not None:
            logging.warning('deprecated lock_duration, please use lock_duration_in_ms')
        ...
        if task_options.get('additional_lock_duration', None) is not None:
            logging.warning('additional_lock_duration is ignored, but lock_duration_in_ms is used')

        if task_options.get('extend_lock_timeout', None) is not None:
            logging.warning('extend_lock_timeout is ignored, but 90 percent of lock_duration is used')
