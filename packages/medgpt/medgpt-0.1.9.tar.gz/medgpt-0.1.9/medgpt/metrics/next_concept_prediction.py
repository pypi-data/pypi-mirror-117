    ...
    for ind, lbl in enumerate(label_ids):
        if start < len(lbl):
            for i in range(start, len(lbl)):
                ...
                #tkn = str(id2tkn.get(outputs[ind][i-label_position_shift], outputs[ind][i-label_position_shift]))
                if not token_types or (id2type.get(lbl[i], 'unk') in token_types):
                    candidates = []
                    if predictions_are_scores:
                        #candidates.extend(outputs[ind][i-label_position_shift][0:topk])

                        # We only get the type of canidate we know we need at this position
                        for k in range(len(outputs[ind][i-label_position_shift])):
                            if id2type.get(outputs[ind][i-label_position_shift][k], 'unk') == id2type.get(lbl[i], 'unk'):
                    ...
                    for candidate in candidates:
                        ...
                        if not token_types or (id2type.get(candidate, 'unk') in token_types):
                            # If predictions are scores we can do topk, if not just do simple label match
                            if candidate in lbl[i:end]:
                                positives[tkn_candidate] = positives.get(tkn_candidate, 0) + 1
                                is_tp = True
                            else:
                                negatives[tkn_candidate] = negatives.get(tkn_candidate, 0) + 1
                        else:
                            negatives[tkn_candidate] = negatives.get(tkn_candidate, 0) + 1
                    if is_tp:
                        tp += 1
