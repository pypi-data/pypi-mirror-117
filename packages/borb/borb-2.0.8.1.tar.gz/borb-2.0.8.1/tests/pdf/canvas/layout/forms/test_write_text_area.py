...
class TestWriteTextArea(unittest.TestCase):
    ...
    def test_write_text_area_at_absolute_position(self):
        ...
        tf.layout(page, Rectangle(Decimal(59), Decimal(600), Decimal(476), Decimal(12.5 * 5)))
    ...
    def test_write_text_field_using_layout_manager(self):
        ...
        l.add(
            ...
            .add(Paragraph(Path(__file__).stem))
            .add(Paragraph("Description", font="Helvetica-Bold"))
            .add(Paragraph("This test creates a PDF with a few TextField in it."))
            .set_padding_on_all_cells(Decimal(2), Decimal(2), Decimal(2), Decimal(2))
        )

        # write TextField
        l.add(
            FixedColumnWidthTable(number_of_rows=3, number_of_columns=2, margin_top=Decimal(20))
            .add(Paragraph("Name:"))
            .add(TextField(value="Lorem", font_color=HexColor("56cbf9"),  font_size=Decimal(20)))
            .add(Paragraph("Firstname:"))
            .add(TextField(value="Ipsum"))
    ...
    def test_check_acroform_value(self):
        ...
        assert doc is not None
        assert doc.get_page(0).has_acroforms()
        assert doc.get_page(0).has_form_field("place_of_residence")

        doc.get_page(0).set_form_value("place_of_residence", "Lorem Ipsum Dolor Sit Amet")

        assert doc.get_page(0).get_form_value("place_of_residence") == "Lorem Ipsum Dolor Sit Amet"
