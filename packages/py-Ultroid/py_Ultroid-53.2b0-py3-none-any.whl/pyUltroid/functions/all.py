...
from telethon.utils import get_input_location, resolve_bot_file_id
...
async def get_user_id(ids, client=ultroid_bot, give_entity=False):
    """Get User Id from text"""
    if str(ids).isdigit():
        userid = int(ids)
    else:
        try:
            userid = await client.get_entity(ids)
            if give_entity:
                return userid
            return userid.id
...
async def fast_download(download_url, file_name=None, progress_callback=None):
    async with aiohttp.ClientSession() as session:
        async with session.get(download_url, timeout=None) as response:
            if not file_name:
                file_name = download_url.rpartition("/")[-1]
