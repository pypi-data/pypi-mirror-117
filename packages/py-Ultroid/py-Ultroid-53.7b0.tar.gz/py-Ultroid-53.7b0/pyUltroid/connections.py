...
def connect_redis():
    if Var.REDIS_URI and Var.REDIS_PASSWORD:
        ...
        if err != "":
            LOGS.info(err)
            exit(1)
        redis_info = Var.REDIS_URI.strip().split(":")
        LOGS.info("Getting Connection With Redis Database")
        time.sleep(3.5)
        return redis.Redis(
            host=redis_info[0],
            port=redis_info[1],
            password=Var.REDIS_PASSWORD.strip(),
...
def vc_connection(udB, ultroid_bot):
    ...
    if VC_SESSION:
        try:
            ...
            CallsClient = GroupCallFactory(
                vcClient, GroupCallFactory.MTPROTO_CLIENT_TYPE.TELETHON
            )
            return vcClient, CallsClient
        except Exception as er:
            LOGS.info("VC_SESSION: {}".format(str(er)))
            exit(1)
    return ultroid_bot, GroupCallFactory(
        ultroid_bot, GroupCallFactory.MTPROTO_CLIENT_TYPE.TELETHON
    )
