...
class CopyContainer(Sequence):
    def __init__(
        ...
        *,
        ...
        self.artifacts = Artifacts("image", expire_in="1h", public=False)
        self.crane_pull_job = crane.Pull(
            src_registry=src_registry,
            image_name=image_name,
            image_tag=image_tag,
            tar_path=self.artifacts.paths[0],
            docker_client_config=copy.deepcopy(docker_client_config),
            **crane_kwargs,
        )
        self.crane_pull_job.set_artifacts(self.artifacts)
        self.add_children(self.crane_pull_job)

        if do_dive_scan:
            self.dive_scan_job = dive.Scan(
                image_path=self.artifacts.paths[0],
                image_name=image_name,
                **dive_kwargs,
            )
            self.dive_scan_job.add_dependencies(self.crane_pull_job)
        ...
        if do_trivy_scan:
            ...
            self.trivy_scan_job.add_dependencies(self.crane_pull_job)
        ...
        self.crane_push_job.add_dependencies(self.crane_pull_job)
