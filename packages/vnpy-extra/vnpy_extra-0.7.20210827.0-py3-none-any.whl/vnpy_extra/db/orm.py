...
class AccountStrategyMapping(Model):
    ...
    @staticmethod
    def get_stats_by_account(user_name=None, broker_id=None):
        if not user_name:
            user_name, broker_id = get_account()
        ...
        stats_list: typing.List[StrategyBacktestStats] = list(query.objects())
        return stats_list
    ...
    @staticmethod
    def delete_by_account(user_name=None, broker_id=None):
        """按账户删除策略"""
        if not user_name:
            user_name, broker_id = get_account()

        count = AccountStrategyMapping.delete().where(
            AccountStrategyMapping.user_name == user_name, AccountStrategyMapping.broker_id == broker_id
        ).execute()
        logger.info(f"{user_name}[{broker_id}] {count} data deleted.")

    @staticmethod
    def delete_if_no_position(*, stg_info_id, symbols_info_id, id_name, short_name, user_name=None, broker_id=None):
        if not user_name:
            user_name, broker_id = get_account()
    ...
    @staticmethod
    def delete_by_unavailable_stg(*, stg_info_id, symbols_info_id, id_name):
