...
class CtaTemplate(CtaTemplateBase):
    ...
    def cancel_order(self, vt_orderid):
        if vt_orderid in self.send_order_dic:
            ...
            self.write_log(f"取消停止单 {vt_orderid} {order.offset.value} {order.direction.value}", 'debug')
    ...
    def send_order_2_target_pos_close_rejected(self, active_order_close, active_order_open):
        ...
        if self.target_pos < current_pos:
            ...
            if active_order_open:
                ...
                self.write_log(f"平多被拒绝，直接开空 {current_pos} - > {self.target_pos}")
                self.short(price, volume)

        elif current_pos < self.target_pos:
            # 平空被拒绝，直接开多
            if active_order_open:
                if self.is_same_as_order(active_order_open, Direction.LONG, volume, price):
                    # 与当前订单一致
                ...
                self.write_log(f"平空被拒绝，直接开多 {current_pos} - > {self.target_pos}")
                self.buy(price, volume)
