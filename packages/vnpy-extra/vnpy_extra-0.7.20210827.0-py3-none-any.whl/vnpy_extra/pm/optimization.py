...
class PortfolioOptimization:
    ...
    def _fit_by_group_keys(self, key_weight_dic, group: InstrumentClassificationEnum, key_set: set):
        if self.goal == PortfolioOptimizeGoalEnum.equal_mdd:
            ...
            _key_weight_dic = {key: benchmark_capital / np.abs(v) * self.group_weight_dic[group]
    ...
    def get_base_positions(self, capital=1_000_000) -> Dict[str, int]:
        ...
        self.logger.info(f"capital: {capital:,d} 按策略统计持仓：\n{pd.DataFrame(msg_list).to_markdown(index=False)}")
...
class PortfolioOptimizationGroupFitting(PortfolioOptimization):
    ...
    def fit(self):
        ...
        for group_name, fitting_key_group in group_fitting_key_group_dic.items():
            ...
            for key in fitting_key_group.key_set:
                key_group_dic[key] = group_name
                vt_symbol_group_dic[self.key_vt_symbol_dic[key]] = group_name

        # msg = "\n".join(
        #     [f"{group_name:50s} {fitting_key_group.weights * 100:.2f}%"
        #      for group_name, fitting_key_group in group_fitting_key_group_dic.items()])
        df = pd.DataFrame({
            group_name: [fitting_key_group.weights, len(fitting_key_group.key_set), fitting_key_group.goal.name]
            for group_name, fitting_key_group in group_fitting_key_group_dic.items()},
        ...
        for num, vt_symbol in enumerate(vt_symbols, start=1):
            ...
            msg_dic_list.append({
                "No": num,
                "group_name": vt_symbol_group_dic[vt_symbol],
                "vt_symbol": vt_symbol,
    ...
    def _fit_by_group_keys(self, fitting_keys_group: FittingKeysGroup):
        ...
        if fitting_keys_group.goal == PortfolioOptimizeGoalEnum.equal_mdd:
            ...
            benchmark_capital = fitting_keys_group.kwargs.get('benchmark', 100_000)
            _key_weight_dic = {key: benchmark_capital / np.abs(v) * fitting_keys_group.weights
                               for key, v in key_mdd_dic.items()}
            key_weight_dic.update(_key_weight_dic)
...
def portfolio_optimization_by_account(
    ...
    if refresh_pos:
        ...
        monitor.logger.info(f"{'*'*20} {user_name}[{broker_id}] 开始更新持仓信息 {'*'*20}")
