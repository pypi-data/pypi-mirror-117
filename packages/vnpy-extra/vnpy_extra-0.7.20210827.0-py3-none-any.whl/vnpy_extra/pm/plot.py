...
def log_contributes(rr_df: pd.DataFrame, key_vt_symbol_dic: Optional[dict] = None,
    ...
    msg = pd.DataFrame([
        {
            ...
            'base_position': key_base_position_dic.get(key, 0),
        }
        if key_base_position_dic else
        {
            "No": num,
            "key": key,
            "贡献(%)": np.round(contribute * 100, 2),
    ...
    if key_vt_symbol_dic:
        ...
        vt_symbol_contribute_df = pd.DataFrame(
            {'贡献度(%)': group.sum().apply(lambda x: np.round(x * 100, 2)), 'STG_COUNT': group.count()})
        # count = len(group)
        # msg = '\n'.join([
        #     f"{num:2d}/{count:2d}) {key:11s}  {stg_count:2d}       {weight * 100:5.2f}%"
        #     for num, ((key, weight), (_, stg_count)) in enumerate(
        #         zip(group.sum().items(), group.count().items()), start=1)
        # ])
        # logger.info(f"按 vt_symbol 分组收益贡献度：\n       VT_SYMBOLS  STG_COUNT    PCT\n{msg}")
        if key_base_position_dic:
            ...
            vt_symbol_contribute_df['base_positions'] = vt_symbol_contribute_df.apply(
                lambda x: vt_symbol_base_position_dic.get(x.name, 0), axis=1, )

        if vt_symbol_market_value_dic:
            market_value_s = vt_symbol_contribute_df.apply(
                lambda x: vt_symbol_market_value_dic.get(x.name, 0), axis=1, )
