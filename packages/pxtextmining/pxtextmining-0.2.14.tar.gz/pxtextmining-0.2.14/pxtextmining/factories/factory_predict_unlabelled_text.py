...
def factory_predict_unlabelled_text(dataset, predictor, pipe_path_or_name,
    ...
    **NOTE:** As described later, argument `theme` is for internal use by Nottinghamshire Healthcare NHS Foundation
    ...
    :param str, sklearn.model_selection._search.RandomizedSearchCV pipe_path_or_name: A string in the form
    ...
    if not isinstance(pipe_path_or_name, str):
        pipe = joblib.load(pipe_path_or_name)
    else:
        pipe = pipe_path_or_name
    if theme is None:
