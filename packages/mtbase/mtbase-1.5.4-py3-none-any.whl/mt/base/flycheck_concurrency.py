...
class WorkIterator(object):
    ...
    If the number of works is provided and is large, the class invokes ProcessParalleliser to do
    a few works ahead of time. Otherwise, it does the works in serial.
    ...
    max_num_workers : int, optional
        maximum number of concurrent workers or equivalently processes to be allocated
    work_cnt : int, optional
        desired number of works to be performed. Only useful to determine whether to do work in
        parallel or serial. See the next argument.
    serial_threshold : int, optional
        if the number of desired works is below the threshold here, the works are computed in
        serial. Otherwise, they are computed in parallel as usual.
    ...
    def __init__(self, func, buffer_size=None, skip_null=True, push_timeout=30, pop_timeout=60*60, max_num_workers=None, work_cnt=None, serial_threshold=None, logger=None):
        self.skip_null = skip_null
        self.send_counter = 0
        self.recv_counter = 0
        self.retr_counter = 0
        if work_cnt is not None and serial_threshold is not None and work_cnt <= serial_threshold:
