...
class CustomTable:
    ...
    def copy(self):
        ...
        """
        return CustomTable(
            self.style, self.data, self.col_width, **self.settings
        )
    ...
    def insert(self, *vals):
        ...
        (proc(*x) for x in enumerate(self.data))
    ...
    def inserts(self, matrix: typing.Iterable[typing.Iterable]):
        ...
        Optional[CustomTable]
            Only in exp mode.
        """
        if any(self.insert(*row) for row in matrix):
            return self.settings["exp"]
    ...
    def _get_l(self, s, row=-1):
        ...
        while any(next_val):  # wrapping required
            ...
            for i, val in enumerate(next_val):
                ...
                spaces_needed = (
                    width
                    if (width := self.col_width[i]) <= max_len
                    else max_len
                ) - len(val)
                res += f"{s} {val} " + " " * spaces_needed
            res = f"{res[:-1]} {s}\n"
