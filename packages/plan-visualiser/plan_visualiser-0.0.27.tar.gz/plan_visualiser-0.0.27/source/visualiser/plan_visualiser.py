...
class PlanVisualiser:
    ...
    def plot_vertical_line(self, current_date):
        ...
        # Don't try to plot line if out of range for visual
        if x >= self.plot_config.left and x <= self.plot_config.right:
            top = self.plot_config.top
            bottom = self.plot_config.bottom
            xx, topp, bottomm = map(lambda flt: int(round(flt)), (x, top, bottom))
            line = self.shapes.add_connector(MSO_CONNECTOR_TYPE.STRAIGHT, xx, topp, xx, bottomm)
