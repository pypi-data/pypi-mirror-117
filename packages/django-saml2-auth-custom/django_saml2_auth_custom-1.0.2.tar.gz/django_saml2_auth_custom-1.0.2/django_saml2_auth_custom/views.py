...
def _get_saml_client(domain):
    acs_url = domain + get_reverse([acs, 'acs', 'django_saml2_auth_custom:acs'])
    import tempfile
...
def welcome(r):
    try:
        return render(r, 'django_saml2_auth_custom/welcome.html', {'user': r.user})
    except TemplateDoesNotExist:
        return HttpResponseRedirect(get_reverse('admin:index'))


def denied(r):
    return render(r, 'django_saml2_auth_custom/denied.html')
...
def acs(r):
    ...
    if not resp:
        return HttpResponseRedirect(get_reverse([denied, 'denied', 'django_saml2_auth_custom:denied']))
    ...
    if(len(user_email) >= 30): user_name = user_email[:30]
    else : user_name = user_email[:len(user_email)]
...
def signout(r):
    logout(r)
    return render(r, 'django_saml2_auth_custom/template/signout.html')
