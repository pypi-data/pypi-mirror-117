...
class CminSampler:
    def __init__(self, variants, p=0):
        ...
        count = self.multiplicity / self.capacity
        self.eo_counts = np.floor(count)
        remainder = count - self.eo_counts
        self.multiplicity = (remainder*self.capacity).astype(int)
    ...
    def sample(self, max_seq, tsp_sort=True):
        """
        ...
        """
        sampler = self.samplingWithEucl(max_seq)
    ...
    def samplingWithEucl(self, max_seq):
        ...
        while len(output_count) != self.p - self.eo_counts.sum():
            ...
            neigh = NearestNeighbors(n_neighbors=min(self.capacity,ldata.shape[0]-1))
            neigh.fit(ldata)
