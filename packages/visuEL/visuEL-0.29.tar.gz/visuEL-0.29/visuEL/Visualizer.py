...
class Vis:
    ...
    square_s = 50
    margin_legend = 30
    colors = ['#000000','#3b156e','#8b2d80','#dc4b6a','#fc9f72', '#eeeeee']

    def __init__(self, legend=None, mapping_name=None, title=None, width_in_block=20):
        """
    ...
    def extract_legend(self, seqs):
        ...
        # Assign color
        y['color'] = self.colors[-1]
        y.loc[y.iloc[0:self.max_n_color].index, 'color'] = self.colors[:y.shape[0]]
        ...
        if y.shape[0] > self.max_n_color:
            y.loc[y.iloc[self.max_n_color-1:].index, 'name'] = '+ {} others...'.format(y.shape[0]-self.max_n_color+1)
            y.loc[y.iloc[self.max_n_color-1:].index, 'color'] = self.colors[-1]
