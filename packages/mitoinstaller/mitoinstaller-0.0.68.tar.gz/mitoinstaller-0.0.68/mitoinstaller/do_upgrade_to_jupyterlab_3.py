...
from mitoinstaller.install import do_install_or_upgrade, get_jupyterlab_metadata
from mitoinstaller.commands import jupyter_labextension_list, uninstall_labextension, uninstall_pip_packages
...
def do_upgrade_to_jupyterlab_3():
    ...
    # First, uninstall mitosheet
    uninstall_pip_packages('mitosheet')
    # Then, uninstall the JLab manager
    uninstall_labextension('@jupyter-widgets/jupyterlab-manager')

    # Log that we finished the upgrade
    log('upgrade_to_jupyterlab_3')

    # Then, install mitosheet
