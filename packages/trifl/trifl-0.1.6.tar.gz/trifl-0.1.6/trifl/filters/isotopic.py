...
@f(requires=['num_ms2', 'ratio', 'clean_sequence'], level=['experiment', 'ratio', 'aggregate'])
def is_not_singleton_with_single_sequence(
    num_ms2: list[int],
    ratio: list[float],
    clean_sequence: list[str],
    min_num_sequences: int = 2
) -> list[bool]:
    num_unique_sequences = len(set(clean_sequence))
    return [
        n > 1 or r != 20 or num_unique_sequences >= min_num_sequences
        for n, r
        in zip(num_ms2, ratio)
    ]
