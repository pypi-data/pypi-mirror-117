...
def __user_console_select(devs):
    ...
    while not user_done:
        ...
        print('Please enter the number of the connected index , multiple sequences separated by spaces,Or directly enter, all the default configuration')
...
async def onebot_getdevices_process():
    ...
    if scan_stats:
        ...
        if len(devices) == 0:
            print(
                "\n" + 'Please Turn on the master motor power switch or Please insert the Bluetooth adapter')
            sys.exit('aiqi quit')
