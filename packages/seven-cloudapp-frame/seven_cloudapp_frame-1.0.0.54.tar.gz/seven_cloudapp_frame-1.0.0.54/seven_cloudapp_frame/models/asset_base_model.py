...
@LastEditTime: 2021-08-19 11:32:26
...
class AssetBaseModel():
    ...
    def get_user_asset_list(self, app_id, act_id, user_ids, asset_type=0, open_ids="", sub_table=None):
        """
        ...
        :param asset_type：资产类型(1-次数2-积分3-价格档位)
        :param open_ids：open_ids 多个逗号,分隔
        :param sub_table：分表名称
        :return: 返回list
        :last_editors: HuangJianYi
        """
        if not act_id or (not user_ids and not open_ids):
            return []
        condition = "act_id=%s"
        params = [act_id]
        if asset_type > 0:
        ...
        if user_ids:
            if ',' in str(user_ids):
                condition += " AND user_id in(%s)"
            else:
                condition += " AND user_id=%s"
            params.append(user_ids)
        if open_ids:
            if ',' in str(open_ids):
                condition += " AND open_id in(%s)"
            else:
                condition += " AND open_id=%s"   
            params.append(open_ids)
