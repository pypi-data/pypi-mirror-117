...
@LastEditTime: 2021-08-27 13:38:11
...
class QueueUpHelper:
    ...
    def query(self, queue_name, user_id, expire_time=300):
        ...
        try:
            ...
            if score:
                ...
                query_user["queue_status"] = 1  #排队状态1排队中0未排队
                invoke_result_data.data = query_user
                return invoke_result_data
            else:
                query_user = {}
                query_user["queue_name"] = queue_name  #队列名称
                query_user["queue_status"] = 0  #排队状态1排队中0未排队
    ...
    def muti_query(self, user_id, expire_time=300):
        """
        :description: 批量查询某用户排队情况,如果只查用户已在排队的信息，则queue_name_list为空数据，如果查指定排队集合，不论是否在排，则user_id和queue_name_list都要传
        :param user_id：用户标识
        :param expire_time：过期时间，单位秒，为0不进行删除操作
        :return: 
        :last_editors: HuangJianYi
    ...
    def muti_queue_query(self, queue_name_list, user_id):
        """
        :description: 批量查询排队情况,如果只查用户已在排队的信息，则queue_name_list为空数据，如果查指定排队集合，不论是否在排，则user_id和queue_name_list都要传
        ...
        try:
            for queue_name in queue_name_list:
                query_invoke_result_data = self.query(queue_name, user_id, 0)
                invoke_result_data.data.append(query_invoke_result_data.data)
            return invoke_result_data
        except Exception as ex:
