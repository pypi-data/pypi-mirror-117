...
@LastEditTime: 2021-08-23 14:22:11
...
class QueueHelper:
    ...
    @classmethod
    def get_zset_name(self, queue_name):
        """
    ...
    @classmethod
    def get_list_name(self, queue_name):
        """
        :description: 获取列表名称
        :param queue_name：队列名称
        :return: 
    ...
    @classmethod
    def get_count_name(self, queue_name):
        """
    ...
    @classmethod
    def get_queue_no(self, queue_name):
        """
        :description: 获取排队号
        :param queue_name：队列名称
        :return: 
        :last_editors: HuangJianYi
        """
        count_name = self.get_count_name(queue_name)
    ...
    def get_queue_num(self, queue_name):
        ...
        :last_editors: HuangJianYi
        """
        zset_name = self.get_zset_name(queue_name)
        redis_init = SevenHelper.redis_init()
        return redis_init.zcard(zset_name)

    @classmethod
    def delete_expire_user(self, queue_name, expire_time):
        """
        ...
        try:
            ...
            redis_init = SevenHelper.redis_init()
            zset_name = self.get_zset_name(queue_name)
            list_name = self.get_list_name(queue_name)
    ...
    def queue(self, queue_name, user_id, user_nick, queue_length=100):
        ...
        try:
            ...
            data = str(user_id)
            redis_init = SevenHelper.redis_init()
            if self.get_queue_num(queue_name) >= queue_length:
                invoke_result_data.success = False
                invoke_result_data.error_code = "error"
                ...
                if not redis_init.zscore(zset_name, data):
                    ...
                    query_detail = {}
                    query_detail["queue_no"] = self.get_queue_no(queue_name)  #排队号
                    ...
                    query_user["queue_num"] = self.get_queue_num(queue_name)  #当前排队人数
    ...
    def query(self, queue_name, user_id, expire_time=10):
        ...
        try:
            ...
            #删除未操作的排队信息
            self.delete_expire_user(queue_name, expire_time)
            ...
            if redis_init.zscore(zset_name, data):
                ...
                query_user["total_num"] = self.get_queue_num(queue_name)  #总排队人数
                query_user["queue_index"] = int(redis_init.zrank(zset_name, data)) + 1  #当前位置
                query_user["before_num"] = query_user["queue_index"] - 1  #排在前面的人数
                invoke_result_data.data = query_user
                return invoke_result_data
