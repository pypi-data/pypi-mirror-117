...
def generate_full_script(child_run: Run) -> str:
    with _codegen_utilities.use_custom_repr():
        ...
        featurization_enabled = (
            settings_obj.get("featurization", "off") != "off" or settings_obj.get("preprocess", False) is not False or
            is_timeseries
        )
        ...
        output.append(get_dataset_code(training_dataset_id))
        output.append("\n")
        ...
        if is_timeseries:
            ...
            output.append(get_model_code(pipeline, featurization_enabled))
            output.append("\n")
            ...
            output.append(get_model_code(pipeline, featurization_enabled))
        output.append("\n")
        output.append(get_train_model_code(is_timeseries))
        output.append("\n")
...
def get_dataset_code(dataset_id: str) -> str:
    output = [
...
def get_prepare_data_code(
    ...
    output.append(
        ...
        " is_timeseries={}, target_column=label_column_name)".format(
            is_timeseries
        )
    )
...
def get_transformer_code(
    task_type: str, data_transformer: DataTransformer
) -> str:
    ...
    output.append("dt._engineered_feature_names_class = pickle.loads({!r})".format(
        pickle.dumps(data_transformer._engineered_feature_names_class)))
    output.append("")
...
def get_model_code(sklearn_pipeline: Pipeline, featurization_enabled: bool) -> str:
    output = []
    ...
    if isinstance(model, (PreFittedSoftVotingRegressor, PreFittedSoftVotingClassifier)):
        ...
        output.append("\n")
        output.append(get_build_model_pipeline_code(True, featurization_enabled, False))
        ...
        func_output.append("meta_learner = {1}_{0}()".format(
            "meta", MODEL_FUNC_NAME
        ))
        func_output.append("")
        func_output.extend(pipeline_definitions)
        ...
        output.append(get_build_model_pipeline_code(False, featurization_enabled, has_preprocessor))

    return "\n".join(output)


def get_code_for_single_estimator(pipeline: Pipeline, name: Optional[Any] = None) -> List[str]:
    output = []
...
def get_modelwrapper_code(model: Tuple[str, Any], name: Optional[Any] = None,
                          extra_imports: Optional[List[Tuple[str, str, Any]]] = None) -> str:
    if name is None:
...
def get_build_model_pipeline_code(is_ensemble: bool, featurization_enabled: bool, has_preprocessor: bool) -> str:
...
def get_train_model_code(is_timeseries: bool) -> str:
    output = [
