...
class input_form(Famcy.FamcyBlock):
    ...
    def get_fblock_types(cls):
        ...
        """
        return ["pureInput", "urlBtn", "inputBtn", "inputPassword", "inputList", "singleChoiceRadioInput", "multipleChoicesRadioInput"]
    ...
    def generate_template_content(cls, fblock_values=None):
        ...
        """
        return {
            "main_desc": "some description some description some description some description some description some description ",
        ...
        <script type="text/javascript">
            for(var i=0; i < %s; i++) {
                $('#mb' + i + '%s').bind('click', (e) => {
                    ...
                    if (flag) {
                        Sijax.request('%s', ["%s", "%s", "%s", response_dict]);
                    }
                });
            }
        </script>
        """ % (context["id"], context["method"], action, input_html, main_button_html, len(context["main_button_name"]), context["id"], context["id"], context["submit_type"], context["id"], action, context["target_id"])
