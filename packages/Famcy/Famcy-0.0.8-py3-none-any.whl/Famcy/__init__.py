...
def update_object_id(content_object_list, file_path_key):
    """
    ...
    for i in range(len(content_object_list)):
        ...
        if not isinstance(content_object_list[i], list):
            content_object_list[i].update_id(file_path_key+"-%d"%i)
            continue

        for j in range(len(content_object_list[i])):
            content_object_list[i][j].update_id(file_path_key+"-%d-%d"%(i, j))
