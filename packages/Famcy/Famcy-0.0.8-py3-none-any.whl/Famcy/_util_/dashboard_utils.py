...
def dashboardSideBar(side_bar_title, side_bar_hierarchy, title_style="bx-game", side_bar_style={}):
    """
    ...
    <div class="l-navbar" id="nav-bar">
        <div class="nav">
            ...
            <button type="submit" name="side_bar_btn" value="logout" class="nav_link toggle_class display_flex">
                <i class='bx bx-log-out nav_icon'></i>
                <span class="nav_name">登出</span>
            </button>
        </div>
    </div>
    </form>""" % (title_style, side_bar_title, btn_html)

def dashboardNavBar(user_name, user_img_path):
    return"""<div class="nav_bar">
        <div class="header_toggle">
            <i class='bx bx-menu' id="header-toggle"></i>
        </div>
        <button class="member_section" onclick="window.location.href='/profile'">
            <img src="%s" id="user_icon">
            <h3 id="user_name">%s</h3>
        </button>
    </div>
    <script src='%s/static/js/side_bar.js' type="text/javascript"></script>""" % (user_img_path, user_name, current_app.config.get("main_url", ""))
...
def generate_section_content(header_dict, content_object, path=""):
    ...
    for sec_size, sec_title, content_obj in zip(header_dict["size"], header_dict["title"], content_object):
        ...
        if type(content_obj) == list:
            temp_dict["target_id"] = "b_" + content_obj[0].context["id"]
            temp_dict["tab"] = path
            content_temp = ""
            temp_dict["block_id"] = []
            for content_sub_obj in content_obj:
                content_sub_obj.context["target_id"] = content_obj[0].context["target_id"]
            ...
            temp_dict["target_id"] = "b_" + str(content_obj.context["id"])
...
def user_defined_contents(path):
    """
    ...
    return generate_section_content(header_dict, content_object, path="dashboard/"+path)
