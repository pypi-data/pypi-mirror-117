...
from grid.sdk._gql.queries import (
    complete_datastore_upload,
    complete_multipart_datastore_upload,
    create_datastore,
    get_datastore_upload_multipart_presigned_urls,
)
...
class DatastoreUploadSession(UploadProgressCallback):
    ...
    def configure(self, session_id: str = ""):
        self.lock = Lock()
    ...
    def _get_presigned_urls(self):
        ...
        result = get_datastore_upload_multipart_presigned_urls(
            path=self.uploaded_target_file, datastore_id=self.id, count=self.part_count
        )
        self.upload_id = result['uploadId']

        presigned_map = {}
        for url in result['presignedUrls']:
    ...
    def _mark_upload_complete(self):
        ...
        complete_multipart_datastore_upload(
            ...
            path=self.uploaded_target_file,
        ...
        complete_datastore_upload(datastore_id=self.id, size_mib=sizeMib)
