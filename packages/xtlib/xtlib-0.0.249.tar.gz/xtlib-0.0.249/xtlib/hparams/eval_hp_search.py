...
class Run():
    def __init__(self, run_summary, primary_metric, verbose=False, use_azure_storage=False):
...
class SearchHistory():
    ...
    def __init__(self, args=None, agg_name=None, workspace=None, timeout=None, primary_metric=None, 
        hp_config_file_name=None, max_workers=1, max_runs=0, xt_config=None, verbose=False, use_azure_storage=False):
    ...
    def load_runs_from_db(self):
        ...
        self.init_xtstore()
        filter_dict = {"job_id":self.args.job, "end_id":{"$gt": 0}, 'status':'completed', "metrics.hpmax":{"$exists": True}, 'is_parent':False}
        fields_dict = {'run_name':1, 'end_id':1, 'hparams':1, 'metrics.hpmax':1}
        ...
        for run_summary in run_summaries:
            run = Run(run_summary, self.args.primary_metric, verbose=self.args.verbose)
            if run.skip:
                continue
            ...
            if global_runset.dn != best_runset:
                ...
                if self.args.verbose:
                    ...
                    if best_runset.post_mean is None:
                        console.print("    HPMAX = None")
                    else:
                        console.print("    HPMAX = {:9.3f}".format(best_runset.post_mean))
    ...
    def evaluate(self):
        ...
        fixed_names = ['RUNSET', 'SIZE', 'BEST_FOR', 'EST_HPMAX', 'BEST_RUN']
