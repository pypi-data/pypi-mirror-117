...
class DGD(object):
    ...
    def define_hyperparameters(self, records):
        ...
        for record in records:
            ...
            # all our values are string but we only accept
            # hparams with a list of values (containing commas)
            if not "," in value_string:
                console.print("skipping hparam with one value: {}={}".format(name_string, value_string))
                continue
