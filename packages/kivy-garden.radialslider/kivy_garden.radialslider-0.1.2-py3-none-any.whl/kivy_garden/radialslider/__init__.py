...
The :class:`RadialSlider` is an angular slider widget, which acts similarly to
the linear slider. Thumb movement is limited to a circular path, with an angle
ranging from 0 to 360 degrees

The radial slider has two components: a track and a thumb. The thumb can be
dragged along the circular path of the track.

You can define the position of the thumb via the angle, the value, or the
normalized value. These values are also automatically updated as the thumb is
dragged.

"""

__all__ = ('RadialSlider', )
