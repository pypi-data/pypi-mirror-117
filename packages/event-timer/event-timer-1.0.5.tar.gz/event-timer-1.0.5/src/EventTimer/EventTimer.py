...
def parse_config():
    parser = ConfigParser()
    parser.read(argv.config)
    counters = []
    for item in parser.items():
        if item[0] != 'DEFAULT':
            event_name, event_data = item[0], item[1]
            counter = delta_seconds(event_data)
            counters.append((counter, event_data))
    return counters
...
def main():
    ...
    parser.add_argument('-c', '--config', type=str, help="Event Configs File (Default to ~/.event.cfg)")
