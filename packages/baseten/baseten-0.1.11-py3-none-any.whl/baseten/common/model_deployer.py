...
def build_baseten_scaffold(
    ...
    logger.info(f'To build this model server locally execute '
                f'`{b10_scaffold.docker_build_string} -t {model_name.lower()}`')
    logger.info(f'To run this model server locally execute `docker run {model_name.lower()}`')
    logger.info(f'To use the Python shell locally execute `docker run -it {model_name.lower()} python3`')
    return b10_scaffold
...
def build_baseten_custom_scaffold(
    ...
    logger.info(f'To run this model server locally execute `docker run {model_name.lower()}`')
    logger.info(f'To use the Python shell locally execute `docker run -it {model_name.lower()} python3`')
    logger.info(f'Be sure to read the `README.md` for guidance on construction of a working custom scaffold')
    return b10_scaffold
...
def deploy_baseten_scaffold(
    ...
    scaffold_resp = api.create_model_from_scaffold(
        model_name,
        s3_key,
        'sklearn',
        'random_forest',
        b10_scaffold.model_framework_requirements,
        b10_scaffold.dockerfile_path,
        version_bump
    )
    # endpoint to send this s3 key and then whatever other artifacts
    deployed_model = None
    return s3_key, deployed_model, scaffold_resp
