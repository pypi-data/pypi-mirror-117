...
def sample_g(sub_r: list, n: int, i_dim:int, rng) -> np.ndarray:
    '''
...
def g(s: np.array, Y, test_function,Y_p:list) -> list:  #True values
    ...
    for k in range(0,len(s)):
        ...
        for i in range(len(s[k])-len(Y_p[k])):#####################
            current = test_function(np.array(s[k][i])) #VECTOR
            Y[k].append(current)
        if len(Y_p[k])!=0:####################
            Y[k]+=Y_p[k]##################
        Y_all += Y[k]
...
def model_construction(s: np.array, Y: list, v_s: int, sub_r: list, lower, upper, level: list, z: int, q: int, i_dim: int, fal_num:int, n_model:int,alpha:float, list_star:list, root_path, rng) -> Tuple[np.array,np.array]:   ##Gaussian process 
    '''
    ...
    for i in range(len(s)):
        ...
        name =  root_path + '/models/'+'trainmodel' +str(0)+ str(q+1) +str(0)+ str(z+1) +str(0)+ str(list_star[i])+'.m'  #save the models # modify your name here
        joblib.dump(gp, name) 
...
def _copy_all_files(source_directory: str, target_directory: str) -> None:
    source_directory = pathlib.Path(source_directory)
    target_directory = pathlib.Path(target_directory)

    for source_file in source_directory.iterdir():
        target_file = target_directory.joinpath(source_file.name)
        shutil.copyfile(str(source_file.resolve()), str(target_file.resolve()))


def replicate(data):

    start_time = time.time()
    q, sub_u, i_dim, i_B, alpha, n ,level, iteration, min_volume, max_budget, fal_num, n_model, n_bo, n_b, sample_method, part_num, root_path, seed = data
    ...
    for z in range(iteration):  ##change for iteration
        ...
        v = vol(sub_u,i_dim)
        if min(v_min) > min_volume*v_s and D + i_B*(n_bo+n)*len(sub_u) < max_budget: #0.01: #v(sub_u)
            ...
            for p in range(len(s)):#############
                sub_all+=[sub_r[p]]*len(s[p])################
                number_all+=[list_star[p]]*len(s[p])##############
            #print(true_fal))
            Y_subm = [[] for i in range(len(s))]
            num = [[0]*i_dim for i in range(len(s))]
            num_min = []
            ...
            model_lower, model_upper,level_quantile = model_construction(s, Y, v_s, sub_r, lower, upper, level, z, q, i_dim, fal_num, n_model,alpha,list_star, root_path, rng)
            ...
            num_m = []###########
            if len(tpn)!=0:
                num_p = ['theta_plus']*len(tpn)  ##############
            if len(tmn)!=0:
                num_m = ['theta_minus']*len(tmn)###############
            if len(tun)!=0:#############
                num_u = ['theta_undefined']*len(tun)#############
            if min(v_min) < min_volume*v_s or D >= max_budget:
                tp = tpn+tmn+tun
                classify = num_p +num_m+ num_u
            if min(v_min) > min_volume*v_s and D < max_budget:
                tp = tpn+tmn
                classify = num_p +num_m 
            Z = []
            Q = []
            list_region = []
            if len(tp)!= 0:
            ...
            _copy_all_files(root_path + '/models', root_path + '/all_gp_result')
            # for m in range(len(list_region)):
            #     shutil.move( root_path + '/models/'+'trainmodel'+str(0)+str(q+1)+str(0)+str(z+1) + str(0)+str(list_region[m])+'.m', root_path + '/all_gp_result/' +'trainmodel'+str(0)+str(q+1)+str(0)+str(z+1) + str(0)+str(list_region[m])+'.m')  
            subregion = pd.DataFrame({'subregion': theta_d,'replication': Q,'deepth': Z,'number':list_region, 'classify':classify })
            Q_all = [q+1]*len(X_all)
            Z_all = [z+1]*len(X_all)
            subregion.to_csv(root_path + '/subregions.csv', mode='a', header=False)
            points = pd.DataFrame({'X': X_all,'Y': Y_all, 'subregion': sub_all, 'replication': Q_all,'deepth': Z_all,'number':number_all})##############
...
#             print(S)
            ...
            lower=[]
            upper=[]
            model_lower, model_upper,level_quantile = model_construction(s_c, Y_c, v_s, sub_r, lower, upper, level, z, q, i_dim, fal_num, n_model,alpha, list_star, root_path, rng)
            p_quantile = [[] for i in range(len(level))]
            p_quantile = part_percent(level, sub_r, v_s,level_quantile, p_quantile,i_dim)
            theta_undefined = []
            tpn = []
            ...
            _copy_all_files(root_path + '/models', root_path + '/all_gp_result')
            # for m in range(len(list_region)):
            #     shutil.move( root_path + '/models/'+'trainmodel'+str(0)+str(q+1)+str(0)+str(z+1) + str(0)+str(list_region[m])+'.m', root_path + '/all_gp_result')  
            subregion = pd.DataFrame({'subregion': theta_d,'replication': Q,'deepth': Z,'number':list_region, 'classify':classify })
            Q_all = [q+1]*len(X_all)
            Z_all = [z+1]*len(X_all)
            points = pd.DataFrame({'X': X_all,'Y': Y_all, 'subregion': sub_all, 'replication': Q_all,'deepth': Z_all,'number':number_all})##############
    ...
    return [P, K, TPP, TMP, TPV, TMV, TUV, H, S, evl, p_iter, Y_minf, X_minf, t_fal, number_subregion, replication_time]

def _part_classify(sub_u: list, i_dim: int, i_B: int, 
    alpha: float, test_function: Callable[[], float], n:int,level:list,replication:int, iteration: int,min_volume:float,max_budget:int, 
    fal_num: float,n_model: int,n_bo:int, n_b:int, sample_method,
    part_num:int, root_path: str, start_seed):
    '''
    algorithm
    ...
    for q in range(replication):
        seed = start_seed + q
        data = (q, sub_u, i_dim, i_B, alpha, n , level, iteration, min_volume, max_budget, fal_num, n_model, n_bo, n_b, sample_method, part_num, root_path, seed)
        inputs.append(data)
    ...
    for index, result in enumerate(results):
        __P, __K, __TPP, __TMP, __TPV, __TMV, __TUV, __H, __S, __evl, __p_iter, __Y_minf, __X_minf, __t_fal, __number_subregion, replication_time = result
...
def part_optimize_with_controlled_budget(
    subregion_file: str,
    ...
    tpv, tmv, tuv, s, h, evl, p_iter, number_subregion = _part_classify(
        sub_u=subregion,
...
# ######example
# sub_u =[[[-1,1],[-1,1]]]#[[[0,20],[0,20]]] #parameter space  
# i_dim = 2 #dimension
# i_B = 2     #parts want to partition, default = 2
...
#     return (1+(X[0]+X[1]+1)**2*(19-14*X[0]+3*X[0]**2-14*X[1]+6*X[0]*X[1]+3*X[1]**2))*(30+(2*X[0]-3*X[1])**2*(18-32*X[0]+12*X[0]**2+48*X[1]-36*X[0]*X[1]+27*X[1]**2))-50

# TPV,TMV,TUV, S, H, evl, p_iter, number_subregion  = Part_classify(sub_u, i_dim, i_B, alpha, test_function, 20,level, 1, 100 ,0.001 ,30000, 15000,20,20,100,'BO Sampling',1)
