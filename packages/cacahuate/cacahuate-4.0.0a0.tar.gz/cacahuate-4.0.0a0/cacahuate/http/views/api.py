...
from coralillo.queryset import FILTERS
...
ALLOWED_REDIS_FILTERS = REDIS_FIELDS + [
    ('{fld}__{fltr}'.format(fld=fld, fltr=fltr), tp)
    for fld, tp in REDIS_FIELDS for fltr in FILTERS
]
...
def execution_list_search():
    ...
    # redis filters
    redis_query = {}
    for k, t in ALLOWED_REDIS_FILTERS:
        if k not in exe_query:
            continue

        val = exe_query.pop(k, None)
        if t == 'datetime':
    ...
    if redis_query:
        exe_query['id'] = {
            '$in': list(map(
                lambda x: x.id,
                Execution.q().filter(**redis_query),
            ))
        }
...
def pointer_list_search():
    ...
    redis_query = {}
    for k, t in ALLOWED_REDIS_FILTERS:
        if k not in ptr_query:
            continue

        val = ptr_query.pop(k, None)
        if t == 'datetime':
            if not isinstance(val, datetime.datetime):
                try:
                    val = datetime.datetime.strptime(
