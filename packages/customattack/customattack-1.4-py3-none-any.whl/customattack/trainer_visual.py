...
logger = customattack.shared.logger
...
class TrainerVisual:
    ...
    def __init__(
        ...
        ), f"`model_wrapper` must be of type `customattack.models.wrappers.ModelWrapper`, but got type `{type(model_wrapper)}`."
        ...
        if train_dataset:
            assert isinstance(
                train_dataset, customattack.datasets.Dataset
            ), f"`train_dataset` must be of type `customattack.datasets.Dataset`, but got type `{type(train_dataset)}`."

        if eval_dataset:
            assert isinstance(
                eval_dataset, customattack.datasets.Dataset
            ), f"`eval_dataset` must be of type `customattack.datasets.Dataset`, but got type `{type(eval_dataset)}`."

        if training_args:
            assert isinstance(
                training_args, TrainingArgs
            ), f"`training_args` must be of type `customattack.TrainingArgs`, but got type `{type(training_args)}`."
        else:
            training_args = TrainingArgs()
    ...
    def _generate_adversarial_examples(self, epoch):
        ...
        adversarial_dataset = customattack.datasets.Dataset(
            adversarial_examples,
    ...
    def _save_model_checkpoint(
        self, model,  step=None, epoch=None, best=False, last=False
    ):
    ...
    def get_train_dataloader(self, dataset, adv_dataset, batch_size):
        ...
        Args:
            dataset (:class:`~customattack.datasets.Dataset`):
                Original training dataset.
            adv_dataset (:class:`~customattack.datasets.Dataset`):
        ...
        def collate_fn(data):
            ...
            return input_images, torch.tensor(targets), torch.tensor(is_adv_sample)
    ...
    def get_eval_dataloader(self, dataset, batch_size):
        ...
        def collate_fn(data):
            ...
            for _input, label in data:
                ...
                input_images.append(_input)
                targets.append(label)
            return input_images, torch.tensor(targets)
    ...
    def training_step(self, model, batch):
        ...
        input_images, targets, is_adv_sample = batch
        _targets = targets
        targets = targets.to(customattack.shared.utils.device)
        input_images = torch.tensor(input_images)
        input_images = input_images.to(customattack.shared.utils.device)
        ...
        #         truncation=True,
        #     )
        #     input_ids.to(customattack.shared.utils.device)
        #     logits = model(**input_ids)[0]
        # else:
        #     input_ids = tokenizer(input_images)
        #     if not isinstance(input_ids, torch.Tensor):
        #         input_ids = torch.tensor(input_ids)
        #     input_ids = input_ids.to(customattack.shared.utils.device)
        #     logits = model(input_ids)

        # from fetchCIFAR10 import fetchCIFAR10
        input_images, targets = fetchCIFAR10()
        ...
        sample_weights = torch.ones(
            is_adv_sample.size(), device=customattack.shared.utils.device
        )
    ...
    def train(self):
        ...
        customattack.shared.utils.set_seed(self.training_args.random_seed)
        ...
        model.to(customattack.shared.utils.device)
        ...
        for epoch in range(1, self.training_args.num_epochs + 1):
            ...
            if self.attack and epoch > num_clean_epochs:
                if (
                    ...
                    # adv_dataset is instance of `customattack.datasets.Dataset`
                    model.eval()
            ...
            for step, batch in enumerate(prog_bar):
                loss, preds, targets = self.training_step(model,  batch)
            ...
            if (
                ...
                self._save_model_checkpoint(model, epoch=epoch)

            if eval_score > best_eval_score:
                best_eval_score = eval_score
                best_eval_score_epoch = epoch
                epochs_since_best_eval_score = 0
                self._save_model_checkpoint(model,  best=True)
                logger.info(
        ...
        if self.training_args.save_last:
            self._save_model_checkpoint(model,  last=True)
    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            for step, batch in enumerate(eval_dataloader):
                preds, targets = self.evaluate_step(model,  batch)
                all_preds.append(preds)
                all_targets.append(targets)
    ...
    def _write_readme(self, best_eval_score, best_eval_score_epoch, train_batch_size):
        ...
        if isinstance(
            self.train_dataset, customattack.datasets.HuggingFaceDataset
            ...
            self.eval_dataset, customattack.datasets.HuggingFaceDataset
...
def fetchCIFAR10():
    ...
    trainset = torchvision.datasets.CIFAR10(root='/home/local/ASUAD/ujeong1/PycharmProjects/Adversarial_UI/datasets', train=True,
