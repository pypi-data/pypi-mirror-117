...
import customattack
from customattack.constraints import Constraint
from customattack.shared.validators import transformation_consists_of_word_swaps

# Set global flair device to be TextAttack's current device
flair.device = customattack.shared.utils.device

stanza = customattack.shared.utils.LazyLoader("stanza", globals(), "stanza")
        ...
        if context_key in self._pos_tag_cache:
            ...
            if self.tagger_type == "flair":
                context_key_sentence = Sentence(
                    context_key, use_tokenizer=customattack.shared.utils.words_from_text
                )
                self._flair_pos_tagger.predict(context_key_sentence)
                word_list, pos_list = customattack.shared.utils.zip_flair_result(
                    context_key_sentence
                )

            if self.tagger_type == "stanza":
                word_list, pos_list = customattack.shared.utils.zip_stanza_result(
