...
def get_textattack_model_num_labels(model_name, model_path):
    ...
    model, if present."""
    model_cache_path = customattack.shared.utils.download_from_s3(model_path)
    train_args_path = os.path.join(model_cache_path, "train_args.json")
    if not os.path.exists(train_args_path):
        customattack.shared.logger.warn(
...
def load_textattack_model_from_path(model_name, model_path):
    ...
    if model_name.startswith("lstm"):
        ...
        model = customattack.models.helpers.LSTMForClassification(
        ...
        model = customattack.models.helpers.WordCNNForClassification(
            model_path=model_path, num_labels=num_labels
        )
    elif model_name.startswith("t5"):
        model = customattack.models.helpers.T5ForTextToText(model_path)
    else:
        raise ValueError(f"Unknown customattack model {model_path}")
    return model
