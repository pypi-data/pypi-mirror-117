...
class ClassificationGoalFunctionResult(GoalFunctionResult):
    ...
    def _processed_output(self):
        ...
        if self.attacked_text.attack_attrs.get("label_names"):
            ...
            output = customattack.shared.utils.process_label_name(output)
            color = customattack.shared.utils.color_from_output(output, output_label)
            return output, color
        else:
            color = customattack.shared.utils.color_from_label(output_label)
