...
from customattack.datasets import HuggingFaceDataset
from customattack.models.helpers import LSTMForClassification, WordCNNForClassification, resnet_for_classfication
from customattack.models.wrappers import (
    HuggingFaceModelWrapper,
    ModelWrapper,
    PyTorchModelWrapper,
    PyTorchVisualModelWrapper
)
from customattack.shared import logger
from customattack.shared.utils import ARGS_SPLIT_TOKEN
...
class TrainingArgs:
    ...
    Args:
        ...
        attack_num_workers_per_device (:obj:`int`, defaults to `optional`, :obj:`1`):
            Number of worker processes to run per device for attack. Same as :obj:`num_workers_per_device` argument for :class:`~customattack.AttackArgs`.
        ...
        wandb_project (:obj:`str`, `optional`, defaults to :obj:`"customattack"`):
    ...
    wandb_project: str = "customattack"
...
class _CommandLineTrainingArgs:
    ...
    def _create_model_from_args(cls, args):
        ...
        if args.model_name_or_path == "lstm":
            logger.info("Loading customattack model: LSTMForClassification")
            ...
            logger.info("Loading customattack model: WordCNNForClassification")
            max_seq_len = args.model_max_length if args.model_max_length else 128
            num_labels = args.model_num_labels if args.model_num_labels else 2
            ...
            model = PyTorchModelWrapper(model, model.tokenizer)
        elif args.model_name_or_path == "resnet50":
            logger.info("Loading customattack model: Resnet50")
            # max_seq_len = args.model_max_length if args.model_max_length else 128
            # num_labels = args.model_num_labels if args.model_num_labels else 2
            model = resnet_for_classfication.resnet50()
            model = PyTorchVisualModelWrapper(model)
        # else:
        #     import transformers
        #
        #     logger.info(
    ...
    def _create_attack_from_args(cls, args, model_wrapper):
        ...
        assert isinstance(
            attack, Attack
        ), "`attack` must be of type `customattack.Attack`."
        return attack
