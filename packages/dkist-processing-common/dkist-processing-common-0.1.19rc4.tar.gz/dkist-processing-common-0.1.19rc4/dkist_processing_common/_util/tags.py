...
class TagDB:
    ...
    def any(self, tags: Union[Iterable[str], str]) -> Set[str]:
        ...
        # Constants use str, paths use list
        if isinstance(tags, list) and len(r) == 0:
            logging.warning(f"No files found matching ANY tags in {tags=}")
    ...
    def all(self, tags: Union[Iterable[str], str]) -> Set[str]:
        ...
        # Constants use str, paths use list
        if isinstance(tags, list) and len(r) == 0:
            logging.warning(f"No files found matching the set of {tags=}")
        return self._format_query_result(r)
