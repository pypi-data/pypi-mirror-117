...
class OAPDash(dash.Dash):
    ...
    def _serve_report_main_js(self):
        ...
        if os.getenv('ON_PROXY') == 'true':
            app_service_base_url = 'https://localhost' + app_service_base_url[app_service_base_url.index('/oap-api'):]

        oap_app_id = os.getenv('OAP_APP_ID')
        body += f"var OAP_API_URL = '{app_service_base_url}';\n"
