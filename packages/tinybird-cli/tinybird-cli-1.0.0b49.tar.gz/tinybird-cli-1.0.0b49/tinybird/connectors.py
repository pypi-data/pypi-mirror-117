...
class BigQuery(Connector):
    ...
    def datasource_analyze(self, resource, type_guessing=False):
...
class Snowflake(Connector):
    ...
    def datasource_analyze(self, resource, type_guessing=False):
        ...
        # TODO: deal with Nullables and the right precisions for Integers
        def from_snowflake_type(t):
        ...
        for row in result:
            if row['kind'] == 'COLUMN':
                sql.append(f"{row['name']} {from_snowflake_type(row['type'])}")
        return {
            'sql_schema': ', '.join(sql)
        }
