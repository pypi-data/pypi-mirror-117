...
from notetool.path import list_file
...
def merge_all_symbol(period='1min',
                     data_type=ALL_DATA_TYPES[0],
                     date=datetime(2021, 5, 21),
                     download_dir='/root/workspace/tmp/coin/merge/'):
    ...
    file_name = f'{history.data_type}_{history.type}_{period}-{date.strftime("%Y%m%d")}.csv'

    new_file = open(os.path.join(download_dir, file_name), 'w')
    for item in list_file(download_dir2, deep=10):
        for txt in open(item, 'r'):
            new_file.write(txt)

    new_file.close()
    os.removedirs(download_dir2)


def load_symbol(symbol='SHIBUSDT', start_date=datetime(2021, 5, 1),
