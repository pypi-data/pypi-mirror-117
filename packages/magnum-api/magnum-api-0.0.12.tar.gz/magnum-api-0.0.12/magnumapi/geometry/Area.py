...
class Area:
   ...
   Attributes:
    ...
    def of_closed_ordered_list_of_points(points: Tuple[Point]) -> "Area":
        #ToDo: add test
        #ToDo: add docstring
        #ToDo: replace in code whenever a closed area is made from points
        lines = []
    ...
    @staticmethod
    def is_outside_of_first_quadrant(area: "Area") -> bool:
        ...
        """
        is_x_nonegative = all([(line.p1.x >= 0) and (line.p2.x >= 0) for line in area.lines])
        is_y_nonegative = all([(line.p1.y >= 0) and (line.p2.y >= 0) for line in area.lines])
