...
from magnumapi.geometry.CosThetaBlock import RelativeCosThetaBlock, CosThetaBlock
...
class TestCosThetaBlock(TestCase):
    cadata_file_path = create_resources_file_path('resources/geometry/roxie/16T/roxieold_2.cadata')
...
#  No Name       Type CableGeom.  Strand    Filament  Insul     Trans     QuenchMat.           T_o  Comment
    ...
    def test_get_first_isosceles_trapezium_rect_no_insulation_no_radius_no_phi_no_alpha(self, mock_show=None):
        block_rel_def = RelativeCosThetaBlockDefinition(radius=0, phi_r=0.0, alpha_r=0.0, nco=1, condname='16TIL6B', no=1,
        ...
        p_ref = Point.of_polar(block.block_def.radius, block.phi_r)
        area_ins = block.get_insulated_isosceles_trapezium(p_ref=p_ref,
                                                           alpha_ref=block.alpha_r)
        area_shift = area_ins.copy()

        if block.is_area_intersecting_or_within_radius(area_ins):
            area_shift = area_ins.translate(Point.of_polar(2 * block.cable_def.thickness_i, block.alpha_r))
        ...
        area_bare = block.get_bare_isosceles_trapezium(p_ref=area_shift.get_line(0).p1,
                                                       alpha_ref=Line.calculate_relative_alpha_angle(area_shift.get_line(0)))
...
#  No Name       Type CableGeom.  Strand    Filament  Insul     Trans     QuenchMat.           T_o  Comment
    ...
    def test_get_first_isosceles_trapezium_rect_with_insulation_with_radius_no_phi_no_alpha(self, mock_show=None):
        block_rel_def = RelativeCosThetaBlockDefinition(radius=25, phi_r=0.0, alpha_r=0.0, nco=1, condname='16TIL6C', no=1,
                                                        current=13500, type=1, n1=2, n2=20, imag=0, turn=0)
        block = RelativeCosThetaBlock(block_def=block_rel_def,
                                      cable_def=self.cadata.get_cable_definition('16TIL6C'),
                                      insul_def=self.cadata.get_insul_definition('16TIL6C'),
...
#  No Name       Type CableGeom.  Strand    Filament  Insul     Trans     QuenchMat.           T_o  Comment
    ...
    def test_get_first_isosceles_trapezium_rect_with_insulation_with_radius_with_phi_no_alpha(self, mock_show=None):
        block_rel_def = RelativeCosThetaBlockDefinition(radius=25, phi_r=5.0, alpha_r=0.0, nco=1, condname='16TIL6C', no=1,
                                                        current=13500, type=1, n1=2, n2=20, imag=0, turn=0)
        block = RelativeCosThetaBlock(block_def=block_rel_def,
                                      cable_def=self.cadata.get_cable_definition('16TIL6C'),
                                      insul_def=self.cadata.get_insul_definition('16TIL6C'),
...
#  No Name       Type CableGeom.  Strand    Filament  Insul     Trans     QuenchMat.           T_o  Comment
    ...
    def test_get_first_isosceles_trapezium_trapz_no_insulation_no_radius_no_phi_no_alpha(self, mock_show=None):
        block_rel_def = RelativeCosThetaBlockDefinition(radius=0, phi_r=0.0, alpha_r=0.0, nco=1, condname='16TIL6D', no=1,
                                                        current=13500, type=1, n1=2, n2=20, imag=0, turn=0)
        block = RelativeCosThetaBlock(block_def=block_rel_def,
...
#  No Name       Type CableGeom.  Strand    Filament  Insul     Trans     QuenchMat.           T_o  Comment
    ...
    def test_get_first_isosceles_trapezium_trapz_with_insulation_no_radius_no_phi_no_alpha(self, mock_show=None):
        block_rel_def = RelativeCosThetaBlockDefinition(radius=0, phi_r=0.0, alpha_r=0.0, nco=1, condname='16TIL6E', no=1,
                                                        current=13500, type=1, n1=2, n2=20, imag=0, turn=0)
        block = RelativeCosThetaBlock(block_def=block_rel_def,
...
#  No Name       Type CableGeom.  Strand    Filament  Insul     Trans     QuenchMat.           T_o  Comment
    ...
    def test_get_first_isosceles_trapezium_traps_with_insulation_with_radius_no_phi_no_alpha(self, mock_show=None):
        block_rel_def = RelativeCosThetaBlockDefinition(radius=25, phi_r=0.0, alpha_r=0.0, nco=1, condname='16TIL6E', no=1,
                                                        current=13500, type=1, n1=2, n2=20, imag=0, turn=0)
        block = RelativeCosThetaBlock(block_def=block_rel_def,
                                      cable_def=self.cadata.get_cable_definition('16TIL6E'),
                                      insul_def=self.cadata.get_insul_definition('16TIL6E'),
        ...
        p_ref = Point.of_polar(block.block_def.radius, block.phi_r)
        area_ins = block.get_insulated_isosceles_trapezium(p_ref=p_ref, alpha_ref=block.alpha_r)
        area_ins_shift = area_ins.copy()
        if block.is_area_intersecting_or_within_radius(area_ins):
            area_ins_shift = area_ins.translate(Point.of_polar(2 * block.cable_def.thickness_i, block.alpha_r))
        ...
        # Phi0(2 - 3) = 0.0000
        # Alph0(2 - 3) = 0.0000
        self.assertAlmostEqual(4.9891, round(Line.calculate_positioning_angle(area_ins_shift.get_line(2), block.block_def.radius), 4), places=4)
        self.assertAlmostEqual(1.0003, round(Line.calculate_relative_alpha_angle(area_ins_shift.get_line(2)), 4), places=4)
        self.assertAlmostEqual(0.0000, Line.calculate_positioning_angle(area_ins_shift.get_line(0), block.block_def.radius), places=4)
        ...
        area_bare = block.get_bare_isosceles_trapezium(p_ref=area_ins_shift.get_line(0).p1,
                                                       alpha_ref=Line.calculate_relative_alpha_angle(area_ins_shift.get_line(0)))
...
#  No Name       Type CableGeom.  Strand    Filament  Insul     Trans     QuenchMat.           T_o  Comment
    ...
    def test_get_first_isosceles_trapezium_trapz_with_insulation_with_radius_with_phi_no_alpha(self, mock_show=None):
        block_rel_def = RelativeCosThetaBlockDefinition(radius=25, phi_r=5.0, alpha_r=0.0, nco=1, condname='16TIL6E', no=1,
                                                        current=13500, type=1, n1=2, n2=20, imag=0, turn=0)
        block = RelativeCosThetaBlock(block_def=block_rel_def,
                                      cable_def=self.cadata.get_cable_definition('16TIL6E'),
                                      insul_def=self.cadata.get_insul_definition('16TIL6E'),
        ...
        # Phi0(2 - 3) = 5.0000
        # Alph0(2 - 3) = 0.0000
        self.assertAlmostEqual(10.0199, Line.calculate_positioning_angle(area_ins_shift.get_line(2), block.block_def.radius), places=4)
        self.assertAlmostEqual(1.0003, Line.calculate_relative_alpha_angle(area_ins_shift.get_line(2)), places=4)
        self.assertAlmostEqual(5.0000, Line.calculate_positioning_angle(area_ins_shift.get_line(0), block.block_def.radius), places=4)
        self.assertAlmostEqual(0.0000, Line.calculate_relative_alpha_angle(area_ins_shift.get_line(0)), places=4)
...
#  No Name       Type CableGeom.  Strand    Filament  Insul     Trans     QuenchMat.           T_o  Comment
    ...
    def test_get_first_isosceles_trapezium_trapz_with_insulation_with_radius_with_phi_with_alpha(self, mock_show=None):
        ...
        # Phi0(2 - 3) = 5.0000
        # Alph0(2 - 3) = 10.0000
        self.assertAlmostEqual(9.9922, Line.calculate_positioning_angle(area_ins_shift.get_line(2), block.block_def.radius), places=4)
    ...
    def test_print_block(self):
        ...
        print_str = '(25.017258, 2.198711) -> (46.821732, 6.043428),' \
                    '(46.821732, 6.043428) -> (46.354814, 8.562654),' \
                    '(46.354814, 8.562654) -> (24.620782, 4.337872),' \
                    '(24.620782, 4.337872) -> (25.017258, 2.198711)'

        self.assertEqual(print_str, fake_out.getvalue().strip())
