...
class computeAudioQuality():
    ...
    def POLQA(self):
        ...
        curCH,curBwidth,curSR = self.__check_format(self.testFile)
        from algorithmLib.POLQA.POLQA import startvqt
        from algorithmLib.POLQA.POLQA import vqtDisConnect
        result =  startvqt(wav2pcm(self.refFile),wav2pcm(self.testFile),curSR)
        time.sleep(2)
        vqtDisConnect()
        return  result
