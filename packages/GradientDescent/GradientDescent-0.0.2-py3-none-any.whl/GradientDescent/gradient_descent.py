...
class Preprocess:
    ...
    def transform(self,values=None,add_ones=False)->np.array:
        """Transforms the data according to the formula x_norm = (x-mean(x))/std(x)

        Args:
            values (np.array, optional): Values to transform, if not given, it transforms the data passed to it in the constructor. Defaults to None.
            add_ones (bool, optional): Add ones for intercept. Defaults to False.

        Returns:
            np.array: array containing the transformed values 
        """              
        if values is None:
